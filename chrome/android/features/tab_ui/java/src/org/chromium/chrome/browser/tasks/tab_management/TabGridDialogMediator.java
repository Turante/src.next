...
public class TabGridDialogMediator
  ...
  interface DialogController {
    /**
     * Handles a reset event originated from {@link TabGridDialogMediator} and
     * {@link TabSwitcherMediator}.
     *
  ...
  /**
   * Defines an interface for a {@link TabGridDialogMediator} to get the source
   * {@link View} in order to prepare show/hide animation.
   */
  ...
  TabGridDialogMediator(
      Context context, DialogController dialogController, PropertyModel model,
    ...
    mTabModelObserver = new TabModelObserver() {
      ...
      public void willCloseTab(Tab tab, boolean animate) {
        ...
        // If current tab is closed and tab group is not empty, hand over ID of
        // the next tab in the group to mCurrentTabId.
      ...
      public void tabPendingClosure(Tab tab) {
        if (!mModel.get(TabGridPanelProperties.IS_DIALOG_VISIBLE))
          return;
        snackbarManager.showSnackbar(
            Snackbar
                .make(tab.getTitle(), TabGridDialogMediator.this,
  ...
  public void
  initWithNative(@Nullable TabSelectionEditorCoordinator
      ...
      if (result == R.id.ungroup_tab) {
        ...
        ShareParams shareParams =
            new ShareParams
                ...
                .build();
        // TODO(crbug.com/1085078): Sharing hub is suppressed for tab group
        // sharing. Re-enable it when tab group sharing is supported by sharing
        // hub.
    ...
    if (TabUiFeatureUtilities.isTabGroupsAndroidContinuationEnabled(mContext)) {
      ...
      mModel.set(TabGridPanelProperties.MENU_CLICK_LISTENER,
                 getMenuButtonClickListener());
    }
  }
    ...
    if (!showAnimation) {
      ...
      if (mAnimationSourceViewProvider != null &&
        ...
        mModel.set(TabGridPanelProperties.ANIMATION_SOURCE_VIEW,
                   mAnimationSourceViewProvider.getAnimationSourceViewForTab(
                       mCurrentTabId));
      }
    }
    if (mTabSelectionEditorController != null) {
  ...
  void onReset(@Nullable List<Tab> tabs) {
    ...
    if (mCurrentTabId != Tab.INVALID_TAB_ID) {
      ...
      mModel.set(TabGridPanelProperties.SCRIMVIEW_CLICK_RUNNABLE,
                 mScrimClickRunnable);
  ...
  private void updateGridTabSwitcher() {
    if (!isVisible() || mTabSwitcherResetHandler == null)
      return;
  ...
  private void updateDialog() {
    ...
    if (mTabGroupTitleEditor != null) {
      ...
      String storedTitle =
          mTabGroupTitleEditor.getTabGroupTitle(getRootId(currentTab));
    ...
    if (TabUiFeatureUtilities.isLaunchPolishEnabled()) {
      mModel.set(TabGridPanelProperties.COLLAPSE_BUTTON_CONTENT_DESCRIPTION,
                 mContext.getResources().getQuantityString(
                     ...
                     relatedTabs.size(), relatedTabs.size()));
    }
    mModel.set(
  ...
  private void updateColorProperties(Context context, boolean isIncognito) {
    int dialogBackgroundColor =
        TabUiThemeProvider.getTabGridDialogBackgroundColor(context,
                                                           isIncognito);
    ColorStateList tintList =
        isIncognito ? AppCompatResources.getColorStateList(
    ...
    int ungroupBarTextColor =
        TabUiThemeProvider.getTabGridDialogUngroupBarTextColor(context,
                                                               isIncognito);
    int ungroupBarHoveredTextColor =
        TabUiThemeProvider.getTabGridDialogUngroupBarHoveredTextColor(
            context, isIncognito);

    mModel.set(TabGridPanelProperties.DIALOG_BACKGROUND_COLOR,
    ...
    mModel.set(TabGridPanelProperties.DIALOG_UNGROUP_BAR_TEXT_COLOR,
               ungroupBarTextColor);
  ...
  private void updateDialogScrollPosition() {
    // If current selected tab is not within this dialog, always scroll to the
    // top.
    ...
    int initialPosition = Math.max(
        relatedTabs.indexOf(currentTab) - INITIAL_SCROLL_INDEX_OFFSET_GTS, 0);
  ...
  private void setupDialogSelectionEditor() {
    ...
    mTabSelectionEditorController.configureToolbar(
        actionButtonText,
        R.plurals.accessibility_tab_selection_dialog_remove_button,
        actionProvider, 1, null);
  }
  ...
  private void setupToolbarEditText() {
    ...
    mModel.set(TabGridPanelProperties.TITLE_TEXT_ON_FOCUS_LISTENER,
  ...
  private View.OnClickListener getAddButtonClickListener() {
    return view -> {
      ...
      mTabCreatorManager.getTabCreator(currentTab.isIncognito())
          .createNewTab(
              new LoadUrlParams("chrome-search://local-ntp/local-ntp.html"),
  ...
  private void saveCurrentGroupModifiedTitle() {
    ...
    if (mCurrentGroupModifiedTitle.length() == 0) {
      // When dialog title is empty, delete previously stored title and restore
      // default title.
      ...
      if (TabUiFeatureUtilities.isLaunchPolishEnabled()) {
        mModel.set(TabGridPanelProperties.COLLAPSE_BUTTON_CONTENT_DESCRIPTION,
                   mContext.getResources().getQuantityString(
                       R.plurals.accessibility_dialog_back_button, tabsCount,
                       tabsCount));
      }
    ...
    mTabGroupTitleEditor.storeTabGroupTitle(getRootId(currentTab),
                                            mCurrentGroupModifiedTitle);
    mTabGroupTitleEditor.updateTabGroupTitle(currentTab,
                                             mCurrentGroupModifiedTitle);
    if (TabUiFeatureUtilities.isLaunchPolishEnabled()) {
      ...
      mModel.set(TabGridPanelProperties.COLLAPSE_BUTTON_CONTENT_DESCRIPTION,
                 mContext.getResources().getQuantityString(
                     R.plurals.accessibility_dialog_back_button_with_group_name,
                     relatedTabsCount, mCurrentGroupModifiedTitle,
                     relatedTabsCount));
    }
  ...
  /**
   * A handler that handles TabGridDialog related changes originated from {@link
   * TabListMediator} and {@link TabGridItemTouchHelperCallback}.
   */
  class DialogHandler implements TabListMediator.TabGridDialogHandler {
    @Override
    public void
    updateUngroupBarStatus(@TabGridDialogView.UngroupBarStatus int status) {
  ...
  private String getTabGroupStringForSharing() {
    ...
    for (int i = 0; i < tabgroup.size(); i++) {
      sb.append(i + 1)
          .append(". ")
          .append(tabgroup.get(i).getUrl().getSpec())
          .append("\n");
    }
    return sb.toString();
  }
