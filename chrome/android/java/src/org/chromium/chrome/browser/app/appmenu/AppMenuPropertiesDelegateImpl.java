...
/**
 * Base implementation of {@link AppMenuPropertiesDelegate} that handles hiding
 * and showing menu items based on activity state.
 */
public class AppMenuPropertiesDelegateImpl
    implements AppMenuPropertiesDelegate {
  ...
  protected final MultiWindowModeStateDispatcher
   ...
   * @param activityTabProvider The {@link ActivityTabProvider} for the
   *     containing activity.
   * @param multiWindowModeStateDispatcher The {@link
   *     MultiWindowModeStateDispatcher} for the
   *         containing activity.
   * @param tabModelSelector The {@link TabModelSelector} for the containing
   *     activity.
   * @param toolbarManager The {@link ToolbarManager} for the containing
   *     activity.
   * @param decorView The decor {@link View}, e.g. from Window#getDecorView(),
   *     for the containing
   *         activity.
   * @param overviewModeBehaviorSupplier An {@link ObservableSupplier} for the
   *         {@link OverviewModeBehavior} associated with the containing
   * activity.
   * @param startSurfaceSupplier An {@link OneshotSupplier} for the Start
   *     surface.
   * @param bookmarkBridgeSupplier An {@link ObservableSupplier} for the {@link
   *     BookmarkBridge}
   *         associated with the containing activity.
   */
  public AppMenuPropertiesDelegateImpl(
      Context context, ActivityTabProvider activityTabProvider,
      MultiWindowModeStateDispatcher multiWindowModeStateDispatcher,
      TabModelSelector tabModelSelector, ToolbarManager toolbarManager,
      View decorView,
      @Nullable OneshotSupplier<OverviewModeBehavior>
    ...
    if (startSurfaceSupplier != null) {
      ...
      startSurfaceSupplier.onAvailable(
          mCallbackController.makeCancelable((startSurface) -> {
            mStartSurfaceState =
                startSurface.getController().getStartSurfaceState();
            mStartSurfaceStateObserver = (newState, shouldShowToolbar) -> {
              assert ReturnToChromeExperimentsUtil.isStartSurfaceEnabled(
                  mContext);
  ...
  public void destroy() {
    ...
    if (mStartSurfaceSupplier != null) {
      if (mStartSurfaceSupplier.get() != null) {
        mStartSurfaceSupplier.get().removeStateChangeObserver(
   ...
   */
  protected int getAppMenuLayoutId() { return R.menu.main_menu; }
  ...
  private boolean isInTabSwitcher() {
    return mOverviewModeBehavior != null &&
        mOverviewModeBehavior.overviewVisible() && !isInStartSurfaceHomepage();
  }
  ...
  private void setMenuGroupVisibility(@MenuGroup int menuGroup, Menu menu) {
    ...
    menu.setGroupVisible(R.id.OVERVIEW_MODE_MENU,
                         menuGroup == MenuGroup.OVERVIEW_MODE_MENU);
    menu.setGroupVisible(R.id.TABLET_EMPTY_MODE_MENU,
                         menuGroup == MenuGroup.TABLET_EMPTY_MODE_MENU);
  }

  @Override
  public ModelList
  getMenuItems(CustomItemViewTypeProvider customItemViewTypeProvider,
    ...
    // If we are not showing extensions first, we append the extensions to the
    // existing menu
    if (!ContextUtils.getAppSharedPreferences().getBoolean(
            "show_extensions_first", false))
      extensionMenu = menu;
    prepareExtensionMenu(
        extensionMenu, isInStartSurfaceHomepage() ? null : currentTab, handler,
        mTabModelSelector.getCurrentModel().isIncognito());

    // TODO(crbug.com/1119550): Programmatically create menu item's
    // PropertyModel instead of converting from MenuItems.
    for (int i = 0; i < menu.size(); ++i) {
      ...
      propertyModel.set(AppMenuItemProperties.ICON_COLOR_RES,
                        getMenuItemIconColorRes(item));
      propertyModel.set(AppMenuItemProperties.SUPPORT_ENTER_ANIMATION, true);
      if (item.hasSubMenu()) {
        // Only support top level menu items have SUBMENU, and a SUBMENU item
        // cannot have a SUBMENU.
        // TODO(crbug.com/1183234) : Create a new SubMenuItemProperties property
        // key set for SUBMENU items.
        ...
        for (int j = 0; j < item.getSubMenu().size(); ++j) {
          ...
          if (subitem.getItemId() == R.id.reload_menu_id) {
            ...
            loadingStateChanged(currentTab == null ? false
      ...
      if (item.getItemId() == R.id.request_desktop_site_row_menu_id ||
        ...
        int customType =
            customItemViewTypeProvider.fromMenuItemId(item.getItemId());
      ...
      // If we gave to show extensions first, we append them after the first row
      // that has 5 action buttons
      if (ContextUtils.getAppSharedPreferences().getBoolean(
              "show_extensions_first", false) &&
        ...
        for (int j = 0; j < extensionMenu.size(); ++j) {
          ...
          if (!extensionItem.isVisible())
            continue;
          PropertyModel extensionPropertyModel =
              AppMenuUtil.menuItemToPropertyModel(extensionItem);
          extensionPropertyModel.set(AppMenuItemProperties.ICON_COLOR_RES,
                                     getMenuItemIconColorRes(extensionItem));
          extensionPropertyModel.set(
              AppMenuItemProperties.SUPPORT_ENTER_ANIMATION, true);

          modelList.add(new MVCListAdapter.ListItem(AppMenuItemType.STANDARD,
                                                    extensionPropertyModel));
        }
      }
    }
  ...
  public void prepareMenu(Menu menu, AppMenuHandler handler) {
    ...
    if (menuGroup == MenuGroup.PAGE_MENU) {
      preparePageMenu(menu, isInStartSurfaceHomepage() ? null : currentTab,
                      handler, isIncognito);
    }
    prepareCommonMenuItems(menu, menuGroup, isIncognito);
  }

  private void prepareExtensionMenu(Menu menu, @Nullable Tab currentTab,
                                    AppMenuHandler handler,
                                    boolean isIncognito) {
    ...
    if (canShowExtensions) {
      ...
      String extensions = AppMenuBridge.getRunningExtensions(
      ...
      if (!extensions.isEmpty()) {
        ...
        for (String extension : extensionsArray) {
          ...
          MenuItem newlyAdded = menu.add(999999, 999999 + itemIndex, Menu.NONE,
                                         extensionsInfo[0]);
          ...
          if (extensionsInfo.length > 3) {
            String cleanImage = extensionsInfo[3]
   ...
   */
  private void preparePageMenu(Menu menu, @Nullable Tab currentTab,
                               AppMenuHandler handler, boolean isIncognito) {
    // Multiple menu items shouldn't be enabled when the currentTab is null. Use
    // a flag to indicate whether the current Tab isn't null.
    ...
    menu.findItem(R.id.move_to_other_window_menu_id)
        .setVisible(shouldShowMoveToOtherWindow());
    ...
    if (showManageAllWindows) {
      menu_all_windows.setTitle(mContext.getString(
          R.string.menu_manage_all_windows, getInstanceCount()));
    }
    ...
    // The order listed here is reflects the relative priority. Subsequent
    // updates should check the `mIsTypeSpecificBookmarkItemRowPresent` bit
    // before updating their row.
    ...
    // or when the current tab is null.
    ...
    if (ImageDescriptionsController.getInstance()
      ...
      // If image descriptions are not enabled, then we want the menu item to be
      // "Get".
      if (!ImageDescriptionsController.getInstance().imageDescriptionsEnabled(
              profile)) {
        titleId = R.string.menu_get_image_descriptions;
      } else if (ImageDescriptionsController.getInstance().onlyOnWifiEnabled(
                     profile) &&
                 DeviceConditions.getCurrentNetConnectionType(mContext) !=
                     ConnectionType.CONNECTION_WIFI) {
        // If image descriptions are enabled, then we want "Stop", except in the
        // special case that the user specified only on Wifi, and we are not
        // currently on Wifi.
        titleId = R.string.menu_get_image_descriptions;
      }
    ...
    prepareAddToHomescreenMenuItem(
        menu, currentTab,
        shouldShowHomeScreenMenuItem(isChromeScheme, isFileScheme,
                                     isContentScheme, isIncognito, url));
    ...
    boolean isProxyEnabled =
        AppMenuBridge.isProxyEnabled(Profile.getLastUsedRegularProfile());
    if (isProxyEnabled) {
    ...
    // Only display reader mode settings menu option if the current page is in
    // reader mode.
    ...
    menu.findItem(R.id.enter_vr_id)
        .setVisible(isCurrentTabNotNull && shouldShowEnterVr());
  ...
  /**
   * @return The number of Chrome instances either running alive or dormant but
   *     the state
  ...
  private void prepareCommonMenuItems(Menu menu, @MenuGroup int menuGroup,
                                      boolean isIncognito) {
    // We have to iterate all menu items since same menu item ID may be
    // associated with more than one menu items.
    ...
    boolean isMenuGroupTabsEnabled =
        isMenuGroupTabsVisible && mTabModelSelector.getTabModelFilterProvider()
                                          .getCurrentTabModelFilter()
                                          .getTabsWithNoOtherRelatedTabs()
                                          .size() > 1;
    boolean isPriceTrackingVisible =
        isOverviewModeMenu &&
    ...
    for (int i = 0; i < menu.size(); ++i) {
      ...
      if (!shouldShowIconBeforeItem()) {
        // Remove icons for menu items except the reader mode prefs and the
        // update menu item.
      ...
      if (item.getItemId() == R.id.new_incognito_tab_menu_id &&
        ...
        // findItem(...).setEnabled(...)" is not enough here, because of the
        // inflated main_menu.xml contains multiple items with the same id in
        // different groups e.g.: menu_new_incognito_tab.
      ...
      if (item.getItemId() == R.id.divider_line_id) {
        if (!hasItemBetweenDividers) {
          // If there isn't any visible menu items between the two divider
          // lines, mark this line invisible.
          item.setVisible(false);
        } else {
          hasItemBetweenDividers = false;
        }
      } else if (!hasItemBetweenDividers && item.isVisible()) {
        // When the item isn't a divider line and is visible, we set
        // hasItemBetweenDividers to be true.
        hasItemBetweenDividers = true;
      }
    }
  }

  protected void updateAdblockMenuItem(Menu menu, Tab currentTab,
                                       boolean canShowAdblockMenu) {
    ...
    boolean itemVisible = canShowAdblockMenu && !isChromeScheme &&
                          !currentTab.isNativePage() && !isDistilledPage;
  ...
  /**
   * @param currentTab The currentTab for which the app menu is showing.
   * @return Whether the {@code currentTab} may be downloaded, indicating
  ...
  /**
   * @param currentTab The currentTab for which the app menu is showing.
   * @return Whether bookmark page menu item should be checked, indicating that
   *     the current tab
   *         is bookmarked.
   */
  @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
  public boolean shouldCheckBookmarkStar(@NonNull Tab currentTab) {
    if (sItemBookmarkedForTesting != null)
      return sItemBookmarkedForTesting;

    if (!mBookmarkBridgeSupplier.hasValue())
      return false;
  ...
  /**
   * @param currentTab The currentTab for which the app menu is showing.
   * @return Whether reading list menu item should be highlighted, indicating
   *     that the current tab
   *         exists in the reading list.
   */
  @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
  public boolean shouldHighlightReadingList(@NonNull Tab currentTab) {
    if (sItemInReadingListForTesting != null)
      return sItemInReadingListForTesting;
  ...
  /**
   * @param currentTab The currentTab for which the app menu is showing.
   * @return Whether price tracking is enabled and the button should be
   *     highlighted.
   */
  @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
  public boolean shouldHighlightPriceTracking(@NonNull Tab currentTab) {
    // TODO(crbug.com/1266624): Read this information from power bookmarks when
    // available.
    return false;
  }
  ...
  protected boolean shouldShowMoveToOtherWindow() {
    ...
    if (!hasMoreThanOneTab && !showAlsoForSingleTab)
      return false;
  ...
  public boolean isPartnerHomepageEnabled() {
    return PartnerBrowserCustomizations.getInstance()
  ...
  protected boolean shouldShowNewWindow() {
    ...
    if (instanceSwitcherEnabled()) {
      ...
      // On phones, show the menu only when in split-screen, with a single
      // instance running on the foreground.
      return isTabletSizeScreen() ||
          (!mMultiWindowModeStateDispatcher.isChromeRunningInAdjacentWindow() &&
           (mMultiWindowModeStateDispatcher.isInMultiWindowMode() ||
            mMultiWindowModeStateDispatcher.isInMultiDisplayMode()));
    } else {
      if (mMultiWindowModeStateDispatcher.isMultiInstanceRunning())
        return false;
  ...
  /**
   * @param isChromeScheme Whether URL for the current tab starts with the
   *     chrome:// scheme.
   * @param currentTab The currentTab for which the app menu is showing.
   ...
   * @param isChromeScheme Whether URL for the current tab starts with the
   *     chrome:// scheme.
   * @param isFileScheme Whether URL for the current tab starts with the file://
   *     scheme.
   * @param isContentScheme Whether URL for the current tab starts with the
   *     file:// scheme.
   ...
   */
  protected boolean shouldShowHomeScreenMenuItem(boolean isChromeScheme,
    ...
    // * incognito pages - To avoid problems where users create shortcuts in
    // incognito
    //                      mode and then open the webapp in regular mode.
    // * file:// - After API 24, file: URIs are not supported in VIEW intents
    // and thus
    //             can not be added to the homescreen.
    // * content:// - Accessing external content URIs requires the calling app
    // to grant
    //                access to the resource via FLAG_GRANT_READ_URI_PERMISSION,
    //                and that is not persisted when adding to the homescreen.
    // * If creating shortcuts it not supported by the current home screen.
    return WebappsUtils.isAddToHomeIntentSupported() && !isChromeScheme &&
        !isFileScheme && !isContentScheme && !isIncognito && !url.isEmpty();
  }

  /**
   * @param currentTab Current tab being displayed.
   * @return Whether the "Managed by your organization" menu item should be
   *     displayed.
   */
  ...
  /**
   * Sets the visibility and labels of the "Add to Home screen" and "Open
   * WebAPK" menu items.
   */
  protected void
  prepareAddToHomescreenMenuItem(Menu menu, Tab currentTab,
    ...
    if (currentTab != null && shouldShowHomeScreenMenuItem) {
      ...
      if (openWebApkItemVisible) {
        ...
        openWebApkItem.setTitle(
            context.getString(R.string.menu_open_webapk, appName));
        ...
        if (installStrings.titleTextId ==
            AppBannerManager.NON_PWA_PAIR.titleTextId) {
          mAddAppTitleShown = AppMenuVerbiage.APP_MENU_OPTION_ADD_TO_HOMESCREEN;
        } else if (installStrings.titleTextId ==
                   AppBannerManager.PWA_PAIR.titleTextId) {
  ...
  @VisibleForTesting(otherwise = VisibleForTesting.PROTECTED)
  public AppBannerManager.InstallStringPair
  getAddToHomeScreenTitle(@NonNull Tab currentTab) {
  ...
  protected void prepareTranslateMenuItem(Menu menu, @Nullable Tab currentTab) {
    boolean isTranslateVisible =
        currentTab != null && shouldShowTranslateMenuItem(currentTab);
    ...
    if (translate_menu != null) {
      try {
        ...
        if (url != null && (url.startsWith("https://translate.yandex.com/") ||
  ...
  public void loadingStateChanged(boolean isLoading) {
    if (mReloadPropertyModel != null) {
      ...
      mReloadPropertyModel.set(
          ...
          resources.getString(isLoading
                                  ? R.string.accessibility_btn_stop_loading
                                  : R.string.accessibility_btn_refresh));
      mReloadPropertyModel.set(
          AppMenuItemProperties.TITLE_CONDENSED,
          resources.getString(isLoading ? R.string.menu_stop_refresh
  ...
  @Override
  public void
  recordHighlightedMenuItemShown(@Nullable @IdRes Integer menuItemId) {
    RecordHistogram.recordEnumeratedHistogram(
        "Mobile.AppMenu.HighlightMenuItem.Shown",
        getUmaEnumForMenuItem(menuItemId), AppMenuHighlightItem.NUM_ENTRIES);
  }

  @Override
  public void
  recordHighlightedMenuItemClicked(@Nullable @IdRes Integer menuItemId) {
    RecordHistogram.recordEnumeratedHistogram(
        "Mobile.AppMenu.HighlightMenuItem.Clicked",
        getUmaEnumForMenuItem(menuItemId), AppMenuHighlightItem.NUM_ENTRIES);
  }
   ...
   *
   * @param bookmarkMenuItemShortcut {@link MenuItem} for adding/editing the
   *     bookmark.
   * @param currentTab Current tab being displayed.
   */
  protected void
  updateBookmarkMenuItemShortcut(MenuItem bookmarkMenuItemShortcut,
    ...
    if (!mBookmarkBridgeSupplier.hasValue() || currentTab == null) {
      // If the BookmarkBridge still isn't available, assume the bookmark menu
   ...
   * @param currentTab Current tab being displayed.
   */
  protected void updateBookmarkMenuItemRow(MenuItem bookmarkMenuItemAdd,
                                           MenuItem bookmarkMenuItemEdit,
                                           @Nullable Tab currentTab) {
    ...
    boolean shouldCheckBookmarkStar =
        currentTab != null && shouldCheckBookmarkStar(currentTab);
   ...
   * Updates the bookmark item's visibility.
   *
   * @param readingListMenuItemAdd {@link MenuItem} for adding to the reading
   *     list.
   * @param readingListMenuItemDelete {@link MenuItem} for deleting from the
   *     reading list.
   * @param currentTab Current tab being displayed.
   */
  protected void
  updateReadingListMenuItemRow(@NonNull MenuItem readingListMenuItemAdd,
                               @NonNull MenuItem readingListMenuItemDelete,
                               @Nullable Tab currentTab) {
   ...
   */
  protected void
  updatePriceTrackingMenuItemRow(@NonNull MenuItem startPriceTrackingMenuItem,
                                 @NonNull MenuItem stopPriceTrackingMenuItem,
                                 @Nullable Tab currentTab) {
    // If price tracking isn't enabled or the page isn't eligible, then hide
    // both items.
   ...
   * @param currentTab Current tab being displayed.
   * @param canShowRequestDesktopSite If the request desktop site menu item
   *     should show or not.
   * @param isChromeScheme Whether URL for the current tab starts with the
   *     chrome:// scheme.
   */
  protected void
  updateRequestDesktopSiteMenuItem(Menu menu, @Nullable Tab currentTab,
                                   boolean canShowRequestDesktopSite,
                                   boolean isChromeScheme) {
    MenuItem requestMenuRow =
        menu.findItem(R.id.request_desktop_site_row_menu_id);
    MenuItem requestMenuLabel = menu.findItem(R.id.request_desktop_site_id);
    MenuItem requestMenuCheck =
        menu.findItem(R.id.request_desktop_site_check_id);
    ...
    boolean isRequestDesktopSite = currentTab.getWebContents()
    ...
    if (CachedFeatureFlags.isEnabled(
      ...
      requestMenuLabel.setIcon(isRequestDesktopSite
      ...
      // This title doesn't seem to be displayed by Android, but it is used to
      // set up accessibility text in {@link AppMenuAdapter#setupMenuButton}.
   ...
   *     chrome:// scheme.
   */
  protected void updateAutoDarkMenuItem(Menu menu, @Nullable Tab currentTab,
                                        boolean isChromeScheme) {
    MenuItem autoDarkMenuRow =
        menu.findItem(R.id.auto_dark_web_contents_row_menu_id);
    MenuItem autoDarkMenuCheck =
        menu.findItem(R.id.auto_dark_web_contents_check_id);
    ...
    boolean itemVisible =
        currentTab != null && !isChromeScheme && isAutoDarkEnabled;
    itemVisible = false;
  ...
  protected void updateManagedByMenuItem(Menu menu, @Nullable Tab currentTab) {
    ...
    managedByMenuItem.setEnabled(
        managedByMenuItemVisible &&
