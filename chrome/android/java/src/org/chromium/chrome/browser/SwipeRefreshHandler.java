...
public class SwipeRefreshHandler
  ...
  private static final Class<SwipeRefreshHandler> USER_DATA_KEY =
      SwipeRefreshHandler.class;
  ...
  // Handles overscroll history navigation. Gesture events from native layer are
  // forwarded to this object. Remains null while navigation feature is disabled
  // due to feature flag, system settings (Q and forward), etc.
  ...
  public static SwipeRefreshHandler from(Tab tab) {
    ...
    if (handler == null) {
      handler = tab.getUserDataHost().setUserData(USER_DATA_KEY,
                                                  new SwipeRefreshHandler(tab));
    }
    return handler;
  }
  ...
  /**
   * Simple constructor to use when creating an OverscrollRefresh instance from
   * code.
   *
  ...
  private SwipeRefreshHandler(Tab tab) {
    ...
    mTabObserver = new EmptyTabObserver() {
      @Override
      public void onActivityAttachmentChanged(Tab tab,
  ...
  public void didStopRefreshing() {
    if (mSwipeRefreshLayout == null || !mSwipeRefreshLayout.isRefreshing())
      return;
    cancelStopRefreshingRunnable();
    mSwipeRefreshLayout.postDelayed(getStopRefreshingRunnable(),
                                    STOP_REFRESH_ANIMATION_DELAY_MS);
  }

  @Override
  public boolean start(@OverscrollAction int type, float startX, float startY,
                       boolean navigateForward) {
    mSwipeType = type;
    if (type == OverscrollAction.PULL_TO_REFRESH) {
      if (mSwipeRefreshLayout == null)
        initSwipeRefreshLayout(mTab.getContext());
   ...
   */
  public void
  setNavigationCoordinator(HistoryNavigationCoordinator navigationHandler) {
  ...
  public void pull(float xDelta, float yDelta) {
    ...
    if (mSwipeType == OverscrollAction.PULL_TO_REFRESH) {
      ...
      if (!ContextUtils.getAppSharedPreferences().getBoolean(
        ...
        return;
      if (mNavigationCoordinator != null)
        mNavigationCoordinator.pull(xDelta);
    }
  ...
  public void release(boolean allowRefresh) {
    ...
    if (mSwipeType == OverscrollAction.PULL_TO_REFRESH) {
      ...
      if (!ContextUtils.getAppSharedPreferences().getBoolean(
        ...
        return;
      if (mNavigationCoordinator != null)
        mNavigationCoordinator.release(allowRefresh);
    }
  ...
  private void cancelDetachLayoutRunnable() {
    if (mDetachRefreshLayoutRunnable != null) {
      ThreadUtils.getUiThreadHandler().removeCallbacks(
          mDetachRefreshLayoutRunnable);
      mDetachRefreshLayoutRunnable = null;
    }
  }
