...
public class HomepageManager
  ...
  public interface HomepageStateListener {
    /**
     * Called when the homepage is enabled or disabled or the homepage URL
     * changes.
     */
  ...
  /**
   * Adds a HomepageStateListener to receive updates when the homepage state
   * changes.
   */
  ...
  public static boolean shouldCloseAppWithZeroTabs() {
    return ContextUtils.getAppSharedPreferences().getBoolean(
        "close_browser_after_last_tab", false);
  }

  /**
   * Get the current homepage URI string, if it's enabled. Null otherwise or
   * uninitialized.
   *
   * This function checks different source to get the current homepage, which
   * listed below according to their priority:
   *
   * <b>isManagedByPolicy > useChromeNTP > useDefaultUri > useCustomUri</b>
   *
   * @return Homepage URI string, if it's enabled. Null otherwise or
   *     uninitialized.
   *
  ...
  /**
   * @return The default homepage URI if the homepage is partner provided or the
   *     new tab page
  ...
  public static String getDefaultHomepageUri() {
    ...
    if (!homepagePartnerDefaultUri.equals(""))
      return homepagePartnerDefaultUri;

    return "chrome-search://local-ntp/local-ntp.html";
  }

  /**
   * Determines whether the homepage is set to something other than the NTP or
   * empty/null. Normally, when loading the homepage the NTP is loaded as a
   * fallback if the homepage is null or empty. So while other helper methods
   * that check if a given string is the NTP will reject null and empty, this
  ...
  /**
   * Determines whether the homepage is set to something other than the NTP or
   * empty/null. This is the same as {@link #isHomepageNonNtp()}, but uses
   * {@link UrlUtilities#isCanonicalizedNTPUrl} instead of {@link
   * UrlUtilities#isNTPUrl} to make it possible to use before native is loaded.
  ...
  /**
   * Returns the user preference for whether the homepage is enabled. This
   * doesn't take into account whether the device supports having a homepage.
   *
  ...
  public String getPrefHomepageCustomUri() {
    return mSharedPreferencesManager.readString(
        ChromePreferenceKeys.HOMEPAGE_CUSTOM_URI, "");
  }

  /**
   * True if the homepage URL is the default value. False means the homepage URL
   * is using the user customized URL. Note that this method does not take
   * enterprise policy into account. Use {@link
   * HomepagePolicyManager#isHomepageManagedByPolicy} if policy information is
   * needed.
   *
  ...
  /**
   * Set homepage related shared preferences, and notify listeners for the
   * homepage status change. These shared preference values will reflect what
   * homepage we are using.
   *
   ...
   * @see #getHomepageUri()
   */
  public void setHomepagePreferences(boolean useChromeNtp,
                                     boolean useDefaultUri, String customUri) {
  ...
  /**
   * Record histogram "Settings.Homepage.LocationType" with the current homepage
   * location type.
   */
  ...
  public @HomepageLocationType int getHomepageLocationType() {
    ...
    // If user type NTP URI as their customized homepage, we'll record user is
    // using NTP
