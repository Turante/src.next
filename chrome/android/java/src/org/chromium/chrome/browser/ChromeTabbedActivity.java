...
/**
 * This is the main activity for ChromeMobile when not running in document mode.
 * All the tabs are accessible via a chrome specific tab switching UI.
 */
public class ChromeTabbedActivity
    extends ChromeActivity<ChromeActivityComponent>
    implements ChromeAccessibilityUtil.Observer {
  private static final String TAG = "ChromeTabbedActivity";

  private static final String HELP_URL_PREFIX =
  ...
  private static final int INVALID_WINDOW_ID =
      TabWindowManager.INVALID_WINDOW_INDEX;

  // How long to delay closing the current tab when our app is minimized.  Have
  // to delay this so that we don't show the contents of the next tab while
  // minimizing.
  private static final long CLOSE_TAB_ON_MINIMIZE_DELAY_MS = 500;

  // Maximum delay for initial tab creation. This is for homepage and NTP, not
  // previous tabs restore. This is needed because we do not know when reading
  // PartnerBrowserCustomizations provider will be finished.
  private static final int INITIAL_TAB_CREATION_TIMEOUT_MS = 1;

  /**
   * Sending an intent with this action to Chrome will cause it to close all
   * tabs (iff the --enable-test-intents command line flag is set). If a URL is
   * supplied in the intent data, this will be loaded and unaffected by the
   * close all action.
   */
  ...
  public static final String MAIN_LAUNCHER_ACTIVITY_NAME =
      "com.google.android.apps.chrome.Main";

  public static final SettingsLauncher SETTINGS_LAUNCHER =
      new SettingsLauncherImpl();

  public static final HashSet<String> TABBED_MODE_COMPONENT_NAMES =
      new HashSet<String>() {
        {
  ...
  /**
   * Identifies a histogram to use in {@link
   * #maybeDispatchExplicitMainViewIntent(Intent, int)}.
   */
  ...
  // Count histogram used to track number of tabs when we show the Overview on
  // Return to Chrome.
  private static final String TAB_COUNT_ON_RETURN =
      "Tabs.TabCountOnStartScreenShown";
  ...
  private BrowserControlsVisibilityDelegate
  ...
  /**
   * Keeps track of whether or not a specific tab was created based on the
   * startup intent.
   */
  ...
  // Whether {@link setInitialOverviewState()} has been called within the
  // current onStart/onStop session.
  ...
  // This is the cached value of mIntentHandler#shouldIgnoreIntent and shouldn't
  // be read directly. Use #shouldIgnoreIntent instead.
  ...
  // Supplier for a dependency to inform about the type of intent used to launch
  // Chrome.
  ...
  // TODO(crbug.com/1108496): Removed after all usages has been migrated to
  // LayoutStateProvider.
  private final OneshotSupplierImpl<OverviewModeBehavior>
      mOverviewModeBehaviorSupplier = new OneshotSupplierImpl<>();
  private OverviewModeController mOverviewModeController;

  private ObservableSupplierImpl<EphemeralTabCoordinator>
      mEphemeralTabCoordinatorSupplier = new ObservableSupplierImpl<>();
  ...
  // ID assigned to each ChromeTabbedActivity instance in Android S+ where
  // multi-instance feature is supported. This can be explicitly set in the
  // incoming Intent or internally assigned.
  ...
  private final IncognitoTabHost mIncognitoTabHost = new IncognitoTabHost() {
    ...
    public void closeAllIncognitoTabs() {
      ...
      // If the tabbed activity has not yet initialized, then finish the
      // activity to avoid timing issues with clearing the incognito tab state
      // in the background.
  ...
  /**
   * This class is used to warm up the chrome split ClassLoader. See
  ...
  public static class Preload extends ChromeTabbedActivity {
    ...
    public Lifecycle getLifecycle() {
      if (mLifecycleRegistry == null) {
        // LifecycleRegistry normally enforces it is called on the main thread,
        // but this class will be preloaded in a background thread. The only
        // method that gets called in the activity constructor is addObserver(),
        // so just override that.
  ...
  /**
   * Specify the proper non-.Main-aliased Chrome Activity for the given
   * component.
   *
   * @param intent The intent to set the component for.
   * @param component The client generated component to be validated.
   */
  public static void setNonAliasedComponent(Intent intent,
                                            ComponentName component) {
    assert component != null;
    Context appContext = ContextUtils.getApplicationContext();
    if (!TextUtils.equals(component.getPackageName(),
  ...
  public ChromeTabbedActivity() {
    ...
    // AppLaunchDrawBlocker may block drawing the Activity content until the
    // initial tab is available.
    // clang-format off
  ...
  protected void onPreCreate() {
    super.onPreCreate();
    mMultiInstanceManager = MultiInstanceManager.create(
        this, getTabModelOrchestratorSupplier(),
  ...
  int maybeDispatchLaunchIntent(Intent intent, Bundle savedInstanceState) {
    // Detect if incoming intent is a result of Chrome recreating itself. For
    // now, restrict this path to reparenting to ensure the launching logic
    // isn't disrupted.
    // TODO(crbug.com/1065491): Unlock this codepath for all incoming intents
    // once it's confirmed working and stable.
    ...
    if (getClass().equals(ChromeTabbedActivity.class) &&
      ...
      // Call dispatchToTabbedActivity() for MAIN intents to activate proper
      // multi-window TabbedActivity (i.e. if CTA2 is currently running and
      // Chrome is started, CTA2 should be brought to front). Don't call
      // dispatchToTabbedActivity() for non-MAIN intents to avoid breaking cases
      // where CTA is started explicitly (e.g. to handle 'Move to other window'
      // command from CTA2).
      return LaunchIntentDispatcher.dispatchToTabbedActivity(this, intent);
    }
    @LaunchIntentDispatcher.Action
    int action =
        maybeDispatchExplicitMainViewIntent(intent, DispatchedBy.ON_CREATE);
  ...
  // We know of at least one app that explicitly specifies .Main activity in
  // custom tab intents. The app shouldn't be doing that, but until it's
  // updated, we need to support such use case.
  //
  // This method attempts to treat VIEW intents explicitly sent to .Main as
  // custom tab intents, and dispatch them accordingly. If the intent was not
  // dispatched, the method returns Action.CONTINUE.
  //
  // The method also updates the supplied boolean histogram with the dispatching
  // result, but only if the intent is a VIEW intent sent explicitly to .Main
  // activity.
  private @LaunchIntentDispatcher.Action
  int maybeDispatchExplicitMainViewIntent(Intent intent,
                                          @DispatchedBy int dispatchedBy) {
    // The first check ensures that this is .Main activity alias (we can't check
    // exactly, but this gets us sufficiently close).
    if (getClass().equals(ChromeTabbedActivity.class) &&
        Intent.ACTION_VIEW.equals(intent.getAction()) &&
        intent.getComponent() != null &&
        MAIN_LAUNCHER_ACTIVITY_NAME.equals(
      ...
      if (action == LaunchIntentDispatcher.Action.CONTINUE) {
        ...
        // Crash if intent came from us, but only in debug builds and only if we
        // weren't explicitly told not to. Hopefully we'll get enough reports to
        // find where these intents come from.
        if (externalId == IntentHandler.ExternalAppId.CHROME &&
          ...
          if (extras != null) {
            intentInfo += ", extras.keySet = [" +
                          TextUtils.join(", ", extras.keySet()) + "]";
          }
    ...
    try {
      ...
      mTabModelObserver = new TabModelSelectorTabModelObserver(
        ...
        private void closeIfNoTabsAndHomepageEnabled(boolean isPendingClosure) {
          ...
          // TODO(crbug.com/1046541) : Remove this when the associated bug is
          // fixed. This
          //  is a band-aid fix for TabGrid and closing tabs with TabGroupUi.
          //  If one of the following is true, then exit Chrome when
          //  TabGroupsAndroid is enabled, and tab switcher is not shown:
          ...
          // TODO(crbug.com/1272821): Investigate unexpected behavior when the
          // last tab is
          //  closed through grid tab switcher without this logic.
          //  If one of the following is true, then hide the grid tab switcher
          //  for tablets when TabGroupsAndroid is enabled, and tab switcher is
          //  shown:
          //   1. If the very last tab is closed.
          //   2. If normal tab model is selected and no normal tabs.
          if (gridTabSwitcherEnabled && overviewVisible && !hasNextTab &&
        ...
        public void didAddTab(Tab tab, @TabLaunchType int type,
          ...
          if (type == TabLaunchType.FROM_LONGPRESS_BACKGROUND &&
            ...
            Toast
                .makeText(ChromeTabbedActivity.this,
                          R.string.open_in_new_tab_toast, Toast.LENGTH_SHORT)
                .show();
          }
        }

        @Override
        public void multipleTabsPendingClosure(List<Tab> tabs,
                                               boolean isAllTabs) {
          if (!isAllTabs)
            return;
          NewTabPageUma.recordNTPImpression(
  ...
  private void refreshSignIn() {
    try (TraceEvent e =
             TraceEvent.scoped("ChromeTabbedActivity.refreshSignIn")) {
  ...
  private void setupCompositorContentPreNativeForPhone() {
    ...
    try (TraceEvent e = TraceEvent.scoped(
      ...
      CompositorViewHolder compositorViewHolder =
          getCompositorViewHolderSupplier().get();

      // TODO(1239025): Remove all GTS enabled checks after GTS is enabled by
      // default on tablets.
  ...
  private void createStartSurface(CompositorViewHolder compositorViewHolder) {
    ...
    if (tabManagementDelegate != null) {
      tabManagementDelegate.createStartSurface(
  ...
  private boolean isLayoutManagerCreated() { return mLayoutManager != null; }
    ...
    try (TraceEvent e = TraceEvent.scoped(
      ...
      OnClickListener tabSwitcherClickHandler = v -> {
        if (ChromeFeatureList.isEnabled(
                ChromeFeatureList.TOOLBAR_IPH_ANDROID)) {
      ...
      OnClickListener newTabClickHandler = v -> {
        ...
        // This assumes that the keyboard can not be seen at the same time as
        // the newtab button on the toolbar.
        if (getTabModelSelector().isIncognitoSelected()) {
          getCurrentTabCreator().launchIncognitoNTP();
        } else {
          getCurrentTabCreator().launchUrl(
      ...
      OnClickListener bookmarkClickHandler =
          v -> addOrEditBookmark(getActivityTab());

      Supplier<Boolean> showStartSurfaceSupplier = () -> {
        // If incognito is selected, tapping the home button stays on the
        // current incognito tab.
        if (ReturnToChromeExperimentsUtil
                .shouldShowStartSurfaceAsTheHomePageOnPhone(this, isTablet()) &&
  ...
  private void maybeCreateIncognitoTabSnapshotController() {
    try (
        TraceEvent e = TraceEvent.scoped(
      ...
      // Check for incognito tabs to handle the case where Chrome was swiped
  ...
  private void initJourneyManager() {
    ...
    try (TraceEvent e =
             TraceEvent.scoped("ChromeTabbedActivity.initJourneyManager")) {
      if (ChromeFeatureList.isEnabled(
              ChromeFeatureList.TAB_ENGAGEMENT_REPORTING_ANDROID)) {
  ...
  public void onNewIntent(Intent intent) {
    // The intent to use in maybeDispatchExplicitMainViewIntent(). We're
    // explicitly adding NEW_TASK flag to make sure backing from CCT brings up
    // the caller activity, and not Chrome
  ...
  public void startNativeInitialization() {
    try (TraceEvent e = TraceEvent.scoped(
             "ChromeTabbedActivity.startNativeInitialization")) {
      // This is on the critical path so don't delay.
      setupCompositorContentPostNative();

      // All this initialization can be expensive so it's split into multiple
      // tasks.
      PostTask.postTask(
  ...
  public void onResumeWithNative() {
    ...
    IncognitoStartup.onResumeWithNative(getTabModelSelectorSupplier(),
  ...
  public void onStartWithNative() {
    ...
    // Don't call setInitialOverviewState if we're waiting for the tab's
    // creation or we risk showing a glimpse of the tab selector during start
    // up.
    ...
    if (savedInstanceState != null &&
      ...
      // This will be executed only once since SavedInstanceState will be reset
      // a few lines later.
  ...
  private void
  setTrackColdStartupMetrics(boolean shouldTrackColdStartupMetrics) {
    assert getActivityTabStartupMetricsTracker() != null;

    if (shouldTrackColdStartupMetrics) {
      getActivityTabStartupMetricsTracker().trackStartupMetrics(
          STARTUP_UMA_HISTOGRAM_SUFFIX);
    } else {
      getActivityTabStartupMetricsTracker().cancelTrackingStartupMetrics();
    }

    // Paint Preview should follow the same logic as startup UMA histograms as
    // the feature should only run on cold startup of Chrome when the user is
    // unable to interact before entering a tab.
    StartupPaintPreviewHelper.setShouldShowOnRestore(
        shouldTrackColdStartupMetrics);
  }

  private void setInitialOverviewState() {
    if (mHasDeterminedOverviewStateForCurrentSession)
      return;
    ...
    if (shouldRefreshAndShowOverview(isOverviewVisible)) {
      if (getCurrentTabModel() != null) {
        RecordHistogram.recordCountHistogram(TAB_COUNT_ON_RETURN,
      ...
      // Cancel recording cold startup metrics if an overview is shown as they
      // expect a tab to be the first thing shown after startup.
  ...
  private boolean shouldRefreshAndShowOverview(boolean isOverviewVisible) {
    // If StartSurfaceConfiguration.NEW_SURFACE_FROM_HOME_BUTTON is turned on,
    // MV tiles and carousels may be hidden before Chrome is brought to the
    // background. If overview should be shown, no matter overview was already
    // visible or not, we should call
    // showOverview(StartSurfaceState.SHOWING_START) to show MV tiles and
    // carousels again.
  ...
  private void logMainIntentBehavior(Intent intent) {
    ...
    // TODO(tedchoc): We should cache the last visible time and reuse it to
    // avoid different
    //                values of this depending on when it is called after the
    //                activity was shown.

    // Temporary safety check to make sure none of this code runs if the feature
    // is disabled.
    if (ReengagementNotificationController.isEnabled()) {
      if (mCallbackController != null) {
        new OneShotCallback<>(
            mTabModelProfileSupplier,
            mCallbackController.makeCancelable(profile -> {
              assert profile != null
  ...
  public void initializeState() {
    ...
    // 2. Process the Intent that this activity received and if that should
    // result in any
    //    new tabs, create them.  This is done after step 1 so that the new tab
    //    gets created after previous tab state was restored.
    // 3. If no tabs were created in any of the above steps, create an NTP,
    // otherwise
    //    start asynchronous tab restore (loading the previously active tab
    //    synchronously if no new tabs created in step 2).

    // Only look at the original intent if this is not a "restoration" and we
    // are allowed to process intents. Any subsequent intents are carried
    // through onNewIntent.
    try {
      ...
      boolean hadCipherData = CipherFactory.getInstance().restoreFromBundle(
          getSavedInstanceState());

      boolean noRestoreState =
          CommandLine.getInstance().hasSwitch(
              ChromeSwitches.NO_RESTORE_STATE) ||
          ContextUtils.getAppSharedPreferences().getBoolean(
              "close_tabs_on_exit", false);
      if (noRestoreState) {
        // Clear the state files because they are inconsistent and useless from
        // now on.
        mTabModelOrchestrator.clearState();
      } else {
        // State should be clear when we start first run and hence we do not
        // need to load a previous state. This may change the current Model,
        // watch out for initialization based on the model. Never attempt to
        // restore incognito tabs when this activity was previously swiped away
        // in Recents. http://crbug.com/626629
      ...
      mIntentMetadataOneshotSupplier.set(new ToolbarIntentMetadata(
          isMainIntentFromLauncher, isIntentWithEffect));

      // If we have tabs to reparent and getSavedInstanceState() is non-null,
      // then the tabs are coming from night mode tab reparenting. In this case,
      // reparenting happens synchronously along with tab restoration so there
      // are no tabs waiting for reparenting like there are for other tab
      ...
      // We always need to try to restore tabs. The set of tabs might be empty,
      // but at least it will trigger the notification that tab restore is
      // complete which is needed by other parts of Chrome such as sync.
      boolean activeTabBeingRestored =
          !isIntentWithEffect || (shouldShowOverviewPageOnStart() &&
                                  !mTabModelSelector.isIncognitoSelected());

      mTabModelOrchestrator.restoreTabs(activeTabBeingRestored);

      // Only create an initial tab if no tabs were restored and no intent was
      // handled. Also, check whether the active tab was supposed to be restored
      // and that the total tab count is now non zero.  If this is not the case,
      // tab restore failed and we need to create a new tab as well.
      if (!mCreatedTabOnStartup ||
          (!hasTabWaitingForReparenting && activeTabBeingRestored &&
           getTabModelSelector().getTotalTabCount() == 0)) {
        // If homepage URI is not determined, due to
        // PartnerBrowserCustomizations provider async reading, then create a
        // tab at the async reading finished. If it takes too long, just create
        // NTP.
      ...
      if (!mPendingInitialTabCreation) {
        mAppLaunchDrawBlocker.onActiveTabAvailable(
            isTabRegularNtp(getActivityTab()));
      }
    } finally {
  ...
  private boolean hasStartWithNativeBeenCalled() {
    ...
    return activity_state ==
        ActivityLifecycleDispatcher.ActivityState.STARTED_WITH_NATIVE ||
        activity_state ==
  ...
  private void createInitialTab() {
    ...
    // If the start surface or grid tab switcher will be shown on start, do not
    // create a new tab.
    ...
    // If we didn't call setInitialOverviewState() in onStartWithNative()
    // because mPendingInitialTabCreation was true then do so now.
    ...
    if (mOverviewModeController != null &&
        ...
        (mIsAccessibilityTabSwitcherEnabled == null ||
         mIsAccessibilityTabSwitcherEnabled !=
             DeviceClassManager.enableAccessibilityLayout(this))) {
      /**
       * If Start surface homepage is showing and launching NTP will show the
       * Start surface again, skips the calls of hideOverview() and launchNTP().
       * We need to check
       * {@link
       * ReturnToChromeExperimentsUtil#shouldShowStartSurfaceHomeAsNTP(Context,
       * boolean, boolean)} to see whether Start surface can be shown when
  ...
  /**
   * Internal class which performs the intent handling operations delegated by
   * IntentHandler.
   */
  private class InternalIntentDelegate
    ...
    public void processUrlViewIntent(LoadUrlParams loadUrlParams,
      ...
      case TabOpenType.REUSE_URL_MATCHING_TAB_ELSE_NEW_TAB:
        ...
        int tabToBeClobberedIndex =
            TabModelUtils.getTabIndexByUrl(tabModel, url);
        ...
        int shortcutSource = intent.getIntExtra(WebappConstants.EXTRA_SOURCE,
      ...
      case TabOpenType.BRING_TAB_TO_FRONT:
        ...
        int tabIndex =
            TabModelUtils.getTabIndexById(tabModel, tabIdToBringToFront);
        if (tabIndex == TabModel.INVALID_TAB_INDEX) {
          TabModel otherModel =
              getTabModelSelector().getModel(!tabModel.isIncognito());
          tabIndex =
              TabModelUtils.getTabIndexById(otherModel, tabIdToBringToFront);
          if (tabIndex != TabModel.INVALID_TAB_INDEX) {
            getTabModelSelector().selectModel(otherModel.isIncognito());
            TabModelUtils.setIndex(otherModel, tabIndex);
          } else {
            Log.e(TAG,
                  "Failed to bring tab to front because it doesn't exist.");
            return;
          }
        } else {
      ...
      case TabOpenType.CLOBBER_CURRENT_TAB:
        // The browser triggered the intent. This happens when clicking links
        // which can be handled by other applications (e.g. www.youtube.com
        // links).
      ...
      case TabOpenType.REUSE_TAB_MATCHING_ID_ELSE_NEW_TAB:
        int tabId = IntentUtils.safeGetIntExtra(
            intent, TabOpenType.REUSE_TAB_MATCHING_ID_STRING,
            Tab.INVALID_TAB_ID);
      ...
      case TabOpenType.OPEN_NEW_INCOGNITO_TAB:
        ...
        if (!IncognitoUtils.isIncognitoModeEnabled()) {
          // The incognito launcher shortcut is manipulated in
          // #onDeferredStartup(), so it's possible for a user to invoke the
          // shortcut before it's disabled. Quick actions search widget is
          // installed on the home screen and may need to be updated before the
          // incognito button is removed. Opening an incognito tab while
          // incognito mode is disabled from somewhere besides the launcher
          // shortcut of from quick action search widget is an error.
          if (fromAppWidget || fromLauncherShortcut) {
            // We are using the message introduced for quick action search
            // widget for both the widget and the launcher shortcut here.
            Toast
                .makeText(
                    ChromeTabbedActivity.this,
            ...
            assert false
                : "Tried to open incognito tab while incognito disabled";
        ...
        if (url == null || url.equals(UrlConstants.NTP_URL) ||
            url.equals("chrome-search://local-ntp/local-ntp.html")) {
          if (fromLauncherShortcut) {
            ...
            Tab tab = getTabCreator(true).launchUrl(
                "chrome-search://local-ntp/incognito-ntp.html",
            ...
            if (IncognitoTabLauncher.shouldFocusOmnibox(intent)) {
              ...
              // focus, and since the Tab and the URL bar are not yet in the
              // same View hierarchy, setting the URL bar's focus here won't
              // clear the Tab's focus. When the Tab is added to the hierarchy,
              // we want the URL bar to retain focus, so we clear the Tab's
              // focus here.
            ...
            // Used by the Account management screen to open a new incognito
            // tab. Account management screen collects its metrics separately.
            getTabCreator(true).launchUrl(
                "chrome-search://local-ntp/local-ntp.html",
          ...
          getTabCreator(true).launchUrl(url, launchType, intent,
      ...
      getToolbarManager().setUrlBarFocusOnceNativeInitialized(
          focus, focus ? OmniboxFocusReason.LAUNCH_NEW_INCOGNITO_TAB
    ...
    public void processTranslateTabIntent(@Nullable String targetLanguageCode,
      ...
      TranslateIntentHandler.translateTab(getActivityTab(), targetLanguageCode,
                                          expectedUrl);
    }

    private boolean isFromChrome(Intent intent, String externalAppId) {
      // To determine if the processed intent is from Chrome, check for any of
      // the following: 1.) The authentication token that will be added to
      // trusted intents. 2.) The app ID matches Chrome.  This value can be
      // spoofed by other applications, but
      //     in cases where we were not able to add the authentication token
      //     this is our only indication the intent was from Chrome.
  ...
  public void performPreInflationStartup() {
    // Create JankTracker before invoking super, because the parent class will
    // invoke createRootUiCoordinator(...), which in our case requires the
    // JankTracker to already have been created.
    mJankTracker = new JankTrackerImpl(this);

    super.performPreInflationStartup();

    // Decide whether to record startup UMA histograms. This is done  early in
    // the main Activity.onCreate() to avoid recording navigation delays when
    // they require user input to proceed. For example, FRE (First Run
    // Experience) happens before the activity is created, and triggers
    // initialization of the native library.
    //
    // An uninitialized native library is an indication of an application start
    // that is followed by navigation immediately without user input.
    ...
    mStartupPaintPreviewHelperSupplier.attach(
  ...
  protected RootUiCoordinator createRootUiCoordinator() {
    return new TabbedRootUiCoordinator(
        this, this::onOmniboxFocusChanged, getShareDelegateSupplier(),
        ...
        mStartSurfaceParentTabSupplier, getBrowserControlsManager(),
        getWindowAndroid(), mJankTracker, getLifecycleDispatcher(),
        ...
        getOverviewModeBehaviorSupplier(), this::getSnackbarManager,
        getActivityType(), this::isInOverviewMode,
        ...
        // TODO(sinansahin): This currently only checks for incognito extras in
        // the intent. We should make it more robust by using more signals.
  ...
  public void performPostInflationStartup() {
    ...
    IncognitoNotificationPresenceController.observeTabModelSelector(
        tabModelSelector);
    ...
    if (TabUiFeatureUtilities.isTabGroupsAndroidEnabled(this) &&
      ...
      dialogVisibilitySupplier = () -> {
        ...
        // Return true if dialog from either tab switcher or tab strip is
        // visible.
    ...
    mUndoBarPopupController = new UndoBarController(
        this, mTabModelSelector, this::getSnackbarManager,
        mOverviewModeBehaviorSupplier, dialogVisibilitySupplier);
    ...
    if (StartupPaintPreviewHelper.isEnabled()) {
      StartupPaintPreviewHelper paintPreviewHelper =
          new StartupPaintPreviewHelper(
              getWindowAndroid(), getOnCreateTimestampMs(),
      ...
      getActivityTabStartupMetricsTracker().registerPaintPreviewObserver(
          paintPreviewHelper);
    }
  }
  ...
  private boolean shouldIgnoreIntent() {
    if (mShouldIgnoreIntent == null) {
      // We call this only once because mIntentHandler#shouldIgnoreIntent has
      // side effects.
      mShouldIgnoreIntent = mIntentHandler.shouldIgnoreIntent(
          getIntent(), /*startedActivity=*/true);
    }
  ...
  protected final void dispatchOnInflationComplete() {
    ...
    // When the feature flag {@link ChromeFeatureList.INSTANT_START} turns on
    // phones (not tablet), a view-only start page created on Java will be shown
    // before native is initialized. The {@link
    // prepareToShowStartPagePreNative()} is only called in a cold start.
  ...
  /**
   * Prepares to show the start page before native is initialized. For example,
   * create an LayoutManagerChrome object, add overview mode observer and so on.
   */
  private void prepareToShowStartPagePreNative() {
    assert TabUiFeatureUtilities.supportInstantStart(
        isTablet() && !hadWarmStart(), this);
    try (TraceEvent e = TraceEvent.scoped(
             "ChromeTabbedActivity.prepareToShowStartPagePreNative")) {
      setupCompositorContentPreNativeForPhone();
  ...
  protected TabModelOrchestrator createTabModelOrchestrator() {
    boolean tabMergingEnabled =
        ...
        mMultiInstanceManager.isTabModelMergingEnabled();
    mTabModelOrchestrator =
        new TabbedModeTabModelOrchestrator(tabMergingEnabled);
  ...
  protected void createTabModels() {
    ...
    // We determine the model as soon as possible so every systems get
    // initialized coherently.
    boolean startIncognito =
        savedInstanceState != null &&
        savedInstanceState.getBoolean(IS_INCOGNITO_SELECTED, false);

    mNextTabPolicySupplier =
        new ChromeNextTabPolicySupplier(mOverviewModeBehaviorSupplier);
    ...
    if (mMultiInstanceManager != null) {
      int assignedIndex =
          TabWindowManagerSingleton.getInstance().getIndexForWindow(this);
      // The given index and the one computed by TabWindowManager should be one
      // and the same.
      assert !MultiWindowUtils.isMultiInstanceApi31Enabled() ||
          assignedIndex == mWindowId;
    ...
    mTabModelSelectorTabObserver = new TabModelSelectorTabObserver(
      ...
      public void onDidFinishNavigation(Tab tab, NavigationHandle navigation) {
        ...
        DataReductionPromoInfoBar.maybeLaunchPromoInfoBar(
            ChromeTabbedActivity.this, tab.getWebContents(),
            navigation.getUrl(), tab.isShowingErrorPage(),
  ...
  private TabDelegateFactory getTabDelegateFactory() {
    if (mTabDelegateFactory == null) {
      mTabDelegateFactory = new TabbedModeTabDelegateFactory(
          ...
          this::getSnackbarManager, getBrowserControlsManager(),
          getActivityTabProvider(), getLifecycleDispatcher(),
          getWindowAndroid(), this::getLastUserInteractionTime,
  ...
  protected Pair<ChromeTabCreator, ChromeTabCreator> createTabCreators() {
    ...
    if (ReturnToChromeExperimentsUtil.isStartSurfaceEnabled(this)) {
      overviewNTPCreator = new ChromeTabCreator.OverviewNTPCreator() {
        ...
        public boolean handleCreateNTPIfNeeded(
          ...
          boolean shouldShowStart = showStartSurfaceHomeForNTP(
    ...
    return Pair.create(
        new ChromeTabCreator(
            this, getWindowAndroid(), getStartupTabPreloader(),
  ...
  protected void initDeferredStartupForActivity() {
    ...
    DeferredStartupHandler.getInstance().addDeferredTask(() -> {
      ...
      RecordHistogram.recordSparseHistogram("MemoryAndroid.DeviceMemoryClass",
                                            am.getMemoryClass());
      ...
      ChromeSurveyController.initialize(
          mTabModelSelector, getLifecycleDispatcher(),
          ChromeTabbedActivity.this,
  ...
  protected void recordIntentToCreationTime(long timeMs) {
    ...
    RecordHistogram.recordCustomTimesHistogram(
        "MobileStartup.IntentToCreationTime.TabbedMode", timeMs, 1,
        DateUtils.SECOND_IN_MILLIS * 30, 50);
  }
  ...
  protected boolean isStartedUpCorrectly(Intent intent) {
    ...
    if (savedInstanceState != null &&
        savedInstanceState.containsKey(WINDOW_INDEX)) {
      // Activity is recreated after destruction. |windowId| must not be valid
      // in this case.
      assert windowId == INVALID_WINDOW_ID;
      mWindowId = savedInstanceState.getInt(WINDOW_INDEX, 0);
    } else if (mMultiInstanceManager != null) {
      // |allocInstanceId| doesn't do any disk I/O that would add a long-running
      // task to pre-inflation startup.
    ...
    if (mWindowId == INVALID_WINDOW_ID) {
      ...
      Toast.makeText(this, R.string.max_number_of_windows, Toast.LENGTH_LONG)
          .show();
      return false;
    }
  ...
  private static int getExtraWindowIdFromIntent(Intent intent) {
    ...
    return IntentUtils.isTrustedIntentFromSelf(intent) ? windowId
                                                       : INVALID_WINDOW_ID;
  }

  private static boolean getExtraPreferNewFromIntent(Intent intent) {
    return IntentUtils.safeGetBooleanExtra(
        intent, IntentHandler.EXTRA_PREFER_NEW, false);
  }
  ...
  public boolean onMenuOrKeyboardAction(final int id, boolean fromMenu) {
    ...
    if (id == R.id.new_tab_menu_id) {
      ...
      if (IncognitoUtils.isIncognitoModeEnabled()) {
        getTabModelSelector().getModel(false).commitAllTabClosures();
        // This action must be recorded before opening the incognito tab since
        // UMA actions are dropped when an incognito tab is open.
      ...
      // Note that 'currentTab' could be null in overview mode when start
      // surface is enabled.
      ...
      if (currentTabIsNtp) {
        NewTabPageUma.recordAction(
            NewTabPageUma.ACTION_OPENED_BOOKMARKS_MANAGER);
      }

      @BrowserProfileType
      int type =
          getCurrentTabModel().isIncognito() ? BrowserProfileType.INCOGNITO
      ...
      LoadUrlParams params = new LoadUrlParams(UrlConstants.RECENT_TABS_URL,
                                               PageTransition.AUTO_BOOKMARK);
      boolean isInOverviewMode = isInOverviewMode();
      if (isInOverviewMode && !isTablet() &&
          ReturnToChromeExperimentsUtil.isStartSurfaceEnabled(this)) {
        // When tapping the "Recent tabs" menu item from the overview page
        // (Start surface or GTS), we will create the tab with the launch type
        // FROM_START_SURFACE. Thus, if the back button is tapped on this
        // "Recent tabs" page, it can go back to the overview page.
      ...
      if (currentTabIsNtp) {
        NewTabPageUma.recordAction(
            NewTabPageUma.ACTION_OPENED_RECENT_TABS_MANAGER);
      }
      ...
      DownloadUtils.showDownloadManager(this, currentTab, otrProfileID,
                                        DownloadOpenSource.MENU);
      if (currentTabIsNtp) {
        NewTabPageUma.recordAction(
            NewTabPageUma.ACTION_OPENED_DOWNLOADS_MANAGER);
      }
  ...
  public boolean handleBackPressed() {
    ...
    // TODO(1091411): Find a better mechanism for back-press handling for
    // features.
    if (mRootUiCoordinator.getBottomSheetController().handleBackPress())
      return true;

    if (mTabModalHandler.handleBackPress())
      return true;
    ...
    // If we are in the tab switcher mode (not in the Start surface homepage)
    // and not a tablet, then leave tab switcher mode on back.
    ...
    // If the current tab url is HELP_URL, then the back button should close the
    // tab to get back to the previous state. The reason for startsWith check is
    // that the actual redirected URL is a different system language based help
    // url.
    final @TabLaunchType int type = currentTab.getLaunchType();
    final boolean helpUrl =
        currentTab.getUrl().getSpec().startsWith(HELP_URL_PREFIX);
    if (type == TabLaunchType.FROM_CHROME_UI && helpUrl) {
      getCurrentTabModel().closeTab(currentTab);
      return true;
    }
    final boolean extensionUrl =
        currentTab.getUrl().getSpec().startsWith("chrome-extension://") ||
        currentTab.getUrl().getSpec().startsWith("kiwi-extension://");
    ...
    if (!mOverviewModeController.overviewVisible() &&
      ...
      if (StartSurfaceUserData.getKeepTab(currentTab) ||
          StartSurfaceUserData.isOpenedFromStart(currentTab)) {
        // If the current tab is created from the start surface with the keepTab
        // property, shows the Start surface non-incognito homepage to prevent a
        // loop between the current tab and previous overview mode. Once in the
        // Start surface, it will close Chrome if back button is tapped again.
        ...
        // Otherwise, clicking the back button should close the tab and go back
        // to the previous overview mode.
    ...
    // - we decided to close the tab, but it was opened by an external app, so
    // we will go
    ...
    assert false
        : "The back button should have already been handled by this point";
    return false;
  }

  /**
   * [true]: Reached the bottom of the back stack on a tab the user did not
   * explicitly create (i.e. it was created by an external app or opening a link
   * in background, etc). [false]: Reached the bottom of the back stack on a tab
   * that the user explicitly created (e.g. selecting "new tab" from menu).
   *
   * @return Whether pressing the back button on the provided Tab should close
   *     the Tab.
   */
  @Override
  public boolean backShouldCloseTab(Tab tab) {
    ...
    return type == TabLaunchType.FROM_LINK ||
        ...
        (type == TabLaunchType.FROM_RESTORE &&
         CriticalPersistedTabData.from(tab).getParentId() !=
             Tab.INVALID_TAB_ID);
  }
  ...
  private void sendToBackground(@Nullable final Tab tabToClose) {
    ...
    if (tabToClose != null) {
      ...
      // action to happen until after our app is minimized to make sure we don't
      // get a brief glimpse of the newly active tab before we exit Chrome.
      //
      // If the runnable doesn't run before the Activity dies, Chrome won't
      // crash but the tab won't be closed (crbug.com/587565).
   ...
   * @param forceNewTab   Whether to force the URL to be launched in a new tab
   *     or to fall
   *                      back to the default behavior for making that
   * determination.
   * @param isRendererInitiated Whether the intent is originally from browser
   *     renderer process.
   * @param initiatorOrigin Origin that initiates the intent.
   * @param intent        The original intent.
   */
  private Tab launchIntent(LoadUrlParams loadUrlParams, String externalAppId,
    ...
    if (IntentHandler.wasIntentSenderChrome(intent)) {
      // If the intent was launched by chrome, open the new tab in the
      // appropriate model.
      ...
      if (launchType == null) {
        if (IntentUtils.safeGetBooleanExtra(
          ...
          launchType = TabLaunchType.FROM_LAUNCHER_SHORTCUT;
                       ...
                       intent, IntentHandler.EXTRA_INVOKED_FROM_APP_WIDGET,
                       false)) {
          launchType = TabLaunchType.FROM_APP_WIDGET;
        } else if (IncognitoTabLauncher.didCreateIntent(intent)) {
          launchType = TabLaunchType.FROM_LAUNCH_NEW_INCOGNITO_TAB;
        } else {
          // Using FROM_LINK ensures the tab is parented to the current tab,
          // which allows the back button to close these tabs and restore
          // selection to the previous tab.
          launchType = TabLaunchType.FROM_LINK;
        }
      }
      return getTabCreator(isIncognito)
          .createNewTab(loadUrlParams, launchType, null, intent);
    }
    ...
    if (ReaderModeManager.isEnabled() &&
      ...
      int readerParentId = IntentUtils.safeGetInt(
          extras, ReaderModeManager.EXTRA_READER_MODE_PARENT,
          Tab.INVALID_TAB_ID);
  ...
  // TODO(crbug.com/1115757): After crrev.com/c/2315823, Overview state and
  // Startsurface state are two different things, we actual can split this into
  // two methods: showOverview() and showStartSurface(state). Let's do some
  // auditing and clean up before perform the actual split.
  private void showOverview(@StartSurfaceState int state,
                            @NewTabPageLaunchOrigin int launchOrigin) {
    ...
    if (mIsAccessibilityTabSwitcherEnabled != null &&
        mIsAccessibilityTabSwitcherEnabled && mOverviewModeController != null) {
      // TODO(1200727): This is a temporary fix that should be removed once grid
      // tab switcher
      //                is completely launched. The "start surface" is now
      //                created regardless of the state of accessibility, so we
      //                check that mode first and try showing the overview list
      //                before going to the start surface.
      mOverviewModeController.showOverview(false);
    } else if (mStartSurfaceSupplier.get() != null) {
      if (ReturnToChromeExperimentsUtil
              .shouldHideStartSurfaceWithAccessibilityOn(this) ||
  ...
  private void hideOverview() {
    ...
    if (activeLayout instanceof StackLayout) {
      ((StackLayout)activeLayout)
          .commitOutstandingModelState(LayoutManagerImpl.time());
    }
  ...
  /**
   * @return Whether opening a new tab is handled by the Start surface. It may
   *     show the Start
   * surface, or open a new tab with the omnibox get focused, depending on the
   * value of
   * {@link StartSurfaceConfiguration.OMNIBOX_FOCUSED_ON_NEW_TAB}.
   */
  private boolean
  showStartSurfaceHomeForNTP(boolean isNTP, boolean incognito, Tab parentTab,
                             @NewTabPageLaunchOrigin int launchOrigin) {
    if (!isNTP ||
        !ReturnToChromeExperimentsUtil.shouldShowStartSurfaceHomeAsNTP(
    ...
    if (StartSurfaceConfiguration.OMNIBOX_FOCUSED_ON_NEW_TAB.getValue() &&
      ...
      ReturnToChromeExperimentsUtil.handleLoadUrlFromStartSurfaceAsNewTab(
          ...
          getCurrentTabModel(), emptyTabCloseCallback);
  ...
  public void onSaveInstanceState(Bundle outState) {
    ...
    // If it's Incognito and native is initialized and profile exists, serialize
    // duration service state.
  ...
  public boolean dispatchKeyEvent(KeyEvent event) {
    Boolean result = KeyboardShortcuts.dispatchKeyEvent(
        event, mUIWithNativeInitialized, getFullscreenManager(),
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    ...
    // Detecting a long press of the back button via onLongPress is broken in
    // Android N. To work around this, use a postDelayed, which is supported in
    // all versions.
    if (keyCode == KeyEvent.KEYCODE_BACK && !isTablet() &&
        !getFullscreenManager().getPersistentFullscreenMode()) {
      if (mShowHistoryRunnable == null)
        mShowHistoryRunnable = this::showFullHistorySheet;
      mHandler.postDelayed(mShowHistoryRunnable,
    ...
    return KeyboardShortcuts.onKeyDown(
               event, isCurrentTabVisible, true, getTabModelSelector(),
               /* menuOrKeyboardActionController= */ this,
               getToolbarManager()) ||
  ...
  public LayoutManagerChrome getLayoutManager() {
    return (LayoutManagerChrome)getCompositorViewHolderSupplier()
        .get()
        .getLayoutManager();
  }
  ...
  private ComposedBrowserControlsVisibilityDelegate
  ...
  protected ModalDialogManager createModalDialogManager() {
    ...
    // TODO(crbug.com/1157310): Transition this::method refs to dedicated
    // suppliers.
    mTabModalHandler = new TabModalLifetimeHandler(
        this, getLifecycleDispatcher(), manager,
        this::getAppBrowserControlsVisibilityDelegate,
        this::getTabObscuringHandler, this::getToolbarManager,
        this::getContextualSearchManager, getTabModelSelectorSupplier(),
        this::getBrowserControlsManager, this::getFullscreenManager);
    return manager;
  }
  ...
  public boolean canShowAppMenu() {
    // The popup menu relies on the model created during the full UI
    // initialization, so do not attempt to show the menu until the UI creation
    // has finished.
    if (!mUIWithNativeInitialized)
      return false;

    // If the current active tab is showing a tab modal dialog, an app menu
    // shouldn't be shown in any cases, e.g. when a hardware menu button is
    // clicked.
    Tab tab = getActivityTab();
    if (tab != null && ChromeTabModalPresenter.isDialogShowing(tab))
      return false;
  ...
  public boolean isInOverviewMode() {
    return mOverviewModeController != null &&
        mOverviewModeController.overviewVisible();
  }
  ...
  public void onSceneChange(Layout layout) {
    ...
    if (!layout.shouldDisplayContentOverlay())
      mTabModelSelector.onTabsViewShown();
  }
  ...
  /**
   * Reports that a new tab launcher shortcut was selected or an action
   * equivalent to a shortcut was performed.
   * @param isIncognito Whether the shortcut or action created a new incognito
   *     tab.
   */
