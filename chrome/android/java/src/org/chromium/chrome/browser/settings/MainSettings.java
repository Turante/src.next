...
public class MainSettings extends PreferenceFragmentCompat
  ...
  // Used for elevating the privacy section behind the flag (see
  // crbug.com/1099233).
  ...
  public void onDestroy() {
    ...
    // The component should only be destroyed when the activity has been closed
    // by the user (e.g. by pressing on the back button) and not when the
    // activity is temporarily destroyed by the system.
  ...
  private void createPreferences() {
    ...
    mSyncPromoPreference.setOnStateChangedCallback(
        this::onSyncPromoPreferenceStateChanged);
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      // If we are on Android O+ the Notifications preference should lead to the
      // Android Settings notifications page.
      ...
      getPreferenceScreen().removePreference(
          findPreference(PREF_NOTIFICATIONS));
    }
    ...
    new AdaptiveToolbarStatePredictor(null).recomputeUiState(uiState -> {
      ...
      // finch. Note, we can still have the old data collection experiment
      // running for which |canShowUi| might be true. In that case, just hide
      // the settings page.
  ...
  /**
   * Stores all preferences in memory so that, if they needed to be
   * added/removed from the PreferenceScreen, there would be no need to reload
   * them from 'main_preferences.xml'.
   */
  private void cachePreferences() {
    ...
    mSyncPromoPreference =
        (SyncPromoPreference)mAllPreferences.get(PREF_SYNC_PROMO);
  ...
  private void setManagedPreferenceDelegateForPreference(String key) {
    ChromeBasePreference chromeBasePreference =
        (ChromeBasePreference)mAllPreferences.get(key);
    chromeBasePreference.setManagedPreferenceDelegate(
  ...
  private void updatePreferences() {
    ...
    dataReduction.setSummary(
        DataReductionPreferenceFragment.generateSummary(getResources()));
  }

  private Preference addPreferenceIfAbsent(String key) {
    Preference preference = getPreferenceScreen().findPreference(key);
    if (preference == null)
      getPreferenceScreen().addPreference(mAllPreferences.get(key));
  ...
  private void updateManageSyncPreference() {
    ...
    boolean isSyncConsentAvailable =
        IdentityServicesProvider.get()
            .getIdentityManager(Profile.getLastUsedRegularProfile())
            .getPrimaryAccountInfo(ConsentLevel.SYNC) != null;
    mManageSync.setIcon(SyncSettingsUtils.getSyncStatusIcon(getActivity()));
    mManageSync.setSummary(
        SyncSettingsUtils.getSyncStatusSummary(getActivity()));
  ...
  public void onSignedIn() {
    // After signing in or out of a managed account, preferences may change or
  ...
  private void onSyncPromoPreferenceStateChanged() {
    if (true)
      return;
    // Remove "Account" section header if the personalized sign-in promo is
    // shown.
    ...
    mSignInPreference.setIsShowingPersonalizedSigninPromo(
