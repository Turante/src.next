 ...
 * in reaction to the gesture which, if goes over a threshold, triggers
 * navigation. The caller that instantiates this view should add an {@link
 * #OnNavigateListener} to be notified whenever the gesture is completed. Based
 * on {@link org.chromium.third_party.android.swiperefresh.SwipeRefreshLayout}
...
public class SideSlideLayout extends ViewGroup {
  ...
  // Swipe offset in dips from the border of the view before applying physical
  // tension effect. The actual arrow bubble position is capped at a value three
  // times as this one, which is where navigation gets triggered.
  ...
  // Maximum amount of overscroll for a single side gesture action. An action is
  // regarded as an attempt to navigate via a gesture ('activated') and used for
  // UMA if the maximum overscroll is bigger than a certain threshold.
  ...
  // True while swiped to a distance where, if released, the navigation would be
  // triggered.
  ...
  private final Animation mAnimateToStartPosition = new Animation() {
    ...
    public void applyTransformation(float interpolatedTime, Transformation t) {
      int targetTop =
          mFrom + (int)((mOriginalOffset - mFrom) * interpolatedTime);
  ...
  public SideSlideLayout(Context context) {
    ...
    mCircleWidth = (int)getResources().getDimensionPixelSize(
        R.dimen.navigation_bubble_size);

    LayoutInflater layoutInflater = LayoutInflater.from(getContext());
    mArrowView = (NavigationBubble)layoutInflater.inflate(
        R.layout.navigation_bubble, null);
    ...
    // The absolute offset has to take into account that the circle starts at an
    // offset
    mTotalDragDistance =
        RAW_SWIPE_LIMIT_DP * getResources().getDisplayMetrics().density;
   ...
   * Stop navigation.
   */
  public void stopNavigating() { setNavigating(false); }
  ...
  private void startHidingAnimation(AnimationListener listener) {
    ...
    if (mNavigating && mListener != null)
      mListener.onNavigate(mIsForward);

    // ScaleAnimation needs to be created again if the arrow widget width
    // changes over time (due to turning on/off close indicator) to set the
    // right x pivot point.
    if (mHidingAnimation == null || mAnimationViewWidth != mArrowViewWidth) {
      mAnimationViewWidth = mArrowViewWidth;
      ScaleAnimation scalingDown = new ScaleAnimation(
          1, 0, 1, 0, mArrowViewWidth / 2, mArrowView.getHeight() / 2);
  ...
  public void setDirection(boolean forward) {
    mIsForward = forward;
    mArrowView.setIcon(forward ? R.drawable.ic_arrow_forward_blue_24dp
  ...
  public boolean start() {
    if (!isEnabled() || mNavigating || mListener == null)
      return false;
    mTotalMotion = 0;
    mMaxOverscroll = 0.f;
   ...
   */
  float getPullOffset() { return mTotalMotion; }
  ...
  public void pull(float offset) {
    ...
    if (overscroll > mMaxOverscroll)
      mMaxOverscroll = overscroll;
    ...
    float tensionPercent = (float)((tensionSlingshotPercent / 4) -
                                   Math.pow((tensionSlingshotPercent / 4), 2)) *
                           2f;
    ...
    // Tint the arrow blue when swiped enough to initiate navigation if
    // released.
  ...
  /**
   * @return {@code true} if swiped long enough to trigger navigation upon
   *     release.
   */
  ...
  private void hideCloseIndicator() {
    ...
    // The width when indicator text view is hidden is slightly bigger than the
    // height. Set the width to circle's diameter for the widget to be of
   ...
   * ignored. If the pull was sufficiently large, the navigation sequence will
   * be initiated.
  ...
  public void release(boolean allowNav) {
    ...
    if (activated)
      GestureNavMetrics.recordHistogram("GestureNavigation.Activated",
                                        mIsForward);

    if (isEnabled() && willNavigate()) {
      if (allowNav) {
        setNavigating(true);
        GestureNavMetrics.recordHistogram("GestureNavigation.Completed",
                                          mIsForward);
        long time = System.currentTimeMillis();
        if (sLastCompletedTime > 0 &&
            time - sLastCompletedTime < NAVIGATION_REVERSAL_MS &&
            mIsForward != sLastCompletedForward) {
          GestureNavMetrics.recordHistogram("GestureNavigation.Reversed",
                                            mIsForward);
        }
    ...
    if (activated)
      GestureNavMetrics.recordHistogram("GestureNavigation.Cancelled",
                                        mIsForward);
  }
