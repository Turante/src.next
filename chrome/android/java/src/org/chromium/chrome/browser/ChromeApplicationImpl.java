...
/**
 * Basic application functionality that should be shared among all browser
 * applications that use chrome layer.
 *
 * Note: All application logic should be added to {@link ChromeApplicationImpl},
 * which will be called from the superclass. See {@link SplitCompatApplication}
 * for more info.
 */
public class ChromeApplicationImpl extends SplitCompatApplication.Impl {
  ...
  public void onCreate() {
    ...
    boolean migrateToDarkTheme = ContextUtils.getAppSharedPreferences()
                                     .getString("active_theme", "")
                                     .equals("Diamond Black");
    if (migrateToDarkTheme) {
      SharedPreferencesManager.getInstance().writeIntUnchecked(
          "previous_ui_theme_setting", ThemeType.DARK);
    ...
    if (SplitCompatApplication.isBrowserProcess()) {
      ...
      if (CachedFeatureFlags.isEnabled(ChromeFeatureList.EARLY_LIBRARY_LOAD)) {
        // Kick off library loading in a separate thread so it's ready when we
        // need it.
  ...
  public void startActivity(Intent intent, Bundle options) {
    ...
    VrModuleProvider.getDelegate().requestToExitVr(
        new OnExitVrRequestListener() {
          @Override
          public void onSucceeded() {
            if (!VrModuleProvider.getDelegate().canLaunch2DIntents()) {
              throw new IllegalStateException(
                  "Still in VR after having exited VR.");
            }
  ...
  public void onConfigurationChanged(Configuration newConfig) {
    ...
    // TODO(huayinz): Add observer pattern for application configuration
    // changes.
    if (SplitCompatApplication.isBrowserProcess()) {
   ...
   * @param level The type of signal as defined in {@link
   *     android.content.ComponentCallbacks2}.
   */
  public static boolean isSevereMemorySignal(int level) {
    // The conditions are expressed using ranges to capture intermediate levels
    // possibly added to the API in the future.
  ...
  private static ChromeAppComponent createComponent() {
    ...
    ChromeAppModule module = overriddenFactory == null
                                 ? new ChromeAppModule()
                                 : overriddenFactory.create();

    AppHooksModule.Factory appHooksFactory =
        ModuleFactoryOverrides.getOverrideFor(AppHooksModule.Factory.class);
    AppHooksModule appHooksModule = appHooksFactory == null
                                        ? new AppHooksModule()
                                        : appHooksFactory.create();
