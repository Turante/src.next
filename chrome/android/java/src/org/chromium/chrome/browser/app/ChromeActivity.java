...
/**
 * A {@link AsyncInitializationActivity} that builds and manages a {@link
 * CompositorViewHolder} and associated classes.
...
public abstract class ChromeActivity<C extends ChromeActivityComponent>
    ...
    implements TabCreatorManager, PolicyChangeListener,
               ContextualSearchTabPromotionDelegate, SnackbarManageable,
               SceneChangeObserver,
               StatusBarColorController.StatusBarColorProvider, AppMenuDelegate,
               AppMenuBlocker, MenuOrKeyboardActionController,
  ...
  private final ObservableSupplierImpl<TabModelOrchestrator>
      mTabModelOrchestratorSupplier = new ObservableSupplierImpl<>();
  /** Used to access the {@link TabModelSelector} from {@link WindowAndroid}. */
  private final UnownedUserDataSupplier<TabModelSelector>
      mTabModelSelectorSupplier = new TabModelSelectorSupplier();
  /**
   * Used to access the {@link TabCreatorManager} from {@link WindowAndroid}.
   */
  private final UnownedUserDataSupplier<TabCreatorManager>
      mTabCreatorManagerSupplier = new TabCreatorManagerSupplier();
  private final UnownedUserDataSupplier<ManualFillingComponent>
      mManualFillingComponentSupplier = new ManualFillingComponentSupplier();
  // TODO(crbug.com/1209864): Move ownership to RootUiCoordinator.
  private final UnownedUserDataSupplier<BrowserControlsManager>
      mBrowserControlsManagerSupplier = new BrowserControlsManagerSupplier();
  ...
  /**
   * Set if {@link #postDeferredStartupIfNeeded()} is called before native has
   * loaded.
   */
  private boolean mDeferredStartupQueued;

  /**
   * Whether or not {@link #postDeferredStartupIfNeeded()} has already
   * successfully run.
   */
  ...
  // The PictureInPictureController is initialized lazily
  // https://crbug.com/729738.
  private PictureInPictureController mPictureInPictureController;

  private ObservableSupplierImpl<CompositorViewHolder>
      mCompositorViewHolderSupplier = new ObservableSupplierImpl<>();
  ...
  /**
   * A means of providing the foreground tab of the activity to different
   * features.
   */
  ...
  @Nullable private Bundle mMenuItemData;

  private String mMenuTitleCondensed;

  /**
   * The current configuration, used to for diffing when the configuration is
   * changed.
   */
   ...
   */
  private OneshotSupplierImpl<TabReparentingController>
      mTabReparentingControllerSupplier = new OneshotSupplierImpl<>();

  /**
   * Track whether {@link #mTabReparentingController} has prepared tab
   * reparenting.
   */
  ...
  /**
   * The RootUiCoordinator associated with the activity. This variable is held
  ...
  @Nullable private StartupTabPreloader mStartupTabPreloader;
  ...
  private List<MenuOrKeyboardActionController.MenuOrKeyboardActionHandler>
      mMenuActionHandlers = new ArrayList<>();

  private ShareRegistrationCoordinator mShareRegistrationCoordinator;

  // Whether this Activity is in Picture in Picture mode, based on the most
  // recent call to
  // {@link onPictureInPictureModeChanged} from the platform.  This might
  // disagree with the value returned by {@link isInPictureInPictureMode}.
  private boolean mLastPictureInPictureModeForTesting;
  private SubscriptionsManager mSubscriptionsManager;

  protected ChromeActivity() {
    mIntentHandler = new IntentHandler(this, createIntentHandlerDelegate());
    mManualFillingComponentSupplier.set(
  ...
  protected ActivityWindowAndroid createWindowAndroid() {
    return new ChromeWindow(
        /* activity= */ this, mActivityTabProvider,
  ...
  @VisibleForTesting
  public static void
  createWindowErrorSnackbar(String error, SnackbarManager snackbarManager) {
    if (snackbarManager != null) {
      Snackbar snackbar = Snackbar.make(error, null, Snackbar.TYPE_NOTIFICATION,
  ...
  public void performPreInflationStartup() {
    ...
    // Make sure the root coordinator is created prior to calling super to
    // ensure all the activity lifecycle events are called.
    mRootUiCoordinator = createRootUiCoordinator();

    // Create component before calling super to give its members a chance to
    // catch onPreInflationStartup event.
    ...
    // There's no corresponding call to removeObserver() for this addObserver()
    // because mTabModelProfileSupplier has the same lifecycle as this activity.
    mTabModelProfileSupplier.addObserver((profile) -> {
      BookmarkBridge oldBridge = mBookmarkBridgeSupplier.get();
      if (oldBridge != null)
        oldBridge.destroy();
      mBookmarkBridgeSupplier.set(
          profile == null ? null : new BookmarkBridge(profile));
    });

    super.performPreInflationStartup();

    VrModuleProvider.getDelegate().doPreInflationStartup(
        this, getSavedInstanceState());

    // Force a partner customizations refresh if it has yet to be initialized.
    // This can happen if Chrome is killed and you refocus a previous activity
    // from Android recents, which does not go through ChromeLauncherActivity
    // that would have normally triggered this.
    ...
    if (!commandLine.hasSwitch(ChromeSwitches.DISABLE_FULLSCREEN)) {
      ...
      getResources().getValue(R.dimen.top_controls_show_threshold, threshold,
                              true);
      commandLine.appendSwitchWithValue(
          ContentSwitches.TOP_CONTROLS_SHOW_THRESHOLD,
          threshold.coerceToString().toString());
      getResources().getValue(R.dimen.top_controls_hide_threshold, threshold,
                              true);
      commandLine.appendSwitchWithValue(
  ...
  private void setupUnownedUserDataSuppliers() {
    ...
    mTabModelSelectorSupplier.attach(
        getWindowAndroid().getUnownedUserDataHost());
    mTabCreatorManagerSupplier.attach(
        getWindowAndroid().getUnownedUserDataHost());
    mManualFillingComponentSupplier.attach(
        getWindowAndroid().getUnownedUserDataHost());
    mBrowserControlsManagerSupplier.attach(
        getWindowAndroid().getUnownedUserDataHost());
    // BrowserControlsManager is ready immediately.
    mBrowserControlsManagerSupplier.set(new BrowserControlsManager(
        this, BrowserControlsManager.ControlsPosition.TOP));
  }

  protected RootUiCoordinator createRootUiCoordinator() {
    // TODO(https://crbug.com/931496): Remove dependency on ChromeActivity in
    // favor of passing in direct dependencies on needed classes. While
    // migrating code from Chrome*Activity to the RootUiCoordinator, passing the
    // activity is an easy way to get access to a number of objects that will
    // ultimately be owned by the RootUiCoordinator. This is not a recommended
    // pattern.
    // clang-format off
  ...
  private C createComponent() {
    ChromeActivityCommonsModule.Factory overridenCommonsFactory =
        ModuleFactoryOverrides.getOverrideFor(
    ...
    ChromeActivityCommonsModule commonsModule =
        overridenCommonsFactory == null
            ? new ChromeActivityCommonsModule(
                  ...
                  getLifecycleDispatcher(), this::getSnackbarManager,
                  mActivityTabProvider, getTabContentManager(),
                  ...
                  getModalDialogManagerSupplier(), getBrowserControlsManager(),
                  this::getSavedInstanceState,
  ...
  /**
   * Override this to create a component that represents a richer dependency
   * graph for a particular subclass of ChromeActivity. The specialized
   * component should be activity-scoped and include all modules for
   * ChromeActivityComponent, such as
   * {@link ChromeActivityCommonsModule}, along with any additional modules.
   *
   * You may immediately resolve some of the classes belonging to the component
   * in this method.
   */
  @SuppressWarnings("unchecked")
  protected C createComponent(ChromeActivityCommonsModule commonsModule) {
    return (C)ChromeApplicationImpl.getComponent()
        .createChromeActivityComponent(commonsModule);
  }

  /**
   * @return the activity-scoped component associated with this instance of
   *     activity.
   */
  public final C getComponent() { return mComponent; }

  @SuppressLint("NewApi")
  @Override
  public void performPostInflationStartup() {
    try (TraceEvent te =
             TraceEvent.scoped("ChromeActivity.performPostInflationStartup")) {
      super.performPostInflationStartup();

      Intent intent = getIntent();
      if (getSavedInstanceState() == null) {
        VrModuleProvider.getDelegate().maybeHandleVrIntentPreNative(this,
                                                                    intent);
      }
      if (0 !=
      ...
      mSnackbarManager =
          new SnackbarManager(this, mBottomContainer, getWindowAndroid());
      ...
      // Set up the animation placeholder to be the SurfaceView. This disables
      // the SurfaceView's 'hole' clipping during animations that are notified
      // to the window.
      ...
      TabModelSelector tabModelSelector =
          mTabModelOrchestrator.getTabModelSelector();
      setTabContentManager(new TabContentManager(
          this, getContentOffsetProvider(), !SysUtils.isLowEndDevice(),
      ...
      mShareRegistrationCoordinator.registerShareType(
          PrintShareActivity.BROADCAST_ACTION,
      ...
      // If onStart was called before postLayoutInflation (because inflation was
      // done in a background thread) then make sure to call the relevant
      // methods belatedly.
      if (mStarted) {
   ...
   * #doLayoutInflation}, no calls to {@link
   * #getCompositorViewHolderSupplier().get()} can be done until inflation is
   * complete and {@link #onInitialLayoutInflationComplete()} is called. If the
   * subclass does not override {@link #doLayoutInflation}, then {@link
   * #getCompositorViewHolderSupplier().get()} is safe to be called after
   * calling super.
   */
  @Override
  protected final void triggerLayoutInflation() {
    ...
    try (TraceEvent te =
             TraceEvent.scoped("ChromeActivity.triggerLayoutInflation")) {
      ...
      if (warmupManager.hasViewHierarchyWithToolbar(
              getControlContainerLayoutId())) {
  ...
  /**
   * This function implements the actual layout inflation, Subclassing
   * Activities that override this method without calling super need to call
   * {@link #onInitialLayoutInflationComplete()}.
   */
  protected void doLayoutInflation() {
    try (TraceEvent te =
             TraceEvent.scoped("ChromeActivity.doLayoutInflation")) {
      // Allow disk access for the content view and toolbar container setup.
      // On certain android devices this setup sequence results in disk writes
      // outside of our control, so we have to disable StrictMode to work. See
      ...
      try (StrictModeContext ignored = StrictModeContext.allowDiskWrites()) {
        ...
        // It cannot be assumed that the result of
        // toolbarContainerStub.inflate() will be the control container since it
        // may be wrapped in another view.
        ControlContainer controlContainer =
            (ControlContainer)findViewById(R.id.control_container);

        if (controlContainer == null) {
          // omnibox_results_container_stub anchors off of control_container,
          // and will crash during layout if control_container doesn't exist.
          UiUtils.removeViewFromParent(
              findViewById(R.id.omnibox_results_container_stub));
        }

        // Inflate the correct toolbar layout for the device.
        int toolbarLayoutId = getToolbarLayoutId();
        if (toolbarLayoutId != ActivityUtils.NO_RESOURCE_ID &&
  ...
  protected void onInitialLayoutInflationComplete() {
    ...
    // If the UI was inflated on a background thread, then the CompositorView
    // may not have been fully initialized yet as that may require the creation
    // of a handler which is not allowed outside the UI thread. This call should
    // fully initialize the CompositorView if it hasn't been yet.
    mCompositorViewHolderSupplier.get().setRootView(rootView);

    // Setting fitsSystemWindows to false ensures that the root view doesn't
    // consume the insets.
    rootView.setFitsSystemWindows(false);

    // Add a custom view right after the root view that stores the insets to
    // access later. WebContents needs the insets to determine the portion of
  ...
  public final void initializeTabModels() {
    ...
    mRootUiCoordinator.getStatusBarColorController().setTabModelSelector(
        tabModelSelector);

    Pair<? extends TabCreator, ? extends TabCreator> tabCreators =
        createTabCreators();
  ...
  /**
   * @return The {@link StartupTabPreloader} associated with this
   *     ChromeActivity. If there isn't
   *         one it creates it.
   */
  protected StartupTabPreloader getStartupTabPreloader() {
    if (mStartupTabPreloader == null) {
      mStartupTabPreloader = new StartupTabPreloader(
          ...
          mIntentHandler, getActivityTabStartupMetricsTracker());
    }
    return mStartupTabPreloader;
  }

  /**
   * @return The {@link TabModelOrchestrator} owned by this {@link
   *     ChromeActivity}.
   */
   ...
   *         by this {@link ChromeActivity}.  The first item in the Pair is the
   * normal model tab creator, and the second is the tab creator for incognito
   * tabs.
   */
  protected abstract Pair<? extends TabCreator, ? extends TabCreator>
  createTabCreators();

  /**
   * @return {@link ToolbarManager} that belongs to this activity or null if the
   *     current activity
   *         does not support a toolbar.
   * TODO(pnoland, https://crbug.com/865801): remove this in favor of having
   * RootUICoordinator inject ToolbarManager directly to sub-components.
   */
  @Nullable
  ...
  /**
   * @return The {@link LaunchCauseMetrics} to be owned by this {@link
   *     ChromeActivity}.
   */
  ...
  public AppMenuPropertiesDelegate createAppMenuPropertiesDelegate() {
    return new AppMenuPropertiesDelegateImpl(
        this, getActivityTabProvider(), getMultiWindowModeStateDispatcher(),
        getTabModelSelector(), getToolbarManager(), getWindow().getDecorView(),
        null, null, mBookmarkBridgeSupplier);
  }

  /**
   * @return The resource id for the layout to use for {@link ControlContainer}.
   *     0 by default.
   */
   ...
   */
  protected int getToolbarLayoutId() { return ActivityUtils.NO_RESOURCE_ID; }

  /**
   * Returns the drawable resource id for the toolbar shadow.
   *
   * TODO(https://crbug.com/1227450): Rename toolbar shadow to hairline or
   * divider.
   */
  protected abstract @DrawableRes int getToolbarShadowResource();

  /**
   * Returns the layout height of the toolbar shadow in pixels, -1, or -2.
   * Negative values, -1 and -2, denote MATCH_PARENT and WRAP_CONTENT
   * respectively. See
   * https://developer.android.com/reference/android/view/ViewGroup.LayoutParams#MATCH_PARENT
   *
   * TODO(https://crbug.com/1227450): Rename toolbar shadow to hairline or
   * divider. This specific method might not even be necessary.
   */
  ...
  public void initializeState() {
    ...
    IntentHandler.setTestIntentsEnabled(CommandLine.getInstance().hasSwitch(
  ...
  public void onStartWithNative() {
    assert mNativeInitialized
        : "onStartWithNative was called before native was initialized.";
    ...
    // postDeferredStartupIfNeeded() is called in
    // TabModelSelectorTabObsever#onLoadStopped(), #onPageLoadFinished() and
    // #onCrash(). If we are not actively loading a tab (e.g. in Android N
    // multi-instance, which is created by re-parenting an existing tab), ensure
    // onDeferredStartup() gets called by calling postDeferredStartupIfNeeded()
    // here.
  ...
  /**
   * Returns whether deferred startup should be run if we are not actively
   * loading a tab (e.g. in Android N multi-instance, which is created by
   * re-parenting an existing tab).
   */
  ...
  private void onActivityShown() {
    ...
    if (tab != null) {
      if (tab.isHidden()) {
        ...
        // The visible Tab's renderer process may have died after the activity
        // was paused. Ensure that it's restored appropriately.
        tab.loadIfNeeded();
      }
    }
  ...
  private void onActivityHidden() {
    ...
    TabModelSelector tabModelSelector =
        mTabModelOrchestrator.getTabModelSelector();
    if (tabModelSelector != null &&
        !tabModelSelector.isReparentingInProgress() && tab != null) {
  ...
  public void onWindowFocusChanged(boolean hasFocus) {
    ...
    if (useWindowFocusForVisibility()) {
      if (hasFocus) {
        ...
        if (ApplicationStatus.getStateForActivity(this) ==
   ...
   * AND
   * - Browser is in a state where it can be themed (no  intersitial showing
   * etc.)
   * {@link TabState#UNSPECIFIED_THEME_COLOR} should be returned if the activity
   * should use the default color in this scenario.
   */
  ...
  private void createContextReporterIfNeeded() {
    if (!mStarted)
      return; // Sync state reporting should work only in started state.
    ...
    if (syncService != null &&
        syncService.isSyncingUrlsWithKeystorePassphrase()) {
  ...
  private static void reportSyncStatus(@Nullable SyncService syncService) {
    ...
    } else if (!syncService.getActiveDataTypes().contains(
                   ModelType.TYPED_URLS)) {
      ContextReporter.reportStatus(
          ContextReporter.STATUS_SYNC_NOT_SYNCING_URLS);
    } else if (syncService.getPassphraseType() !=
                   PassphraseType.KEYSTORE_PASSPHRASE &&
               syncService.getPassphraseType() !=
                   PassphraseType.TRUSTED_VAULT_PASSPHRASE) {
      ContextReporter.reportStatus(
          ContextReporter.STATUS_SYNC_NOT_KEYSTORE_PASSPHRASE);
    } else {
  ...
  public void onResumeWithNative() {
    ...
    final boolean useFixedUmaSessionResumeOrder = ChromeFeatureList.isEnabled(
        ChromeFeatureList.FIXED_UMA_SESSION_RESUME_ORDER);
    if (useFixedUmaSessionResumeOrder) {
      // First, update the activity type in order to have it properly captured
      // in markSessionResume; stage the activity type value such that it can be
      // picked up when the new UMA record is opened as a part of the subsequent
      // session resume.
      //
      // TODO(b/182286787): Move this inside markSessionResume, plumb through to
      // UMA session resumption.
      ChromeSessionState.setActivityType(getActivityType());

      // Close the current UMA record and start a new UMA one.
      markSessionResume();

      // Inform the actity lifecycle observers. Among other things, the
      // observers record metrics pertaining to the "resumed" activity. This
      // needs to happens after markSessionResume has closed the old UMA record,
      // pertaining to the previous (backgrounded) activity, and opened a new
      // one pertaining to the "resumed" activity.
  ...
  protected void onUserLeaveHint() {
    ...
    // The attempt might not be successful.  If it is, then
    // `onPictureInPictureModeChanged` will let us know later.  Note that the
    // activity might report that it is in PictureInPicture mode at any point
    // after this, which might be before we finish setup after receiving
    // notification from mOnPictureInPictureModeChanged.
  }

  /**
   * When we're notified that Picture-in-Picture mode has changed, make sure
   * that the controller is kept up-to-date.
   */
  @Override
  public void onPictureInPictureModeChanged(boolean inPicture,
  ...
  /**
   * Return the status of a Picture-in-Picture transition.  This is separate
   * from
   * {@link isInPictureInPictureMode}, because this will trigger only after we
   * have received and processed an Activity.onPictureInPictureModeChanged call.
   */
  ...
  public void onStopWithNative() {
    if (GSAState.getInstance(this).isGsaAvailable() &&
      ...
      if (mGSAAccountChangeListener != null)
        mGSAAccountChangeListener.disconnect();
    }
    if (mSyncStateChangedListener != null) {
  ...
  public void onNewIntent(Intent intent) {
    // This should be called before the call to super so that the needed VR
    // flags are set as soon as the VR intent is received.
  ...
  public void onNewIntentWithNative(Intent intent) {
    ...
    getLaunchCauseMetrics().onReceivedIntent();
    if (mIntentHandler.shouldIgnoreIntent(intent, /*startedActivity=*/false))
      return;

    // We send this intent so that we can enter WebVr presentation mode if
    // needed. This call doesn't consume the intent because it also has the url
   ...
   */
  @ActivityType public abstract int getActivityType();
  ...
  /**
   * Actions that may be run at some point after startup. Place tasks that are
   * not critical to the startup path here.  This method will be called
   * automatically.
   */
  ...
  /**
   * All deferred startup tasks that require the activity rather than the app
   * should go here.
   *
   * Overriding methods should queue tasks on the DeferredStartupHandler before
   * or after calling super depending on whether the tasks should run before or
   * after these ones.
   */
  @CallSuper
  protected void initDeferredStartupForActivity() {
    ...
    DeferredStartupHandler.getInstance().addDeferredTask(() -> {
      ...
      if (getToolbarManager() != null) {
        ...
        getToolbarManager().onDeferredStartup(getOnCreateTimestampMs(),
                                              simpleName);
      }

      if (MultiWindowUtils.getInstance().isInMultiWindowMode(
              ChromeActivity.this)) {
      ...
      int playServicesVersion =
          PlayServicesVersionInfo.getApkVersionNumber(this);
      RecordHistogram.recordBooleanHistogram("Android.PlayServices.Installed",
                                             playServicesVersion > 0);
      RecordHistogram.recordSparseHistogram("Android.PlayServices.Version",
                                            playServicesVersion);
    ...
    // GSA connection is not needed on low-end devices because Icing is
    // disabled.
  ...
  /**
   * Actions that may be run at some point after startup for Android N
   * multi-window mode. Should be called from #onDeferredStartup() if the
   * activity is in multi-window mode.
   */
  ...
  /**
   * Records the time it takes from creating an intent for {@link
   * ChromeActivity} to activity creation, including time spent in the
   * framework.
   * @param timeMs The time from creating an intent to activity creation.
   */
  @CallSuper
  protected void recordIntentToCreationTime(long timeMs) {
    RecordHistogram.recordTimesHistogram("MobileStartup.IntentToCreationTime",
                                         timeMs);
  }
  ...
  public void onStart() {
    ...
    if (AsyncTabParamsManagerSingleton.getInstance()
            .hasParamsWithTabToReparent()) {
      // TODO(https://crbug.com/1252526): Remove logging once root cause of bug
      // is identified
      //  & fixed.
      Log.i(TAG, "#onStart, num async tabs: " +
  ...
  /**
   * WARNING: DO NOT USE THIS METHOD. PASS TabObscuringHandler TO THE OBJECT
   * CONSTRUCTOR INSTEAD.
  ...
  public void onStop() {
    ...
    if (mCompositorViewHolderSupplier.hasValue())
      mCompositorViewHolderSupplier.get().onStop();

    // If postInflationStartup hasn't been called yet (because inflation was
    // done asynchronously and has not yet completed), it no longer needs to do
    // the belated onStart code since we were stopped in the mean time.
    mStarted = false;
  }
  ...
  public void onProvideAssistContent(AssistContent outContent) {
    ...
    // Attempt to fetch translate data here so we can record UMA even if it
    // won't be attached.
    @Nullable
    String structuredData =
        TranslateAssistContent.getTranslateDataForTab(tab, inOverviewMode);
  ...
  public void onPerformDirectAction(String actionId, Bundle arguments,
    ...
    mRootUiCoordinator.onPerformDirectAction(actionId, arguments,
  ...
  protected final void onDestroy() {
    ...
    // TODO(1168131): Destruction and detaching of the LayoutManager should be
    // moved to the
    //                RootUiCoordinator.
    ...
    if (mShareRegistrationCoordinator != null)
      mShareRegistrationCoordinator.destroy();
    ...
    ChromeActivitySessionTracker.getInstance()
        .unregisterTabModelSelectorSupplier(this);
  ...
  /**
   * Override this to perform destruction tasks.  Note that by the time this is
   * called, the
   * {@link CompositorViewHolder} will be destroyed, but the {@link
   * WindowAndroid} and
   * {@link TabModelSelector} will not.
   * <p>
   * After returning from this, the {@link TabModelSelector} will be destroyed
   * followed by the {@link WindowAndroid}.
   */
  ...
  public SnackbarManager getSnackbarManager() {
    BottomSheetController controller =
        mRootUiCoordinator == null
            ? null
            : mRootUiCoordinator.getBottomSheetController();
    if (mRootUiCoordinator != null && controller != null &&
        controller.isSheetOpen() && !controller.isSheetHiding()) {
  ...
  protected Drawable getBackgroundDrawable() {
    return new ColorDrawable(ApiCompatibilityUtils.getColor(
        getResources(), R.color.light_background_color));
  }

  /**
   * Change the Window background color that will be used as the resizing
   * background color on Android N+ multi-window mode. Note that subclasses can
   * override this behavior accordingly in case there is already a Window
   * background Drawable and don't want it to be replaced with the
   * ColorDrawable.
   */
  ...
  private void maybeRemoveWindowBackground() {
    ...
    if (mRemoveWindowBackgroundDone)
      return;

    // Remove the window background only after native init and window getting
    // focus. It's done after native init because before native init, a fake
    // background gets shown. The window focus dependency is because doing it
    // earlier can cause drawing bugs, e.g. crbug/673831.
    if (!mNativeInitialized || !hasWindowFocus())
      return;
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
      ...
      // Post the removeWindowBackground() call as a separate task, as doing it
      // synchronously here can cause redrawing glitches. See crbug.com/686662
  ...
  public void finishNativeInitialization() {
    ...
    // The first launch of the screenshot feature benefits from this DFM being
    // installed proactively. However without the isolated split feature there
    // are performance regressions as a result of adding this extra code.
    if (BundleUtils.isolatedSplitsEnabled() &&
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.O &&
        AppHooks.get().getImageEditorModuleProvider() != null) {
      AppHooks.get()
          .getImageEditorModuleProvider()
          .maybeInstallModuleDeferred();
    }

    super.finishNativeInitialization();

    mManualFillingComponentSupplier.get().initialize(
        getWindowAndroid(), mRootUiCoordinator.getBottomSheetController(),
        (ChromeKeyboardVisibilityDelegate)getWindowAndroid()
            .getKeyboardDelegate(),
    ...
    // This must be initialized after initialization of tab reparenting
    // controller.
    if (ChromeFeatureList.isEnabled(
      ...
      mSubscriptionsManager =
          factory.getForLastUsedProfile().getSubscriptionsManager();
    }

    // Make sure the user is reporting into one of the feed spinner groups, so
    // that we can analyze daily power impact for a typical Chrome user. The
    // flag only has an effect if the spinner is shown, but our earlier UMA
    // analysis shows that it may have a side-effect on a future browsing
    // session's power, even if the spinner is not shown (by causing more
    // cold-starts).
    ...
    WebsitePreferenceBridge.setContentSettingEnabled(
        ...
        ContextUtils.getAppSharedPreferences().getBoolean(
            "darken_websites_enabled", false));

    WebContentsDarkModeController.updateDarkModeStringSettings();
  }

  /**
   * @return OverviewModeBehavior if this activity supports an overview mode and
   *     the
  ...
  /**
   * @return {@link ObservableSupplier} for the {@link OverviewModeBehavior} for
   *     this activity
   *         if it supports an overview mode, null otherwise.
   */

  public @Nullable OneshotSupplier<OverviewModeBehavior>
   ...
   */
  public boolean didFinishNativeInitialization() { return mNativeInitialized; }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    if (item != null) {
      if (onOptionsItemSelected(item.getItemId(), null))
        return true;
    }
   ...
   */
  public boolean isInOverviewMode() { return false; }

  /**
   * Returns whether grid Tab switcher or the Start surface should be shown at
   * startup.
   */
  public boolean shouldShowOverviewPageOnStart() { return false; }
  ...
  public boolean canShowAppMenu() {
    ...
    if (state != ActivityState.RESUMED &&
        (!inMultiWindow || state != ActivityState.PAUSED)) {
      return false;
    }
  ...
  protected IntentHandlerDelegate createIntentHandlerDelegate() {
    return new IntentHandlerDelegate() {
      ...
      @Override
      public void processTranslateTabIntent(@Nullable String targetLanguageCode,
                                            @Nullable String expectedUrl) {}

      @Override
      public void processUrlViewIntent(
          LoadUrlParams loadUrlParams, @TabOpenType int tabOpenType,
          String externalAppId, int tabIdToBringToFront, Intent intent) {}
    };
  }

  /**
   * Add the specified tab to bookmarks or allows to edit the bookmark if the
   * specified tab is already bookmarked. If a new bookmark is added, a snackbar
   * will be shown.
   *
   ...
   *
   * @param tabToBookmark The tab that needs to be bookmarked.
   * @param fromExplicitTrackUi Whether the addOrEditAction was taken from an
   *     explicit tracking
   *         UI.
   */
  public void addOrEditBookmark(final Tab tabToBookmark,
  ...
  /**
   * Adds the specified tab to the Reading List. Opens a new item if an item was
   * added. Opens UI for editing the Reading List item if it was already present
   * on the list.
   *
  ...
  private void addOrEditBookmark(final Tab tabToBookmark,
    ...
    bookmarkModel.finishLoadingBookmarkModel(() -> {
      ...
      // TODO(crbug.com/1150559): Make getUserBookmarkIdForTab return
      // BookmarkItem instead, currently it's a sync call that doesn't check
      // loading states, and only checks the bookmark backend and managed
      // bookmarks.
      ...
      if (currentBookmarkItem == null) {
        // Note we get user bookmark ID over just a bookmark ID here: Managed
        // bookmarks can't be edited. If the current URL is only bookmarked by
        // managed bookmarks, this will return INVALID_ID.
        // TODO(bauerb): This does not take partner bookmarks into account.
        final BookmarkId bookmarkId =
            bridge.getUserBookmarkIdForTab(tabToBookmark);
        if (bookmarkId != null) {
      ...
      onBookmarkModelLoaded(tabToBookmark, currentBookmarkItem, bookmarkModel,
                            bookmarkType, fromExplicitTrackUi);
    });
  }

  private void onBookmarkModelLoaded(
      final Tab tabToBookmark, @Nullable final BookmarkItem currentBookmarkItem,
      final BookmarkModel bookmarkModel, @BookmarkType int bookmarkType,
      boolean fromExplicitTrackUi) {
    // The BookmarkModel will be destroyed by BookmarkUtils#addOrEditBookmark()
    // when done.
    BookmarkUtils.addOrEditBookmark(
        currentBookmarkItem, bookmarkModel, tabToBookmark, getSnackbarManager(),
        mRootUiCoordinator.getBottomSheetController(), ChromeActivity.this,
        isCustomTab(), bookmarkType, (newBookmarkId) -> {
          BookmarkId currentBookmarkId = (currentBookmarkItem == null)
                                             ? null
                                             : currentBookmarkItem.getId();
  ...
  /**
   * {@link TabModelSelector} no longer implements TabModel.  Use
   * getTabModelSelector() or getCurrentTabModel() depending on your needs.
   ...
   */
  public final ObservableSupplier<TabModelOrchestrator>
  getTabModelOrchestratorSupplier() {
    return mTabModelOrchestratorSupplier;
  }

  /**
   * Returns an {@link ObservableSupplier} for {@link TabModelSelector}. Prefer
   * this method over using {@link #getTabModelSelector()} directly.
   */
  public final ObservableSupplier<TabModelSelector>
   ...
   */
  protected OneshotSupplier<TabReparentingController>
   ...
   */
  public InsetObserverView getInsetObserverView() { return mInsetObserverView; }
  ...
  /**
   * Convenience method that returns a tab creator for the currently selected
   * {@link TabModel}.
  ...
  /**
   * Gets the {@link TabContentManager} instance which holds snapshots of the
   * tabs in this model.
  ...
  private void setTabContentManager(TabContentManager tabContentManager) {
    ...
    TabContentManagerHandler.create(tabContentManager, getFullscreenManager(),
                                    getTabModelSelector());
   ...
   * getModelSelector().getCurrentModel().  It is *not* equivalent to the former
   * getModel()
   * @return Never null, if modelSelector or its field is uninstantiated returns
   *     a
  ...
  public TabModel getCurrentTabModel() {
    ...
    if (modelSelector == null)
      return EmptyTabModel.getInstance();
    return modelSelector.getCurrentModel();
  }

  /**
   * DEPRECATED: Instead, use/hold a reference to {@link #mActivityTabProvider}.
   * See https://crbug.com/871279 for more details. Note that there are
   * important functional differences between {@link ActivityTabProvider} and
   * this function when transitioning to/from the tab switcher. For a drop-in
   * replacement, use
   *             {@link TabModelSelector#getCurrentTab} instead.
   *
   * Returns the tab being displayed by this ChromeActivity instance. This
   * allows differentiation between ChromeActivity subclasses that swap between
   * multiple tabs (e.g. ChromeTabbedActivity) and subclasses that only display
   * one Tab (e.g. DocumentActivity).
   *
   * The default implementation grabs the tab currently selected by the
   * TabModel, which may be null if the Tab does not exist or the system is not
   * initialized.
   */
  ...
  /**
   * @return The current WebContents, or null if the tab does not exist or is
   *     not showing a
   *         WebContents.
   */
   ...
   * @deprecated Instead, inject this directly to your constructor. If that's
   *     not possible, then
  ...
  public BrowserControlsManager getBrowserControlsManager() {
    if (!mBrowserControlsManagerSupplier.hasValue() &&
      ...
      // BrowserControlsManagerSupplier should always have a value unless it's
      // in the process of destruction (and in that case, nothing should be
      // called this method).
      throw new IllegalStateException();
    }
  ...
  /**
   * Exits the fullscreen mode, if any. Does nothing if no fullscreen is
   * present.
  ...
  public void initializeCompositorContent(LayoutManagerImpl layoutManager,
                                          ...
                                          ControlContainer controlContainer) {
    // TODO(1168131): The responsibility of managing the availability of the
    // LayoutManager
    ...
    compositorViewHolder.setBrowserControlsManager(
    ...
    compositorViewHolder.onFinishNativeInitialization(getTabModelSelector(),
                                                      this);
    ...
    if (mContextualSearchManager != null) {
      mContextualSearchManager.initialize(
          contentContainer, layoutManager,
  ...
  /**
   * @return An {@link ObservableSupplier} that will supply the {@link
   *     LayoutManagerImpl} when it
   *         is ready.
   */
  ...
  /**
   * @return An {@link ObservableSupplier} that will supply the {@link
   *     CompositorViewHolder} when
   *         it is ready.
   */
  public ObservableSupplier<CompositorViewHolder>
  ...
  /**
   * @return If no higher priority back actions occur, whether pressing the back
   *     button
   *         would result in closing the tab. A true return value does not
   * guarantee that a subsequent call to {@link #handleBackPressed()} will close
   * the tab.
   */
  public boolean backShouldCloseTab(Tab tab) { return false; }
  ...
  public void performOnConfigurationChanged(Configuration newConfig) {
    ...
    if (mConfig != null) {
      ...
      // We only handle VR UI mode and UI mode night changes. Any other changes
      // should follow the default behavior of recreating the activity. Note
      // that if UI mode night changes, with or without other changes, we will
      // still recreate() until we get a callback from the
      // ChromeBaseAppCompatActivity#onNightModeStateChanged or the overridden
      // method in sub-classes if necessary.
  ...
  /**
   * Called by the system when the activity changes from fullscreen mode to
   * multi-window mode and visa-versa.
   * @param isInMultiWindowMode True if the activity is in multi-window mode.
   */
  @Override
  public void onMultiWindowModeChanged(boolean isInMultiWindowMode) {
    // If native is not initialized, the multi-window user action will be
    // recorded in #onDeferredStartupForMultiWindowMode() and
    // CachedFeatureFlags#setIsInMultiWindowMode() will be called in
    // #onResumeWithNative(). Both of these methods require native to be
    // initialized, so do not call here to avoid crashing. See
    // https://crbug.com/797921.
    if (mNativeInitialized) {
      recordMultiWindowModeChanged(isInMultiWindowMode,
                                   /* isDeferredStartup= */ false);

      if (!isInMultiWindowMode && ApplicationStatus.getStateForActivity(this) ==
                                      ActivityState.RESUMED) {
        // Start a new UMA session when exiting multi-window mode if the
        // activity is currently resumed. When entering multi-window Android
        // recents gains focus, so ChromeActivity will get a call to
        // onPauseWithNative(), ending the current UMA session. When exiting
        // multi-window, however, if ChromeActivity is resumed it stays in that
        // state.
        markSessionEnd();
        markSessionResume();
        ChromeSessionState.setIsInMultiWindowMode(
    ...
    VrModuleProvider.getDelegate().onMultiWindowModeChanged(
        isInMultiWindowMode);
  ...
  /**
   * Records user actions and ukms associated with entering and exiting Android
   * N multi-window mode.
   * @param isInMultiWindowMode True if the activity is in multi-window mode.
   * @param isDeferredStartup True if the activity is deferred startup.
   */
  private void recordMultiWindowModeChanged(boolean isInMultiWindowMode,
                                            boolean isDeferredStartup) {
    MultiWindowUtils.getInstance().recordMultiWindowModeChanged(
        isInMultiWindowMode, isDeferredStartup, isFirstActivity(),
        getActivityTab());
  }
   ...
   */
  protected boolean isFirstActivity() { return true; }
  ...
  private void openChromeManagementPage() {
    ...
    TabCreator tabCreator =
        getTabCreator(currentTab != null && currentTab.isIncognito());
    if (tabCreator == null)
      return;

    tabCreator.createNewTab(new LoadUrlParams(UrlConstants.MANAGEMENT_URL,
                                              PageTransition.AUTO_TOPLEVEL),
                            TabLaunchType.FROM_CHROME_UI, getActivityTab());
  }

  /**
   * @return The {@link MenuOrKeyboardActionController} for registering menu or
   *     keyboard action handler for this activity.
   */
  ...
  @Override
  public void
  registerMenuOrKeyboardActionHandler(MenuOrKeyboardActionHandler handler) {
    mMenuActionHandlers.add(handler);
  }

  @Override
  public void
  unregisterMenuOrKeyboardActionHandler(MenuOrKeyboardActionHandler handler) {
  ...
  public boolean onMenuOrKeyboardAction(int id, boolean fromMenu) {
    for (MenuOrKeyboardActionController.MenuOrKeyboardActionHandler handler :
         mMenuActionHandlers) {
      if (handler.handleMenuOrKeyboardAction(id, fromMenu))
        return true;
    }
    ...
    if (mMenuTitleCondensed != null && !mMenuTitleCondensed.equals("") &&
        mMenuTitleCondensed.contains("Extension: ")) {
      ...
      Log.d("Kiwi", "Pressed extension menu: " + extensionId +
                        " - url: " + extensionUrl);
      Tab tab = getActivityTab();
      if (tab != null) {
        WebContents webContents = tab.getWebContents();
        LaunchMetrics.commitLaunchMetrics(webContents);
        AppMenuBridge.grantExtensionActiveTab(
            Profile.fromWebContents(webContents).getOriginalProfile(),
            webContents, extensionId);
        if (!extensionUrl.equals(""))
          getCurrentTabCreator().launchUrl(extensionUrl,
                                           TabLaunchType.FROM_CHROME_UI);
        else
          AppMenuBridge.callExtension(
              Profile.fromWebContents(webContents).getOriginalProfile(),
              webContents, extensionId);
        return true;
      }
    ...
    if (id == R.id.open_history_menu_id) {
      // 'currentTab' could only be null when opening history from start
      // surface, which is not available on tablet.
      ...
      RecordHistogram.recordEnumeratedHistogram(
          "Android.OpenHistoryFromMenu.PerProfileType", type,
          BrowserProfileType.MAX_VALUE + 1);
      return true;
    }

    // All the code below assumes currentTab is not null, so return early if it
    // is null.
    if (currentTab == null) {
      return false;
    }
    ...
    if (id == R.id.delete_from_reading_list_menu_id) {
      ReadingListUtils.deleteFromReadingList(mSnackbarManager,
    ...
    if (id == R.id.enable_price_tracking_menu_id) {
      ...
      if (bookmarkId == null) {
        ...
        PowerBookmarkUtils.setPriceTrackingEnabled(
            mSubscriptionsManager, mBookmarkBridgeSupplier.get(), bookmarkId,
            /*enabled=*/true, (status) -> {});
      }
    ...
    if (id == R.id.translate_id) {
      ...
      if (ContextUtils.getAppSharedPreferences()
        ...
        if (currentTab != null && !url.isEmpty()) {
          ...
          if (ContextUtils.getAppSharedPreferences()
          ...
          } else if (ContextUtils.getAppSharedPreferences()
                         .getString("active_translator", "")
                         .equals("Yandex")) {
            hasTranslated = false;
            try {
            ...
            if (!hasTranslated) {
              LoadUrlParams loadUrlParams = new LoadUrlParams(
                  "https://translate.yandex.com/?text=" + Uri.encode(url));
              currentTab.loadUrl(loadUrlParams);
            }
          } else if (ContextUtils.getAppSharedPreferences()
                         .getString("active_translator", "")
            ...
            hasTranslated = false;
            try {
              if (url != null && (url.startsWith("https://fanyi.baidu.com/") ||
            ...
            if (!hasTranslated) {
              LoadUrlParams loadUrlParams = new LoadUrlParams(
                  "http://fanyi.baidu.com/transpage?source=url&ie=utf8&from=auto&to=zh&render=1&query=" +
                  Uri.encode(url));
              currentTab.loadUrl(loadUrlParams);
            }
          } else {
            ...
            if (!hasTranslated) {
              LoadUrlParams loadUrlParams = new LoadUrlParams(
                  "http://www.microsofttranslator.com/bv.aspx?r=true&a=" +
                  Uri.encode(url));
              currentTab.loadUrl(loadUrlParams);
            }
          }
    ...
    if (id == R.id.add_to_homescreen_id) {
      ...
      AddToHomescreenCoordinator.showForAppMenu(
          this, getWindowAndroid(), getModalDialogManager(),
    ...
    if (id == R.id.open_webapk_id) {
      ...
      try {
        ...
        Toast.makeText(context, R.string.open_webapk_failed, Toast.LENGTH_SHORT)
            .show();
      }
      return true;
    }

    if (id == R.id.request_desktop_site_id ||
        id == R.id.request_desktop_site_check_id) {
      boolean usingDesktopUserAgent = currentTab.getWebContents()
      ...
      RequestDesktopUtils.recordUserChangeUserAgent(usingDesktopUserAgent,
                                                    getActivityTab());
      return true;
    }

    if (id == R.id.adblock_id || id == R.id.adblock_check_id) {
      boolean adBlockIsActive =
          (WebsitePreferenceBridgeJni.get().isContentSettingEnabled(
      ...
      if (!adBlockIsActive) {
        ...
        int adblockSettingForThisSite =
            WebsitePreferenceBridgeJni.get().getPermissionSettingForOrigin(
                Profile.getLastUsedRegularProfile(), ContentSettingsType.ADS,
                currentTab.getUrl().getSpec(), currentTab.getUrl().getSpec());
        if (adblockSettingForThisSite == ContentSettingValues.DEFAULT ||
            adblockSettingForThisSite == ContentSettingValues.BLOCK)
    ...
    if (id == R.id.disable_proxy_id) {
      AppMenuBridge.disableProxy(
          Profile.fromWebContents(currentTab.getWebContents())
              .getOriginalProfile());
    }

    if (id == R.id.auto_dark_web_contents_id ||
        id == R.id.auto_dark_web_contents_check_id) {
      ...
      // Show dialog informing user how to disable the feature globally and give
      // feedback if disabling through the app menu for the nth time (determined
    ...
    if (id == R.id.night_mode_switcher_id) {
      Log.d("Kiwi", "Initializing night mode with mode: " +
                        ContextUtils.getAppSharedPreferences().getString(
                            "active_nightmode", "default"));
      WebContentsDarkModeController.updateDarkModeStringSettings();
      ...
      if (!isDarkModeEnabled) {
        ...
        int currentTheme = ContextUtils.getAppSharedPreferences().getInt(
            "ui_theme_setting", 0);
        SharedPreferencesManager.getInstance().writeBooleanSync(
            "darken_websites_enabled", true);
        SharedPreferencesManager.getInstance().writeIntUnchecked(
            "previous_ui_theme_setting", currentTheme);
        SharedPreferencesManager.getInstance().writeInt("ui_theme_setting",
                                                        ThemeType.DARK);
      } else {
        getWindow().setBackgroundDrawable(
            new ColorDrawable(ApiCompatibilityUtils.getColor(
                getResources(), R.color.resizing_background_color)));
        int previousTheme = ContextUtils.getAppSharedPreferences().getInt(
            "previous_ui_theme_setting", 0);
        SharedPreferencesManager.getInstance().writeBooleanSync(
            "darken_websites_enabled", false);
        SharedPreferencesManager.getInstance().writeInt("ui_theme_setting",
                                                        previousTheme);
    ...
    if (id == R.id.clear_data_menu_id) {
      ...
      settingsLauncher.launchSettingsActivity(
          this, ClearBrowsingDataTabsFragment.class);
    }
   ...
   */
  public void startHelpAndFeedback(String url, String recordAction,
                                   Profile profile) {
    // Since reading back the compositor is asynchronous, we need to do the
    // readback before starting the GoogleHelp.
    String helpContextId = HelpAndFeedbackLauncherImpl.getHelpContextIdFromUrl(
        this, url, getCurrentTabModel().isIncognito());
    HelpAndFeedbackLauncherImpl.getInstance().show(this, helpContextId, profile,
                                                   url);
  ...
  private void markSessionEnd() {
    if (mUmaSessionStats == null) {
      // If you hit this assert, please update crbug.com/172653 on how you got
      // there.
      assert false;
      return;
    }
  ...
  /**
   * Looks up the Chrome activity of the given web contents. This can be null.
   * Should never be cached, because web contents can change activities, e.g.,
   * when user selects "Open in Chrome" menu item.
   *
   * @param webContents The web contents for which to lookup the Chrome
   *     activity.
   * @return Possibly null Chrome activity that should never be cached.
   * @deprecated Use {@link
   *     ActivityUtils#getActivityFromWebContents(WebContents)} instead.
   */
  @Nullable
  @Deprecated
  public static ChromeActivity
  fromWebContents(@Nullable WebContents webContents) {
  ...
  public boolean onActivityResultWithNative(int requestCode, int resultCode,
                                            Intent intent) {
    if (super.onActivityResultWithNative(requestCode, resultCode, intent))
      return true;
    if (VrModuleProvider.getDelegate().onActivityResultWithNative(requestCode,
                                                                  resultCode)) {
      return true;
    }
    return false;
  ...
  /**
   * Called when VR mode is entered using this activity. 2D UI components that
   * steal focus or draw over VR contents should be hidden in this call.
   */
  public void onEnterVr() {}

  /**
   * Called when VR mode using this activity is exited. Any state set for VR
   * should be restored in this call, including showing 2D UI that was hidden.
   */
  ...
  private void clearToolbarResourceCache() {
    ...
    try {
      ...
      // This is a workaround for crbug.com/1236981. Doing nothing here is
      // better than crashing. We assert, which will be stripped in builds that
      // get shipped to users.
      Log.e(TAG, "crbug.com/1236981", e);
      String extraInfo = "";
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
        extraInfo = " inflated from layout ID #" + v.getSourceLayoutResId();
      }
      assert false : "View " + v.toString() + extraInfo +
  ...
  public boolean supportsAppMenu() {
    // Derived classes that disable the toolbar should also have the Menu
    // disabled without having to explicitly disable the Menu as well.
   ...
   */
  public boolean supportsFindInPage() { return true; }
  ...
  public void onNightModeStateChanged() {
    // Note: order matters here because the call to super will recreate the
    // activity. Note: it's possible for this method to be called before
    // mNightModeReparentingController is constructed.
    if (mTabReparentingControllerSupplier.get() != null) {
  ...
  public boolean didChangeTabletMode() {
    ...
    boolean wasTablet = mConfig.smallestScreenWidthDp >=
                        DeviceFormFactor.MINIMUM_TABLET_WIDTH_DP;
    return wasTablet != isTablet;
  }

  /**
   * Switch between phone and tablet mode and do the tab re-parenting in the
   * meantime.
   */
  ...
  private boolean doPrintShare(Activity activity,
    ...
    printingController.startPrint(new TabPrinter(currentTabSupplier.get()),
