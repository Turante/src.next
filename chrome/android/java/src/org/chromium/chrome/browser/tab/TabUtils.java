...
public class TabUtils {
  private static final String REQUEST_DESKTOP_SCREEN_WIDTH_PARAM =
      "screen_width_dp";
  ...
  public static Rect estimateContentSize(Context context) {
    ...
    // The bounds rectangle includes the bottom bar and the custom tabs bar as
    // well.
  ...
  public static void switchUserAgent(Tab tab, boolean switchToDesktop,
    ...
    if (forcedByUser) {
      @TabUserAgent
      int tabUserAgent =
          switchToDesktop ? TabUserAgent.DESKTOP : TabUserAgent.MOBILE;
      if (ContentFeatureList.isEnabled(
              ContentFeatureList.REQUEST_DESKTOP_SITE_GLOBAL) &&
          isDesktopSiteGlobalEnabled(Profile.fromWebContents(
              tab.getWebContents())) == switchToDesktop) {
  ...
  /**
   * Check if the tab is large enough for displaying desktop sites. This method
   * will only check for tablets, if the device is a phone, will return false
   * regardless of tab size.
   * @param tab The tab to be checked if the size is large enough for desktop
   *     site.
  ...
  public static boolean isTabLargeEnoughForDesktopSite(Tab tab) {
    if ((ContextUtils.getAppSharedPreferences().getBoolean("desktop_mode",
                                                           false)))
      return true;
    ...
    if (activity == null) {
      // It is possible that we are in custom tabs or tests, and need to access
