...
public class TabbedRootUiCoordinator extends RootUiCoordinator {
  ...
  private final ObservableSupplierImpl<EphemeralTabCoordinator>
  ...
  private StatusIndicatorCoordinator
      .StatusIndicatorObserver mStatusIndicatorObserver;
  private OfflineIndicatorControllerV2 mOfflineIndicatorController;
  private OfflineIndicatorInProductHelpController
  ...
  private @Nullable ToolbarButtonInProductHelpController
  ...
  private AddToHomescreenMostVisitedTileClickObserver
  ...
  private ContinuousSearchContainerCoordinator
      mContinuousSearchContainerCoordinator;
  ...
  // Activity tab observer that updates the current tab used by various UI
  // components.
   ...
   * @param tabProvider The {@link ActivityTabProvider} to get current tab of
   *     the activity.
   * @param profileSupplier Supplier of the currently applicable profile.
   * @param bookmarkBridgeSupplier Supplier of the bookmark  bridge for the
   *     current profile.
   * @param contextualSearchManagerSupplier Supplier of the {@link
   *     ContextualSearchManager}.
   * @param tabModelSelectorSupplier Supplies the {@link TabModelSelector}.
   * @param startSurfaceSupplier Supplier of the {@link StartSurface}.
   * @param intentMetadataOneshotSupplier Supplier with information about the
   *     launching intent.
   * @param layoutStateProviderOneshotSupplier Supplier of the {@link
   *     LayoutStateProvider}.
   * @param startSurfaceParentTabSupplier Supplies the parent tab for the
   *     StartSurface.
   * @param browserControlsManager Manages the browser controls.
   * @param windowAndroid The current {@link WindowAndroid}.
   * @param jankTracker Tracks the jank in the app.
   * @param activityLifecycleDispatcher Allows observation of the activity
   *     lifecycle.
   * @param layoutManagerSupplier Supplies the {@link LayoutManager}.
   * @param menuOrKeyboardActionController Controls the menu or keyboard action
   *     controller.
   ...
   * @param compositorViewHolderSupplier Supplies the {@link
   ...
   * @param isInOverviewModeSupplier Supplies whether the app is in overview
   *     mode.
   * @param shouldShowOverviewPageOnStartSupplier Supplies whether the overview
   *     page should be
   *         shown on start.
   ...
   * @param ephemeralTabCoordinatorSupplier Supplies the {@link
   *     EphemeralTabCoordinator}.
   * @param intentRequestTracker Tracks intent requests.
   * @param controlContainerHeightResource The resource for the control
   *     container.
   * @param insetObserverViewSupplier Supplier for the {@link
   *     InsetObserverView}.
   * @param backButtonShouldCloseTabFn Function which supplies whether or not
   *     the back button
   *         should close the tab.
   * @param tabReparentingControllerSupplier Supplier for the {@link
   *     TabReparentingController}.
   * @param initializeUiWithIncognitoColors Whether to initialize the UI with
   *     incognito colors.
   */
  public TabbedRootUiCoordinator(
      ...
      @NonNull Supplier<ContextualSearchManager>
          contextualSearchManagerSupplier,
      @NonNull ObservableSupplier<TabModelSelector> tabModelSelectorSupplier,
      @NonNull OneshotSupplier<StartSurface> startSurfaceSupplier,
      @NonNull OneshotSupplier<ToolbarIntentMetadata>
          intentMetadataOneshotSupplier,
      @NonNull OneshotSupplier<LayoutStateProvider>
      ...
      @NonNull ObservableSupplier<ModalDialogManager>
      ...
      @NonNull OneshotSupplier<OverviewModeBehavior>
    ...
    mCanAnimateBrowserControls = () -> {
      // These null checks prevent any exceptions that may be caused by
      // callbacks after destruction.
      if (mActivity == null || mActivityTabProvider == null)
        return false;
  ...
  public void onDestroy() {
    if (mSystemUiCoordinator != null)
      mSystemUiCoordinator.destroy();
    if (mEmptyBackgroundViewWrapper != null)
      mEmptyBackgroundViewWrapper.destroy();
    ...
    if (mToolbarManager != null) {
      mToolbarManager.getOmniboxStub().removeUrlFocusChangeListener(
    ...
    if (mRootUiTabObserver != null)
      mRootUiTabObserver.destroy();

    if (mAddToHomescreenIPHController != null)
      mAddToHomescreenIPHController.destroy();

    if (mAppBannerInProductHelpController != null) {
      AppBannerInProductHelpControllerFactory.detach(
    ...
    if (mHistoryNavigationCoordinator != null) {
      TouchEventObserver obs =
          mHistoryNavigationCoordinator.getTouchEventObserver();
    ...
    if (mContinuousSearchContainerCoordinator != null) {
      getTabObscuringHandler().removeObserver(
          mContinuousSearchTabObscuringHandlerObserver);
      mContinuousSearchContainerCoordinator.removeHeightObserver(
          mContinuousSearchObserver);
      if (mFindToolbarManager != null) {
        mFindToolbarManager.removeObserver(
            mContinuousSearchFindToolbarObserver);
      }
  ...
  public void onPostInflationStartup() {
    ...
    mSystemUiCoordinator = new TabbedSystemUiCoordinator(
        mActivity.getWindow(), mTabModelSelectorSupplier.get(),
        mOverviewModeBehaviorSupplier, mFullscreenManager);
  }
  ...
  protected void onFindToolbarShown() {
    ...
    if (coordinator != null && coordinator.isOpened())
      coordinator.close();
  }

  /**
   * @return The toolbar button IPH controller for the tabbed UI this
   *     coordinator controls.
   * TODO(pnoland, https://crbug.com/865801): remove this in favor of wiring it
   * directly.
   */
  ...
  public void showFullHistorySheet() {
    ...
    mNavigationSheet = NavigationSheet.create(
        ...
        mActivity, this::getBottomSheetController, profile);
    ...
    if (!mNavigationSheet.startAndExpand(/* forward= */ false,
                                         /* animate=*/true)) {
  ...
  public void onFinishNativeInitialization() {
    ...
    // final Function<Tab, Boolean> backButtonShouldCloseTabFn =
    ...
    mHistoryNavigationCoordinator = HistoryNavigationCoordinator.create(
        ...
        mCompositorViewHolderSupplier.get()::removeTouchEventObserver,
        mLayoutManager);
    ...
    // TODO(twellington): Supply TabModelSelector as well and move
    // initialization earlier.
    if (DeviceFormFactor.isNonMultiDisplayContextOnTablet(mActivity)) {
      AppMenuHandler appMenuHandler =
          mAppMenuCoordinator == null ? null
      ...
      mEmptyBackgroundViewWrapper = new EmptyBackgroundViewWrapper(
          ...
          appMenuHandler, mSnackbarManagerSupplier.get(),
    ...
    if (EphemeralTabCoordinator.isSupported()) {
      mEphemeralTabCoordinatorSupplier.set(new EphemeralTabCoordinator(
          mActivity, mWindowAndroid, mActivity.getWindow().getDecorView(),
    ...
    mIntentMetadataOneshotSupplier.onAvailable(
        mCallbackController.makeCancelable(
            (metadata) -> initializeIPH(metadata.getIsIntentWithEffect())));

    // TODO(https://crbug.com/1157955): Investigate switching to per-Activity
    // coordinator that uses signals from the current Tab to decide when to show
    // the PWA install bottom sheet rather than relying on unowned user data.
    mPwaBottomSheetController =
        PwaBottomSheetControllerFactory.createPwaBottomSheetController(
            mActivity);
    PwaBottomSheetControllerFactory.attach(mWindowAndroid,
                                           mPwaBottomSheetController);
  ...
  @Override
  protected void
  setLayoutStateProvider(LayoutStateProvider layoutStateProvider) {
  ...
  protected ScrimCoordinator buildScrimWidget() {
    ...
    return new ScrimCoordinator(
        mActivity, delegate, coordinator,
  ...
  private void initializeIPH(boolean intentWithEffect) {
    ...
    mToolbarButtonInProductHelpController =
        new ToolbarButtonInProductHelpController(
            mActivity, mWindowAndroid, mAppMenuCoordinator,
            mActivityLifecycleDispatcher, mActivityTabProvider,
            mIsInOverviewModeSupplier, mToolbarManager.getMenuButtonView(),
            mToolbarManager.getSecurityIconView());
    mReadLaterIPHController = new ReadLaterIPHController(
    ...
    if (mOfflineIndicatorController != null) {
      ...
      // mOfflineIndicatorController is enabled and initialized. For example, it
      // wouldn't be initialized if the OfflineIndicatorV2 feature is disabled.
      assert mOfflineIndicatorInProductHelpController == null;
      mOfflineIndicatorInProductHelpController =
          new OfflineIndicatorInProductHelpController(
              mActivity, mToolbarManager,
              mAppMenuCoordinator.getAppMenuHandler(),
              mStatusIndicatorCoordinator);
    }

    mAddToHomescreenIPHController = new AddToHomescreenIPHController(
        mActivity, mWindowAndroid, mModalDialogManagerSupplier.get(),
        mAppMenuCoordinator.getAppMenuHandler(),
    ...
    mLinkToTextIPHController = new LinkToTextIPHController(
        mActivityTabProvider, mTabModelSelectorSupplier.get());
    mAddToHomescreenMostVisitedTileObserver =
        new AddToHomescreenMostVisitedTileClickObserver(
            mActivityTabProvider, mAddToHomescreenIPHController);
    mAppBannerInProductHelpController =
        AppBannerInProductHelpControllerFactory
            .createAppBannerInProductHelpController(
    ...
    if (!didTriggerPromo &&
      ...
      // TODO(crbug.com/1252965): Investigate locking feature engagement system
      // during "second run promos" to avoid !didTriggerPromo check.
      Tab tab;
  ...
  private void updateTopControlsHeight(boolean animate) {
    ...
    browserControlsSizer.setTopControlsHeight(topControlsNewHeight,
  ...
  private void initUndoGroupSnackbarController() {
    if (TabUiFeatureUtilities.isTabGroupsAndroidEnabled(mActivity)) {
      mUndoGroupSnackbarController = new UndoGroupSnackbarController(
          mActivity, mTabModelSelectorSupplier.get(),
  ...
  private void initStatusIndicatorCoordinator(LayoutManagerImpl layoutManager) {
    if (ContextUtils.getAppSharedPreferences().getBoolean(
      ...
      return;
    // TODO(crbug.com/1035584): Disable on tablets for now as we need to do one
    // or two extra things for tablets.
    if (DeviceFormFactor.isNonMultiDisplayContextOnTablet(mActivity) ||
    ...
    mStatusIndicatorCoordinator = new StatusIndicatorCoordinator(
        mActivity, mCompositorViewHolderSupplier.get().getResourceManager(),
    ...
    // Don't initialize the offline indicator controller if the feature is
    // disabled.
    if (!ChromeFeatureList.isEnabled(ChromeFeatureList.OFFLINE_INDICATOR_V2)) {
      return;
    }

    ObservableSupplierImpl<Boolean> isUrlBarFocusedSupplier =
    ...
    mUrlFocusChangeListener = new UrlFocusChangeListener() {
      ...
      public void onUrlFocusChange(boolean hasFocus) {
        // Offline indicator should assume the UrlBar is focused if it's
        // focusing.
        if (hasFocus) {
    ...
    if (mToolbarManager.getOmniboxStub() != null) {
      mToolbarManager.getOmniboxStub().addUrlFocusChangeListener(
          mUrlFocusChangeListener);
    }
  }
  ...
  private void initContinuousSearchCoordinator() {
    ...
    Supplier<Integer> defaultTopContainerHeightSupplier = ()
        -> mActivity.getResources().getDimensionPixelSize(
            mControlContainerHeightResource);
    final ViewStub viewStub =
        mActivity.findViewById(R.id.continuous_search_container_stub);
    final BrowserControlsSizer browserControlsSizer = mBrowserControlsManager;
    mContinuousSearchContainerCoordinator =
        new ContinuousSearchContainerCoordinator(
            viewStub, mLayoutManager,
            mCompositorViewHolderSupplier.get().getResourceManager(),
            mActivityTabProvider, browserControlsSizer,
            mCanAnimateBrowserControls, defaultTopContainerHeightSupplier,
            getTopUiThemeColorProvider(), mActivity,
    ...
    mContinuousSearchContainerCoordinator.addHeightObserver(
        mContinuousSearchObserver);
    mContinuousSearchTabObscuringHandlerObserver = isObscured
        -> mContinuousSearchContainerCoordinator.updateTabObscured(isObscured);
    getTabObscuringHandler().addObserver(
        mContinuousSearchTabObscuringHandlerObserver);

    if (!mSupportsFindInPageSupplier.getAsBoolean())
      return;
  ...
  /**
   * @return {@link ComposedBrowserControlsVisibilityDelegate} object for tabbed
   *     activity.
   */
  ...
  @VisibleForTesting
  public HistoryNavigationCoordinator
  getHistoryNavigationCoordinatorForTesting() {
  ...
  /**
   * Triggers the display of an appropriate promo, if any, returning true if a
   * promo is actually displayed.
   */
  private boolean triggerPromo(boolean intentWithEffect) {
    try (TraceEvent e =
             TraceEvent.scoped("TabbedRootUiCoordinator.triggerPromo")) {
      ...
      // after FRE is complete. Native initialization can finish before the FRE
      // flow is complete, and this will only show promos on the second
      // opportunity. This is because the FRE is shown on the first opportunity,
      // and we don't want to show such content back to back.
      //
      // TODO(https://crbug.com/865801, pnoland): Unify promo dialog logic and
      // move into a single PromoDialogCoordinator.
      ...
      if (!isShowingPromo && !intentWithEffect &&
          ...
          // VrModuleProvider.getDelegate().isInVr may not return true at this
          // point even though Chrome is about to enter VR, so we need to also
          // check whether we're launching into VR.
  ...
  private boolean maybeShowPromo() {
    ...
    if (SigninPromoUtil.launchSigninPromoIfNeeded(
            mActivity, SyncConsentActivityLauncherImpl.get(),
    ...
    if (AppLanguagePromoDialog.maybeShowPrompt(
            mActivity, mModalDialogManagerSupplier,
