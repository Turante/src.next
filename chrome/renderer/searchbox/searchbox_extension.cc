...
// "title".
v8::Local<v8::Object>
GenerateMostVisitedItem(v8::Isolate *isolate, float device_pixel_ratio,
                        int render_frame_id,
...
v8::Local<v8::Object>
GenerateMostVisitedItemData(v8::Isolate *isolate, int render_view_id,
                            InstantRestrictedID restricted_id,
...
SearchBox *GetSearchBoxForCurrentContext() {
  ...
  LOG(INFO) << "[Kiwi] SearchBox* GetSearchBoxForCurrentContext - main_frame: "
            << main_frame;
  if (!main_frame)
    return nullptr;
  LOG(INFO) << "[Kiwi] SearchBox* GetSearchBoxForCurrentContext - searchbox: "
            << SearchBox::Get(main_frame);
...
private:
  ...
  GetObjectTemplateBuilder(v8::Isolate *isolate) final;
...
gin::ObjectTemplateBuilder
SearchBoxBindings::GetObjectTemplateBuilder(v8::Isolate *isolate) {
...
gin::ObjectTemplateBuilder
NewTabPageBindings::GetObjectTemplateBuilder(v8::Isolate *isolate) {
...
bool NewTabPageBindings::GetMostVisitedAvailable(v8::Isolate *isolate) {
  const SearchBox *search_box = GetSearchBoxForCurrentContext();
  if (search_box)
    LOG(INFO) << "[Kiwi] NewTabPageBindings::GetMostVisitedAvailable - "
                 "search_box available";
  else
    LOG(INFO) << "[Kiwi] NewTabPageBindings::GetMostVisitedAvailable - "
                 "search_box NOT available";
  if (!search_box)
    return false;
...
void NewTabPageBindings::UndoMostVisitedDeletion(
    v8::Isolate *isolate, v8::Local<v8::Value> rid_value) {
  // Manually convert to integer, so that the string "\"1\"" is also accepted.
  absl::optional<int> rid = CoerceToInt(isolate, *rid_value);
  if (!rid.has_value())
    return;
...
// static
v8::Local<v8::Value>
NewTabPageBindings::GetMostVisitedItemData(v8::Isolate *isolate, int rid) {
  const SearchBox *search_box = GetSearchBoxForCurrentContext();
