...
public:
  ...
  std::string
  GetURLStringFromRestrictedID(InstantRestrictedID rid) const override;
...
bool ParseFrameIdAndRestrictedId(const std::string &id_part, int *frame_id_out,
...
bool ParseIconRestrictedUrl(const GURL &url, std::string *param_part,
                            int *frame_id, InstantRestrictedID *rid) {
  DCHECK(param_part);
  DCHECK(frame_id);
  DCHECK(rid);
  // Strip leading slash.
...
SearchBox::SearchBox(content::RenderFrame *render_frame)
    : content::RenderFrameObserver(render_frame),
      ...
      can_run_js_in_renderframe_(false), page_seq_no_(0), is_focused_(false),
      is_input_in_progress_(false), is_key_capture_enabled_(false),
...
void SearchBox::OnDestruct() { delete this; }
