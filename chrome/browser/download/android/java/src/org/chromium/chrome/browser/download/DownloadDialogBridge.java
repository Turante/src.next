...
/**
 * Glues download dialogs UI code and handles the communication to download
 * native backend. When {@link ChromeFeatureList#DOWNLOAD_LATER} is enabled, the
 * following dialogs will be shown in a sequence. Download later dialog ==>
 * (optional) Download date time picker ==> Download location dialog When {@link
 * ChromeFeatureList#DOWNLOAD_LATER} is disabled, only the download location
...
public class DownloadDialogBridge
  ...
  private static DownloadDialogBridge create(long nativeDownloadDialogBridge) {
    DownloadLocationDialogCoordinator locationDialog =
        new DownloadLocationDialogCoordinator();
    DownloadDateTimePickerDialog dateTimePickerDialog =
        new DownloadDateTimePickerDialogImpl();
    DownloadLaterDialogCoordinator downloadLaterDialog =
  ...
  private boolean downloadWithAdm(WindowAndroid windowAndroid, long totalBytes,
    ...
    boolean useAdmIfPossible =
        ContextUtils.getAppSharedPreferences().getBoolean(
            "enable_external_download_manager", false);
    ...
    String activeDownloadManagerPackageName =
        ContextUtils.getAppSharedPreferences().getString(
    ...
    if (useAdmIfPossible &&
        ...
        activeDownloadManagerPackageName.equals("com.kiwibrowser.browser") !=
            true &&
      ...
      if (urlToDownload.toLowerCase(Locale.ROOT)
              .contains(".googleusercontent.com/crx"))
        return false;
      Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(urlToDownload));
      //            intent.addCategory("android.intent.category.BROWSABLE");
      intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
      intent.setClassName(activeDownloadManagerPackageName,
                          activeDownloadManagerActivityName);

      intent.putExtra("android.intent.extra.TEXT", urlToDownload);

      // ADM
      intent.putExtra("com.android.extra.filename",
      ...
      try {
        Log.i("Kiwi", "[Download] Starting activity intent [" +
                          activeDownloadManagerPackageName + "/" +
                          activeDownloadManagerActivityName + "]");
        activity.startActivity(intent);
        return true;
      } catch (ActivityNotFoundException exception) {
        Log.i("Kiwi",
              "[Download] Starting activity intent: ActivityNotFoundException");
      }
    }
    return false;
  }
  ...
  private void showDialog(WindowAndroid windowAndroid, long totalBytes,
                          ...
                          boolean isIncognito, String urlToDownload) {
    Activity activity = windowAndroid.getActivity().get();
    if (activity == null) {
      onCancel();
      return;
    }
    if (downloadWithAdm(windowAndroid, totalBytes, suggestedPath,
                        urlToDownload)) {
      onCancel();
      return;
    }
    ...
    DownloadDirectoryProvider.getInstance().getAllDirectoriesOptions((dirs) -> {
      ...
      showDialog(activity, modalDialogManager, getPrefService(), totalBytes,
                 connectionType, suggestedDialogType, suggestedPath,
                 supportsLaterDialog, isIncognito);
    });
  }

  @VisibleForTesting
  void showDialog(Context context, ModalDialogManager modalDialogManager,
                  PrefService prefService, long totalBytes,
                  @ConnectionType int connectionType,
                  @DownloadLocationDialogType int dialogType,
                  String suggestedPath, boolean supportsLaterDialog,
                  boolean isIncognito) {
    ...
    // Download later dialogs flow only when the network is cellular, where
    // supportsLaterDialog is true.
    ...
    mLocationDialog.showDialog(mContext, mModalDialogManager, totalBytes,
  ...
  @Override
  public void onDownloadLaterDialogComplete(@DownloadLaterDialogChoice
                                            int choice, long startTime) {
    mDownloadLaterChoice = choice;
    mDownloadLaterTime = startTime;

    DownloadLaterMetrics.recordDownloadLaterDialogChoice(
        choice, DownloadDialogBridgeJni.get().isDataReductionProxyEnabled(),
        mTotalBytes);
    ...
    // The location dialog has error message text, show the location dialog
    // after the download later dialog. isIncognito is false because
    // DownloadLater is not available in Incognito.
  ...
  public void onEditLocationClicked() {
    ...
    // The user clicked the edit location text. isIncognito is false because
    // DownloadLater is not available in Incognito.
  ...
  private void showLocationDialog(boolean editLocation, boolean isIncognito) {
    ...
    mLocationDialog.showDialog(mContext, mModalDialogManager, mTotalBytes,
                               mLocationDialogType, mSuggestedPath,
                               isIncognito);
  }
  ...
  private void showDownloadLaterDialog() {
    ...
    PropertyModel.Builder builder =
        new PropertyModel.Builder(DownloadLaterDialogProperties.ALL_KEYS)
            .with(DownloadLaterDialogProperties.CONTROLLER,
                  mDownloadLaterDialog)
    ...
    mDownloadLaterDialog.showDialog(mContext, mModalDialogManager, mPrefService,
                                    builder.build());
    DownloadLaterMetrics.recordDownloadLaterUiEvent(
  ...
  private String getDownloadLaterDialogSubtitle() {
    if (mConnectionType == ConnectionType.CONNECTION_2G) {
      return mContext.getResources().getString(
          R.string.download_later_slow_network_subtitle,
    ...
    if (mTotalBytes >=
        DownloadDialogBridgeJni.get().getDownloadLaterMinFileSize()) {
      return mContext.getResources().getString(
  ...
  public void onDownloadLocationDialogComplete(String returnedPath) {
    ...
    if (mLocationDialogType == DownloadLocationDialogType.LOCATION_SUGGESTION) {
      boolean isSelected =
          !mSuggestedPath.equals(getDownloadDefaultDirectory());
      DownloadLocationDialogMetrics.recordDownloadLocationSuggestionChoice(
          isSelected);
    }
    ...
    // The location dialog is triggered by the "Edit" text. Show the download
    // later dialog again.
    mEditLocation = false;
    showDownloadLaterDialog();
  }
  ...
  public static void setDownloadAndSaveFileDefaultDirectory(String directory) {
    DownloadDialogBridgeJni.get().setDownloadAndSaveFileDefaultDirectory(
        directory);
  }

  /**
   * @return The status of prompt for download pref, defined by {@link
   ...
   */
  public static void
  setPromptForDownloadAndroid(@DownloadPromptStatus int status) {
    getPrefService().setInteger(Pref.PROMPT_FOR_DOWNLOAD_ANDROID, status);
  }

  /**
   * @return The value for {@link Pref#PROMPT_FOR_DOWNLOAD}. This is currently
   *     only used by
   * enterprise policy.
   */
  ...
  /**
   * @return whether to prompt the download location dialog is controlled by
   *     enterprise policy.
   */
  ...
  public interface Natives {
    void onComplete(long nativeDownloadDialogBridge,
                    DownloadDialogBridge caller, String returnedPath,
                    boolean onWifi, long startTime);
