...
public class DownloadSettings extends PreferenceFragmentCompat
  ...
  static final String PREF_DOWNLOAD_LATER_PROMPT_ENABLED =
  ...
  public void onCreatePreferences(@Nullable Bundle savedInstanceState,
    ...
    mDownloadLaterPromptEnabledPref = (ChromeSwitchPreference)findPreference(
        PREF_DOWNLOAD_LATER_PROMPT_ENABLED);
    ...
    if (locationManaged ||
        !ChromeFeatureList.isEnabled(ChromeFeatureList.DOWNLOAD_LATER)) {
    ...
    mExternalDownloadManager = (ChromeBaseCheckBoxPreference)findPreference(
        "enable_external_download_manager");
  }
  ...
  public void onDisplayPreferenceDialog(Preference preference) {
    if (preference instanceof DownloadLocationPreference) {
      ...
      dialogFragment.show(getFragmentManager(),
                          DownloadLocationPreferenceDialog.TAG);
    } else {
  ...
  private void updateDownloadSettings() {
    ...
    if (DownloadDialogBridge.isLocationDialogManaged()) {
      ...
      // Location prompt is marked enabled if the prompt status is not
      // DONT_SHOW.
    ...
    if (mExternalDownloadManager != null) {
      ...
      mExternalDownloadManager.setChecked(
          ContextUtils.getAppSharedPreferences().getBoolean(
              "enable_external_download_manager", false));
      if (ContextUtils.getAppSharedPreferences().getBoolean(
              "enable_external_download_manager", false) &&
          !TextUtils.isEmpty(ContextUtils.getAppSharedPreferences().getString(
              "selected_external_download_manager_package_name", ""))) {
        mExternalDownloadManager.setSummary(
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    Log.i("Kiwi",
          "[DownloadPreferences] Received activity result, RQ: " + requestCode);
    if (requestCode == 4242 && resultCode == Activity.RESULT_OK &&
      ...
      Log.i("Kiwi", "[DownloadPreferences] Received activity result, PN: " +
                        packageName + " - AN: " + activityName);
      SharedPreferences.Editor sharedPreferencesEditor =
          ContextUtils.getAppSharedPreferences().edit();
      sharedPreferencesEditor.putString(
          "selected_external_download_manager_package_name", packageName);
      sharedPreferencesEditor.putString(
          "selected_external_download_manager_activity_name", activityName);
    ...
    if (PREF_DOWNLOAD_LATER_PROMPT_ENABLED.equals(preference.getKey())) {
      ...
      if (!(boolean)newValue) {
        mPrefService.setInteger(Pref.DOWNLOAD_LATER_PROMPT_STATUS,
                                DownloadLaterPromptStatus.DONT_SHOW);
        return true;
      }
      ...
      SharedPreferences.Editor sharedPreferencesEditor =
          ContextUtils.getAppSharedPreferences().edit();
      sharedPreferencesEditor.putBoolean("enable_external_download_manager",
                                         (boolean)newValue);
      sharedPreferencesEditor.apply();
      if ((boolean)newValue == true) {
        List<Intent> targetedShareIntents = new ArrayList<Intent>();
        Intent shareIntent = new Intent(android.content.Intent.ACTION_VIEW,
                                        Uri.parse("http://test.com/file.rar"));
        ...
        List<ResolveInfo> resInfo =
            getActivity().getPackageManager().queryIntentActivities(shareIntent,
                                                                    0);
        if (!resInfo.isEmpty()) {
          for (ResolveInfo info : resInfo) {
            if (!"com.kiwibrowser.browser".equalsIgnoreCase(
                    info.activityInfo.packageName)) {
              Intent targetedShare =
                  new Intent(android.content.Intent.ACTION_VIEW);
              targetedShare.setPackage(
                  info.activityInfo.packageName.toLowerCase(Locale.ROOT));
          ...
          intentPick.putExtra(Intent.EXTRA_INITIAL_INTENTS,
                              targetedShareIntents.toArray());
          // Call StartActivityForResult so we can get the app name selected by
          // the user
          this.startActivityForResult(intentPick,
                                      /* REQUEST_CODE_MY_PICK */ 4242);
        }
      }
    }
    return true;
