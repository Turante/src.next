...
void DownloadDialogBridge::ShowDialog(
    gfx::NativeWindow native_window, int64_t total_bytes,
    net::NetworkChangeNotifier::ConnectionType connection_type,
    DownloadLocationDialogType dialog_type,
    const base::FilePath &suggested_path, bool supports_later_dialog,
    bool show_date_time_picker, bool is_incognito,
    DialogCallback dialog_callback, download::DownloadItem *download) {
  if (!native_window)
    return;
  ...
  Java_DownloadDialogBridge_showDialog(
      ...
      base::android::ConvertUTF8ToJavaString(env, url_to_download));
}

void DownloadDialogBridge::OnComplete(
    JNIEnv *env, const base::android::JavaParamRef<jobject> &obj,
    const base::android::JavaParamRef<jstring> &returned_path, jboolean on_wifi,
    jlong start_time) {
  DownloadDialogResult dialog_result;
...
void JNI_DownloadDialogBridge_SetDownloadAndSaveFileDefaultDirectory(
    JNIEnv *env, const base::android::JavaParamRef<jstring> &directory) {
  PrefService *pref_service =
      ProfileManager::GetActiveUserProfile()->GetOriginalProfile()->GetPrefs();
