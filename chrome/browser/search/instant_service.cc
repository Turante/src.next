...
InstantService::InstantService(Profile *profile)
  ...
  content::URLDataSource::Add(profile_, std::make_unique<NewTabPageSource>());
...
void InstantService::OnNewTabPageOpened() {
  ...
  if (most_visited_sites_) {
    LOG(INFO)
        << "[Kiwi] InstantService::OnNewTabPageOpened - most_visited_sites_";
...
void InstantService::UpdateMostVisitedInfo() { NotifyAboutMostVisitedInfo(); }
...
void InstantService::OnURLsAvailable(
  ...
  LOG(INFO) << "[Kiwi] InstantService::OnURLsAvailable - tiles: "
            << tiles.size();
...
bool InstantService::ShouldServiceRequest(
    const GURL &url, content::BrowserContext *browser_context,
