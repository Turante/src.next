...
/**
 * This class is responsible for rendering the 'Add to home screen' IPH, such as
 * IPH bubble/message creation, metrics logging etc.
 */
public class AddToHomescreenIPHController {
   ...
   * @param modalDialogManager The {@link ModalDialogManager} for showing the
   *     dialog.
   * @param appMenuHandler The {@link AppMenuHandler}.
   * @param highlightMenuItemId The resource id of 'Add to Home screen' in the
   *     app menu.
   * @param menuButtonView The view representing the menu button.
   * @param messageDispatcher The {@link MessageDispatcher} for displaying
   *     messages.
   */
  public AddToHomescreenIPHController(Activity activity,
  ...
  public void showAddToHomescreenIPH(Tab tab) {
    if (mActivity == null)
      return;
    if (!canShowAddToHomescreenMenuItem(mActivity, tab))
      return;

    if (ChromeFeatureList.getFieldTrialParamByFeatureAsBoolean(
            ChromeFeatureList.ADD_TO_HOMESCREEN_IPH,
            VARIATION_KEY_USE_TEXT_BUBBLE, false)) {
      showTextBubbleIPH();
    } else if (ChromeFeatureList.getFieldTrialParamByFeatureAsBoolean(
                   ChromeFeatureList.ADD_TO_HOMESCREEN_IPH,
                   VARIATION_KEY_USE_MESSAGE, false)) {
      showMessageIPH(tab);
    }
  }
  ...
  private static boolean canShowAddToHomescreenMenuItem(Context context,
                                                        Tab tab) {
    if (tab.isIncognito())
      return false;
    // Always disable can add to home screen for now, to-do add a ContextUtils
    // check
    if (true)
      return false;
    ...
    boolean isContentScheme =
        urlString.startsWith(UrlConstants.CONTENT_URL_PREFIX);
    boolean urlSchemeSupported =
        !isChromeScheme && !isFileScheme && !isContentScheme;

    if (!urlSchemeSupported)
      return false;
    ...
    ResolveInfo resolveInfo =
        WebApkValidator.queryFirstWebApkResolveInfo(context, urlString);
    ...
    AppBannerManager manager =
        webContents != null ? AppBannerManager.forWebContents(webContents)
                            : null;
    ...
    PropertyModel model =
        new PropertyModel.Builder(MessageBannerProperties.ALL_KEYS)
            ...
            .with(MessageBannerProperties.ON_DISMISSED,
                  this::onMessageDismissed)
            .with(MessageBannerProperties.ON_PRIMARY_ACTION,
                  () -> onMessageAddButtonClicked(tab))
            .build();
    mMessageDispatcher.enqueueMessage(model, tab.getWebContents(),
  ...
  private void onMessageAddButtonClicked(Tab tab) {
    if (tab.isDestroyed() || mActivity == null)
      return;

    Bundle menuItemData = new Bundle();
    // Used for UMA.
    menuItemData.putInt(AppBannerManager.MENU_TITLE_KEY,
                        AppMenuVerbiage.APP_MENU_OPTION_ADD_TO_HOMESCREEN);
    AddToHomescreenCoordinator.showForAppMenu(
        mActivity, mWindowAndroid, mModalDialogManager, tab.getWebContents(),
        menuItemData);
