...
ExtensionActionViewController::Create(
    const extensions::ExtensionId &extension_id, Browser *browser,
...
ExtensionActionViewController::ExtensionActionViewController(
    ...
    ExtensionsContainer *extensions_container)
    : extension_(std::move(extension)), browser_(browser),
      extension_action_(extension_action),
      extensions_container_(extensions_container), popup_host_(nullptr),
      view_delegate_(nullptr),
      platform_delegate_(ExtensionActionPlatformDelegate::Create(this)),
      icon_factory_(browser->profile(), extension_.get(), extension_action,
                    this),
...
gfx::Image
ExtensionActionViewController::GetIcon(content::WebContents *web_contents,
                                       const gfx::Size &size) {
  if (!ExtensionIsValid())
    return gfx::Image();
...
bool ExtensionActionViewController::TriggerPopupWithUrl(
    PopupShowAction show_action, const GURL &popup_url,
    bool grant_tab_permissions) {
  if (!ExtensionIsValid())
    return false;
...
void ExtensionActionViewController::ShowPopup(
    ...
    bool grant_tab_permissions, PopupShowAction show_action) {
