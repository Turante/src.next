 ...
 *
 * The icon row in the menu is a special case of a MenuItem that displays
 * multiple buttons in a row. If, for some unfathomable reason, you need to add
 * yet another icon to the row (the current max is five), then you will need to:
 *
...
class AppMenuAdapter extends BaseAdapter {
  @IntDef({MenuItemType.STANDARD, MenuItemType.TITLE_BUTTON,
           MenuItemType.THREE_BUTTON, MenuItemType.FOUR_BUTTON,
           MenuItemType.FIVE_BUTTON})
  @Retention(RetentionPolicy.SOURCE)
  @VisibleForTesting
  @interface MenuItemType {
    /**
     * Regular Android menu item that contains a title and an icon if icon is
     * specified.
     */
    int STANDARD = 0;
    /**
     * Menu item that has two buttons, the first one is a title and the second
     * one is an icon. It is different from the regular menu item because it
     * contains two separate buttons.
     */
    int TITLE_BUTTON = 1;
    /**
     * Menu item that has three buttons. Every one of these buttons is displayed
     * as an icon.
     */
    int THREE_BUTTON = 2;
    /**
     * Menu item that has four buttons. Every one of these buttons is displayed
     * as an icon.
     */
    int FOUR_BUTTON = 3;
    /**
     * Menu item that has five buttons. Every one of these buttons is displayed
     * as an icon.
     */
    int FIVE_BUTTON = 4;
    /**
     * The number of view types specified above.  If you add a view type you
     * MUST increment this.
     */
    int NUM_ENTRIES = 5;
  }
  ...
  private static final int[] BUTTON_IDS = {R.id.button_one, R.id.button_two,
  ...
  public AppMenuAdapter(AppMenuClickHandler appMenuClickHandler,
                        List<MenuItem> menuItems, LayoutInflater inflater,
    ...
    populateCustomViewBinderOffsetMap(customViewBinders, mViewTypeOffsetMap,
                                      MenuItemType.NUM_ENTRIES);
  }
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    case MenuItemType.STANDARD: {
      ...
      if (convertView == null ||
        ...
        if (mIconBeforeItem) {
          convertView = mInflater.inflate(R.layout.menu_item_start_with_icon,
                                          parent, false);
        } else {
        ...
        holder.image =
            (ChromeImageView)convertView.findViewById(R.id.menu_item_icon);
    ...
    case MenuItemType.THREE_BUTTON:
      convertView =
          createMenuItemRow(convertView, parent, item, 3, itemViewType);
      break;
    case MenuItemType.FOUR_BUTTON:
      convertView =
          createMenuItemRow(convertView, parent, item, 4, itemViewType);
      break;
    case MenuItemType.FIVE_BUTTON:
      convertView =
          createMenuItemRow(convertView, parent, item, 5, itemViewType);
      break;
    case MenuItemType.TITLE_BUTTON: {
      ...
      if (convertView == null ||
        ...
        convertView =
            mInflater.inflate(R.layout.title_button_menu_item, parent, false);

        holder = new TitleButtonMenuItemViewHolder();
        holder.title =
            (TextViewWithCompoundDrawables)convertView.findViewById(R.id.title);
        holder.checkbox =
            (AppMenuItemIcon)convertView.findViewById(R.id.checkbox);
        holder.button =
            (ChromeImageButton)convertView.findViewById(R.id.button);
      ...
      holder.title.setOnClickListener(
          v -> mAppMenuClickHandler.onItemClick(titleItem));
    ...
    default:
      ...
      for (int i = 0; i < mCustomViewBinders.size(); i++) {
        ...
        // If the convertView wasn't previously used for the same item view
        // type, set it back to null so that the custom binder isn't passed a
        // view that it can't/shouldn't re-use.
        if (convertView != null &&
    ...
    if (getCustomItemViewType(item) == CustomViewBinder.NOT_HANDLED) {
      ...
      if (mHighlightedItemId != null &&
          item.getItemId() == mHighlightedItemId) {
  ...
  /**
   * @return Whether the menu item corresponding to {@code menuItemId} is
  ...
  private void setupCheckBox(AppMenuItemIcon button, final MenuItem item) {
    ...
    // The checkbox must be tinted to make Android consistently style it across
    // OS versions. http://crbug.com/571445
    ApiCompatibilityUtils.setImageTintList(
        button, AppCompatResources.getColorStateList(button.getContext(),
                                                     R.color.checkbox_tint));
  ...
  private void setupMenuButton(View button, final MenuItem item) {
    ...
    button.setOnLongClickListener(
        v -> mAppMenuClickHandler.onItemLongClick(item, v));

    if (mHighlightedItemId != null && item.getItemId() == mHighlightedItemId) {
      ViewHighlighter.turnOnHighlight(
          button, new HighlightParams(HighlightShape.CIRCLE));
    } else {
      ViewHighlighter.turnOffHighlight(button);
    }

    // Menu items may be hidden by command line flags before they get to this
    // point.
    button.setVisibility(item.isVisible() ? View.VISIBLE : View.GONE);
  }

  private void
  setupStandardMenuItemViewHolder(StandardMenuItemViewHolder holder,
    ...
    if (titleCondensed != null &&
        titleCondensed.toString().contains("Extension: ")) {
  ...
  /**
   * This builds an {@link Animator} for the enter animation of a standard menu
   * item.  This means it will animate the alpha from 0 to 1 and translate the
   * view from -10dp to 0dp on the y axis.
   *
   * @param view     The menu item {@link View} to be animated.
   * @param position The position in the menu.  This impacts the start delay of
   *     the animation.
   * @return         The {@link Animator}.
   */
  private Animator buildStandardItemEnterAnimator(final View view,
                                                  int position) {
    final int startDelay =
        ENTER_ITEM_BASE_DELAY_MS + ENTER_ITEM_ADDL_DELAY_MS * position;
  ...
  /**
   * This builds an {@link Animator} for the enter animation of icon row menu
   * items.  This means it will animate the alpha from 0 to 1 and translate the
   * views from 10dp to 0dp on the x axis.
   *
  ...
  private Animator buildIconItemEnterAnimator(final ImageView[] buttons) {
    ...
    final float offsetXPx =
        ENTER_STANDARD_ITEM_OFFSET_X_DP * mDpToPx * (rtl ? -1.f : 1.f);
    ...
    for (int i = 0; i < maxViewsToAnimate; i++) {
      ...
      Animator translate =
          ObjectAnimator.ofFloat(view, View.TRANSLATION_X, offsetXPx, 0);
  ...
  private View createMenuItemRow(View convertView, ViewGroup parent,
                                 MenuItem item, int numItems,
                                 int itemViewType) {
    ...
    if (convertView == null ||
      ...
      convertView =
          mInflater.inflate(R.layout.icon_row_menu_item, parent, false);
      ...
      for (int j = numItems; j < 5; j++) {
        ((ViewGroup)convertView)
            .removeView(convertView.findViewById(BUTTON_IDS[j]));
      }
  ...
  private static class RowItemViewHolder {
    ...
    RowItemViewHolder(int numButtons) { buttons = new ImageButton[numButtons]; }
  }
  ...
  @VisibleForTesting
  static int
  getCustomViewTypeCount(@Nullable List<CustomViewBinder> customViewBinders) {
  ...
  static void populateCustomViewBinderOffsetMap(
      @Nullable List<CustomViewBinder> customViewBinders,
      Map<CustomViewBinder, Integer> map, int startingOffset) {
    if (customViewBinders == null)
      return;
