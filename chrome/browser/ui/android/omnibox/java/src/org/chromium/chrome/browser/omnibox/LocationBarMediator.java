...
/**
 * Mediator for the LocationBar component. Intended location for LocationBar
 * business logic; currently, migration of this logic out of LocationBarLayout
 * is in progress.
 */
class LocationBarMediator
  ...
  private final Property<LocationBarMediator, Float>
  ...
  private final Property<LocationBarMediator, Float>
      mWidthChangeFractionPropertyTablet =
          new Property<LocationBarMediator, Float>(Float.class, "") {
            @Override
            public Float get(LocationBarMediator object) {
              return ((LocationBarTablet)mLocationBarLayout)
                  .getWidthChangeFraction();
            }

            @Override
            public void set(LocationBarMediator object, Float value) {
              ((LocationBarTablet)mLocationBarLayout)
  ...
  // Whether the microphone and bookmark buttons should be shown in the tablet
  // location bar. These buttons are hidden if the window size is < 600dp.
  ...
  // Tracks if the location bar is laid out in a focused state due to an ntp
  // scroll.
  private boolean mIsLocationBarFocusedFromNtpScroll;

  /*package */ LocationBarMediator(
      @NonNull Context context, @NonNull LocationBarLayout locationBarLayout,
      ...
      @NonNull WindowAndroid windowAndroid, boolean isTablet,
  ...
  /**
   * Sets coordinators post-construction; they can't be set at construction time
   * since LocationBarMediator is a delegate for them, so is constructed
   * beforehand.
   *
   ...
   */
  /*package */ void
  setCoordinators(UrlBarCoordinator urlCoordinator,
  ...
  /*package */ void onUrlFocusChange(boolean hasFocus) {
    ...
    if (hasFocus) {
      ...
      if (urlBarData != null && urlBarData.editingText != null) {
        setUrlBarText(urlBarData, UrlBar.ScrollType.NO_SCROLL,
    ...
    } // Focus change caused by a closed tab may result in there not being an
      // active tab.
    if (!hasFocus && mLocationBarDataProvider.hasTab()) {
  ...
  /*package */ void onFinishNativeInitialization() {
    ...
    mAssistantVoiceSearchServiceSupplier.set(new AssistantVoiceSearchService(
        mContext, ExternalAuthUtils.getInstance(), templateUrlService,
        GSAState.getInstance(mContext), this,
  ...
  /* package */ void setUrlFocusChangeFraction(float fraction) {
    ...
    if (mIsTablet) {
      mLocationBarDataProvider.getNewTabPageDelegate()
      ...
      if (isLocationBarFocusedFromNtpScroll !=
      ...
      if (fraction > 0f) {
        ...
        // visibility correctly after it completes.  If done here, it would
        // cause the URL to jump due to a badly timed layout call.
  ...
  /* package */ void
  setLensControllerForTesting(LensController lensController) {
    mLensController = lensController;
  }
  ...
  /*package */ void updateVisualsForState() { onPrimaryColorChanged(); }
  ...
  /*package */ void showUrlBarCursorWithoutFocusAnimations() {
    ...
    // This method should only be called on devices with a hardware keyboard
    // attached, as described in the documentation for
    // LocationBar#showUrlBarCursorWithoutFocusAnimations.
  ...
  /*package */ void revertChanges() {
    if (mUrlHasFocus) {
      ...
      if (NativePage.isNativePageUrl(currentUrl,
                                     mLocationBarDataProvider.isIncognito())) {
        setUrlBarTextEmpty();
      } else {
  ...
  /* package */ void onUrlTextChanged() { updateButtonVisibility(); }

  /* package */ void onSuggestionsChanged(String autocompleteText,
                                          boolean defaultMatchIsSearch) {
    // TODO (https://crbug.com/1152501): Refactor the LBM/LBC relationship such
    // that LBM doesn't need to communicate with other coordinators like this.
    ...
    // Handle the case where suggestions (in particular zero suggest) are
    // received without the URL focusing happening.
  ...
  /* package */ void loadUrlWithPostData(String url, int transition,
                                         long inputStart, String postDataType,
                                         byte[] postData) {
    assert mLocationBarDataProvider != null;
    Tab currentTab = mLocationBarDataProvider.getTab();

    // The code of the rest of this class ensures that this can't be called
    // until the native side is initialized
    ...
    if (currentTab != null && (currentTab.isNativePage() ||
      ...
      // Passing in an empty string should not do anything unless the user is at
      // the NTP. Since the NTP has no url, pressing enter while clicking on the
      // URL bar should refresh the page as it does when you click and press
      // enter on any other site.
      if (url.isEmpty())
        url = currentTab.getUrl().getSpec();
    }

    // Loads the |url| in a new tab or the current ContentView and gives focus
    // to the ContentView.
    if (currentTab != null && !url.isEmpty()) {
      LoadUrlParams loadUrlParams = new LoadUrlParams(url);
      loadUrlParams.setVerbatimHeaders(
          GeolocationHeader.getGeoHeader(url, currentTab));
      ...
      if (postData != null && postData.length != 0) {
        loadUrlParams.setPostData(
            ResourceRequestBody.createFromBytes(postData));
      }
   ...
   *
   * <p>The URL is converted to the most user friendly format (removing HTTP://
   * for example).
   *
   * <p>If the current tab is null, the URL text will be cleared.
   */
  /* package */ void setUrl(String currentUrlString, UrlBarData urlBarData) {
    // If the URL is currently focused, do not replace the text they have
    // entered with the URL. Once they stop editing the URL, the current tab's
    // URL will automatically be filled in.
    if (mUrlCoordinator.hasFocus()) {
      if (mUrlFocusedWithoutAnimations &&
          !UrlUtilities.isNTPUrl(currentUrlString)) {
        // If we did not run the focus animations, then the user has not typed
        // any text. So, clear the focus and accept whatever URL the page is
        // currently attempting to display. If the NTP is showing, the current
        // page's URL should not be displayed.
    ...
    setUrlBarText(urlBarData, UrlBar.ScrollType.SCROLL_TO_TLD,
                  SelectionState.SELECT_ALL);
  }
  ...
  /** package */ void lensButtonClicked(View view) {
    if (!mNativeInitialized || mLocationBarDataProvider == null)
      return;
  ...
  /* package */ void setUrlFocusChangeInProgress(boolean inProgress) {
    ...
    if (!inProgress) {
      ...
      // The accessibility bounding box is not properly updated when focusing
      // the Omnibox from the NTP fakebox.  Clearing/re-requesting focus
      // triggers the bounding box to be recalculated.
      if (didFocusUrlFromFakebox() && mUrlHasFocus &&
        ...
        // Existing text (e.g. if the user pasted via the fakebox) from the fake
        // box should be restored after toggling the focus.
  ...
  /**
   * Handles any actions to be performed after all other actions triggered by
   * the URL focus change.  This will be called after any animations are
   * performed to transition from one focus state to the other.
   * @param showExpandedState Whether the url bar is expanded.
   * @param shouldShowKeyboard Whether the keyboard should be shown. This value
   *     is determined by
   *         whether url bar has got focus. Most of the time this is the same as
   *         showExpandedState, but in some cases, e.g. url bar is scrolled to
   * the top of the screen on homepage but not focused, we set it differently.
   */
  /* package */ void finishUrlFocusChange(boolean showExpandedState,
  ...
  /**
   * Handle and run any necessary animations that are triggered off focusing the
   * UrlBar.
  ...
  /* package */ void handleUrlFocusAnimation(boolean hasFocus) {
    ...
    // The focus animation for phones is driven by ToolbarPhone, so we don't
    // currently have any phone-specific animation logic in this class.
    if (mIsTablet) {
      if (mUrlFocusChangeAnimator != null &&
          mUrlFocusChangeAnimator.isRunning()) {
        mUrlFocusChangeAnimator.cancel();
        mUrlFocusChangeAnimator = null;
      }

      if (mLocationBarDataProvider.getNewTabPageDelegate()
              .isCurrentlyVisible()) {
      ...
      float screenSizeRatio = (mRootViewBounds.height() /
                               (float)(Math.max(mRootViewBounds.height(),
                                                mRootViewBounds.width())));
  ...
  /* package */ void
  setShouldShowMicButtonWhenUnfocusedForPhone(boolean shouldShow) {
    assert !mIsTablet;
    mShouldShowMicButtonWhenUnfocused = shouldShow;
  }

  /* package */ void
  setShouldShowLensButtonWhenUnfocusedForPhone(boolean shouldShow) {
    assert !mIsTablet;
    mShouldShowLensButtonWhenUnfocused = shouldShow;
  }

  /* package */ void
  setShouldShowMicButtonWhenUnfocusedForTesting(boolean shouldShow) {
    assert mIsTablet;
    mShouldShowMicButtonWhenUnfocused = shouldShow;
  }

  /**
   * @param shouldShow Whether buttons should be displayed in the URL bar when
   *     it's not
   *                          focused.
   */
  /* package */ void
  setShouldShowButtonsWhenUnfocusedForTablet(boolean shouldShow) {
    assert mIsTablet;
   ...
   * Returns An animator to run for the given view when showing buttons in the
   * unfocused location bar. This should also be used to create animators for
  ...
  /* package */ ObjectAnimator createShowButtonAnimatorForTablet(View button) {
    ...
    ObjectAnimator buttonAnimator =
        ObjectAnimator.ofFloat(button, View.ALPHA, 1.f);
   ...
   * Returns An animator to run for the given view when hiding buttons in the
   * unfocused location bar. This should also be used to create animators for
  ...
  /* package */ ObjectAnimator createHideButtonAnimatorForTablet(View button) {
    assert mIsTablet;
    ObjectAnimator buttonAnimator =
        ObjectAnimator.ofFloat(button, View.ALPHA, 0.f);
  ...
  /**
   * Creates animators for showing buttons in the unfocused tablet location bar.
   * The buttons fade in while the width of the location bar decreases. There
   * are toolbar buttons that show at the same time, causing the width of the
   * location bar to change.
   *
   * @param toolbarStartPaddingDifference The difference in the toolbar's start
   *     padding between
  ...
  /* package */ List<Animator> getShowButtonsWhenUnfocusedAnimatorsForTablet(
    ...
    widthChangeAnimator.addListener(new AnimatorListenerAdapter() {
      ...
      public void onAnimationStart(Animator animation) {
        locationBarTablet.startAnimatingWidthChange(
    ...
    // When buttons show in the unfocused location bar, either the delete button
    // or bookmark button will be showing. If the delete button is currently
    // showing, the bookmark button should not fade in.
    ...
    if (shouldShowSaveOfflineButton()) {
      ...
      // If the microphone button is already fully visible, don't animate its
      // appearance.
      animators.add(createShowButtonAnimatorForTablet(
  ...
  /**
   * Creates animators for hiding buttons in the unfocused tablet location bar.
   * The buttons fade out while the width of the location bar increases. There
   * are toolbar buttons that hide at the same time, causing the width of the
   * location bar to change.
   *
   * @param toolbarStartPaddingDifference The difference in the toolbar's start
   *     padding between
  ...
  /* package */ List<Animator> getHideButtonsWhenUnfocusedAnimatorsForTablet(
    ...
    // When buttons show in the unfocused location bar, either the delete button
    // or bookmark button will be showing. If the delete button is currently
    // showing, the bookmark button should not fade out.
    if (!locationBarTablet.isDeleteButtonVisible()) {
    ...
    if (shouldShowSaveOfflineButton() &&
        locationBarTablet.isSaveOfflineButtonVisible()) {
      animators.add(createHideButtonAnimatorForTablet(
          locationBarTablet.getSaveOfflineButtonForAnimation()));
    } else if (!(mUrlHasFocus && !locationBarTablet.isDeleteButtonVisible())) {
      // If the save offline button isn't enabled, the microphone button always
      // shows when buttons are shown in the unfocused location bar. When
      // buttons are hidden in the unfocused location bar, the microphone shows
      // if the location bar is focused and the delete button isn't showing. The
      // microphone button should not be hidden if the url bar is currently
      // focused and the delete button isn't showing.
      animators.add(createHideButtonAnimatorForTablet(
          locationBarTablet.getMicButtonForAnimation()));
    }
  ...
  /**
   * Changes the text on the url bar.  The text update will be applied
   * regardless of the current focus state (comparing to {@link
   * LocationBarMediator#setUrl} which only applies text updates when not
   * focused).
   *
   * @param urlBarData The contents of the URL bar, both for editing and
   *     displaying.
   * @param scrollType Specifies how the text should be scrolled in the
   *     unfocused state.
   * @param selectionState Specifies how the text should be selected in the
   *     focused state.
   * @return Whether the URL was changed as a result of this call.
   */
  /* package */ boolean setUrlBarText(UrlBarData urlBarData,
                                      @UrlBar.ScrollType int scrollType,
                                      @SelectionState int selectionState) {
    return mUrlCoordinator.setUrlBarData(urlBarData, scrollType,
                                         selectionState);
  }
  ...
  /* package */ boolean setUrlBarTextEmpty() {
    boolean textChanged = mUrlCoordinator.setUrlBarData(
        UrlBarData.EMPTY, UrlBar.ScrollType.SCROLL_TO_BEGINNING,
        SelectionState.SELECT_ALL);
    forceOnTextChanged();
    return textChanged;
  }
  ...
  private void setProfile(Profile profile) {
    if (profile == null || !mNativeInitialized)
      return;
  ...
  /* package */ void updateAssistantVoiceSearchDrawableAndColors() {
    ...
    mLocationBarLayout.setMicButtonTint(
        assistantVoiceSearchService.getButtonColorStateList(
  ...
  /**
   * Update visuals to use a correct color scheme depending on the primary
   * color.
   */
  @VisibleForTesting
  /* package */ void updateOmniboxTheme() {
    // TODO(crbug.com/1114183): Unify light and dark color logic in chrome and
    // make it clear whether the foreground or background color is dark.
    final boolean useDarkForegroundColors =
        !ColorUtils.shouldUseLightForegroundOnBackground(
            getPrimaryBackgroundColor());
    final @OmniboxTheme int omniboxTheme =
    ...
    // If the URL changed colors and is not focused, update the URL to account
    // for the new color scheme.
  ...
  private int getPrimaryBackgroundColor() {
    // If the url bar is focused, the toolbar background color is the default
    // color regardless of whether it is branded or not.
    if (isUrlBarFocused()) {
  ...
  private boolean shouldShowDeleteButton() {
    // Show the delete button at the end when the bar has focus and has some
    // text.
    boolean hasText =
        mUrlCoordinator != null &&
  ...
  private boolean shouldShowLensButton() {
    // When this method is called on UI inflation, return false as the native is
    // not ready.
    if (!mNativeInitialized) {
      return false;
    }
    // When this method is called after native initialized, check omnibox
    // conditions and Lens eligibility.
    if (mIsTablet && mShouldShowButtonsWhenUnfocused) {
    ...
    // This widget must guarantee consistent feature set regardless of search
    // engine choice or other aspects that may not be met by Lens.
  ...
  private boolean shouldShowSaveOfflineButton() {
    ...
    if (tab == null)
      return false;
    // The save offline button should not be shown on native pages. Currently,
    // trying to save an offline page in incognito crashes, so don't show it on
    // incognito either.
  ...
  private boolean shouldShowPageActionButtons() {
    ...
    // There are two actions, bookmark and save offline, and they should be
    // shown if the omnibox isn't focused.
    return !(mUrlHasFocus || mIsUrlFocusChangeInProgress);
  }
  ...
  private void updateOmniboxPrerender() {
    if (mOmniboxPrerender == null)
      return;
    // Profile may be null if switching to a tab that has not yet been
    // initialized.
  ...
  private boolean handleKeyEvent(View view, int keyCode, KeyEvent event) {
    ...
    if (mAutocompleteCoordinator.handleKeyEvent(keyCode, event)) {
      ...
      // Ensures URL bar doesn't lose focus, when RIGHT or LEFT (RTL) key is
      // pressed while the cursor is positioned at the end of the text.
  ...
  private void updateSearchEngineStatusIconShownState() {
    // The search engine icon will be the first visible focused view when it's
    // showing.
    ...
    if (shouldShowSearchEngineLogo && mLocationBarLayout instanceof
                                          LocationBarPhone) {
      ((LocationBarPhone)mLocationBarLayout)
          .setFirstVisibleFocusedView(/* toStatusView= */ true);

      // When the search engine icon is enabled, icons are translations into the
      // parent view's padding area. Set clip padding to false to prevent them
    ...
    mLocationBarLayout.setShowIconsWhenUrlFocused(shouldShowSearchEngineLogo ||
                                                  mIsTablet);
    mStatusCoordinator.setShowIconsWhenUrlFocused(shouldShowSearchEngineLogo ||
                                                  mIsTablet);
  }
  ...
  public void onIncognitoStateChanged() {
    ...
    mUrlCoordinator.setIncognitoColorsEnabled(
        mLocationBarDataProvider.isIncognito());
  }
  ...
  public void setUrlBarFocus(boolean shouldBeFocused,
                             @Nullable String pastedText, int reason) {
    ...
    if (pastedText != null) {
      // This must be happen after requestUrlFocus(), which changes the
      // selection.
      mUrlCoordinator.setUrlBarData(
          UrlBarData.forNonUrlText(pastedText), UrlBar.ScrollType.NO_SCROLL,
          UrlBarCoordinator.SelectionState.SELECT_END);
      forceOnTextChanged();
    }
  }
  ...
  public void performSearchQuery(String query, List<String> searchParams) {
    ...
    String queryUrl = mTemplateUrlServiceSupplier.get().getUrlForSearchQuery(
        query, searchParams);
  ...
  public @Nullable VoiceRecognitionHandler getVoiceRecognitionHandler() {
    // TODO(crbug.com/1140333): StartSurfaceMediator can call this method after
    // destroy().
  ...
  public void clearOmniboxFocus() {
    setUrlBarFocus(/*shouldBeFocused=*/false, /*pastedText=*/null,
  ...
  public void loadUrlFromVoice(String url) {
    ...
    if (url.startsWith("kiwi-extension://")) {
      url = UrlBarData.replaceOnce(url, "kiwi-extension://",
                                   "chrome-extension://");
    }
  ...
  public void setSearchQuery(String query) {
    ...
    // Ensure the UrlBar has focus before entering text. If the UrlBar is not
    // focused, autocomplete text will be updated but the visible text will not.
    setUrlBarFocus(
        /*shouldBeFocused=*/true, /*pastedText=*/null,
  ...
  public void onConfigurationChanged(Configuration newConfig) {
    if (mUrlHasFocus && mUrlFocusedWithoutAnimations &&
      ...
      // If we lose the hardware keyboard and the focus animations were not run,
      // then the user has not typed any text, so we will just clear the focus
      // instead.
      setUrlBarFocus(
          /*shouldBeFocused=*/false, /*pastedText=*/null,
          OmniboxFocusReason.UNFOCUS);
    }
  }
  ...
  public boolean isLensEnabled(@LensEntryPoint int lensEntryPoint) {
    return mLensController.isLensEnabled(
        new LensQueryParams
            .Builder(lensEntryPoint, mLocationBarDataProvider.isIncognito(),
                     mIsTablet)
            .build());
  }
