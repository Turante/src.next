...
class UrlBarMediator implements UrlBar.UrlBarTextContextMenuDelegate,
                                UrlBar.UrlTextChangeListener, TextWatcher {
  ...
  private @SelectionState int mSelectionState =
      UrlBarCoordinator.SelectionState.SELECT_ALL;

  // The numbers for "MobileOmnibox.LongPressPasteAge", the expected time range
  // of time is from 1ms to 1 hour, and 100 buckets.
  ...
  private final List<UrlTextChangeListener> mUrlTextChangeListeners =
      new ArrayList<>();
   ...
   * @param focusChangeCallback The callback that will be notified when focus
   *     changes on the
   *         UrlBar.
   */
  public UrlBarMediator(@NonNull PropertyModel model,
   ...
   * @param selectionState Specifies how the text should be selected when
   *     focused.
   * @return Whether this data differs from the previously passed in values.
   */
  public boolean setUrlBarData(UrlBarData data, @ScrollType int scrollType,
                               @SelectionState int selectionState) {
    if (data != null && data.url != null &&
        data.originEndIndex == data.originStartIndex) {
      scrollType = UrlBar.ScrollType.SCROLL_TO_BEGINNING;
    }

    // Do not scroll to the end of the host for URLs such as data:, javascript:,
    // etc...
    if (data != null && data.url != null &&
  ...
  private void pushTextToModel() {
    if (mUrlBarData == null)
      return;
    CharSequence text = !mHasFocus ? mUrlBarData.displayText
    ...
    UrlBarTextState state =
        new UrlBarTextState(text, textForAutofillServices, scrollType,
  ...
  isNewTextEquivalentToExistingText(UrlBarData existingUrlData,
                                    UrlBarData newUrlData) {
    if (existingUrlData == null)
      return newUrlData == null;
    if (newUrlData == null)
      return false;

    if (!TextUtils.equals(existingUrlData.editingText, newUrlData.editingText))
      return false;

    CharSequence existingCharSequence = existingUrlData.displayText;
    CharSequence newCharSequence = newUrlData.displayText;
    if (existingCharSequence == null)
      return newCharSequence == null;

    // Regardless of focus state, ensure the text content is the same.
    if (!TextUtils.equals(existingCharSequence, newCharSequence))
      return false;

    // If both existing and new text is empty, then treat them equal regardless
    // of their spanned state.
    if (TextUtils.isEmpty(newCharSequence))
      return true;

    // When not focused, compare the emphasis spans applied to the text to
    // determine equality.  Internally, TextView applies many additional spans
    // that need to be ignored for this comparison to be useful, so this is
    // scoped to only the span types applied by our UI.
    if (!(newCharSequence instanceof Spanned) ||
    ...
    UrlEmphasisSpan[] newSpans =
        newText.getSpans(0, newText.length(), UrlEmphasisSpan.class);
    if (currentSpans.length != newSpans.length)
      return false;
    for (int i = 0; i < currentSpans.length; i++) {
      ...
      if (!currentSpan.equals(newSpan) ||
          currentText.getSpanStart(currentSpan) !=
              newText.getSpanStart(newSpan) ||
          currentText.getSpanEnd(currentSpan) != newText.getSpanEnd(newSpan) ||
          currentText.getSpanFlags(currentSpan) !=
              newText.getSpanFlags(newSpan)) {
        return false;
      }
    }
    return true;
  ...
  public boolean setOmniboxTheme(@OmniboxTheme int omniboxTheme) {
    // TODO(bauerb): Make clients observe the property instead of checking the
    // return value.
    @OmniboxTheme
  ...
  public void setIncognitoColorsEnabled(boolean incognitoColorsEnabled) {
    mModel.set(UrlBarProperties.INCOGNITO_COLORS_ENABLED,
               incognitoColorsEnabled);
  }
  ...
  @Override
  public String getReplacementCutCopyText(String currentText,
                                          int selectionStart,
                                          int selectionEnd) {
    if (mUrlBarData == null || mUrlBarData.url == null)
      return null;

    // Replace the cut/copy text only applies if the user selected from the
    // beginning of the display text.
    if (selectionStart != 0)
      return null;

    // Trim to just the currently selected text as that is the only text we are
    // replacing.
    ...
    try {
      ...
      originalUrlLocation =
          getUrlContentsPrePath(mUrlBarData.url, javaUrl.getHost());
    } catch (MalformedURLException mue) {
      // Just keep the existing selected text for cut/copy if unable to parse
      // the URL.
      return null;
    }

    // If we are copying/cutting the full previously formatted URL, reset the
    // URL text before initiating the TextViews handling of the context menu.
    //
    // Example:
    ...
    //
    // As long as the full original text was selected, it will replace that with
    // the original URL and keep any further modifications by the user.
    ...
    return originalUrlLocation +
        currentText.substring(formattedUrlLocation.length());
  }
  ...
  /**
   * Returns the portion of the URL that precedes the path/query section of the
   * URL.
   *
   * @param url The url to be used to find the preceding portion.
   * @param host The host to be located in the URL to determine the location of
   *     the path.
   * @return The URL contents that precede the path (or the passed in URL if the
   *     host is
   *         not found).
   */
