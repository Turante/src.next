...
/**
 * Handles updating the model state for the currently visible omnibox
 * suggestions.
 */
class AutocompleteMediator implements OnSuggestionsReceivedListener,
  ...
  // Delay triggering the omnibox results upon key press to allow the location
  // bar to repaint with the new characters.
  ...
  private final
      @NonNull UrlBarEditingTextStateProvider mUrlBarEditingTextProvider;
  ...
  private final
      @NonNull Supplier<ModalDialogManager> mModalDialogManagerSupplier;
  private final
      @NonNull DropdownItemViewInfoListBuilder mDropdownViewInfoListBuilder;
  private final
      @NonNull DropdownItemViewInfoListManager mDropdownViewInfoListManager;
  ...
  @IntDef({SuggestionVisibilityState.DISALLOWED,
           SuggestionVisibilityState.PENDING_ALLOW,
  ...
  @SuggestionVisibilityState private int mSuggestionVisibilityState;
  ...
  @interface EditSessionState {
    ...
    int ACTIVATED_BY_USER_INPUT =
        1; // The edit session is triggered by user input.
    int ACTIVATED_BY_QUERY_TILE =
        2; // The edit session is triggered from query tile.
  }
  @EditSessionState private int mEditSessionState = EditSessionState.INACTIVE;

  @RefineActionUsage
  private int mRefineActionUsage = RefineActionUsage.NOT_USED;

  // The timestamp (using SystemClock.elapsedRealtime()) at the point when the
  // user started modifying the omnibox with new input.
  private long mNewOmniboxEditSessionTimestamp = -1;
  // Set at the end of the Omnibox interaction to indicate whether the user
  // selected an item from the list (true) or left the Omnibox and suggestions
  // list with no action taken (false).
  private boolean mOmniboxFocusResultedInNavigation;

  /**
   * The text shown in the URL bar (user text + inline autocomplete) after the
   * most recent set of omnibox suggestions was received. When the user presses
   * enter in the omnibox, this value is compared to the URL bar text to
   * determine whether the first suggestion is still valid.
   */
  ...
  public AutocompleteMediator(
      @NonNull Context context, @NonNull AutocompleteDelegate delegate,
    ...
    mSuggestionModels =
        mListPropertyModel.get(SuggestionListProperties.SUGGESTION_MODELS);
    mDropdownViewInfoListBuilder = new DropdownItemViewInfoListBuilder(
        activityTabSupplier, bookmarkState, exploreIconProvider);
    mDropdownViewInfoListBuilder.setShareDelegateSupplier(
        shareDelegateSupplier);
    mDropdownViewInfoListManager =
        new DropdownItemViewInfoListManager(mSuggestionModels);
  }
   ...
   */
  void
  initDefaultProcessors(Callback<List<QueryTile>> queryTileSuggestionCallback) {
  ...
  /**
   * @return DropdownItemViewInfoListBuilder instance used to convert
   *     OmniboxSuggestions to list
   * of ViewInfos.
   */
  ...
  ModelList getSuggestionModelListForTest() { return mSuggestionModels; }
  ...
  /**
   * Retrieve the omnibox suggestion at the specified index.  The index
   * represents the ordering in the underlying model.  The index does not
   * represent visibility due to the current scroll position of the list.
   *
  ...
  void updateVisualsForState(@OmniboxTheme int omniboxTheme) {
    ...
    mListPropertyModel.set(SuggestionListProperties.OMNIBOX_THEME,
                           omniboxTheme);
  }

  /**
   * Show cached zero suggest results.
   * Enables Autocomplete subsystem to offer most recently presented suggestions
   * in the event where Native counterpart is not yet initialized.
   *
   * Note: the only supported page context right now is the
  ...
  /**
   * Notify the mediator that a item selection is pending and should be
   * accepted.
   */
  void allowPendingItemSelection() { mIgnoreOmniboxItemSelection = false; }
  ...
  void onNativeInitialized() {
    ...
    mEnableAdaptiveSuggestionsCount = ChromeFeatureList.isEnabled(
  ...
  /**
   * @see
   *     org.chromium.chrome.browser.omnibox.UrlFocusChangeListener#onUrlFocusChange(boolean)
   */
  void onUrlFocusChange(boolean hasFocus) {
    if (hasFocus) {
      ...
      // Ask directly for zero-suggestions related to current input, unless the
      // user is currently visiting SearchActivity and the input is populated
      // from the launch intent. For SearchActivity, in most cases the input
      // will be empty, triggering the same response (starting zero
      // suggestions), but if the Activity was launched with a QUERY, then the
      // query might point to a different URL than the reported Page, and the
      // suggestion would take the user to the DSE home page.
      // This is tracked by MobileStartup.LaunchCause /
      // EXTERNAL_SEARCH_ACTION_INTENT metric.
      if (mDataProvider.getPageClassification(false) !=
          PageClassification.ANDROID_SEARCH_WIDGET_VALUE) {
        postAutocompleteRequest(this::startZeroSuggest,
      ...
      // Prevent any upcoming omnibox suggestions from showing once a URL is
      // loaded (and as a consequence the omnibox is unfocused).
      hideSuggestions();
    }
  ...
  void onUrlAnimationFinished(boolean hasFocus) {
    setSuggestionVisibilityState(hasFocus
                                     ? SuggestionVisibilityState.ALLOWED
  ...
  /**
   * Whether omnibox autocomplete should currently be prevented from generating
   * suggestions.
   */
   ...
   */
  void
  onVoiceResults(@Nullable List<VoiceRecognitionHandler.VoiceResult> results) {
  ...
  void onQueryTileSelected(QueryTile queryTile) {
    // For last level tile, start a search query, unless we want to let user
    // have a chance to edit the query.
    if (queryTile.children.isEmpty()) {
      launchSearchUrlForQueryTileSuggestion(queryTile);
      return;
    }

    // If the tile has sub-tiles, start a new request to the backend to get the
    // new set of tiles. Also set the tile text in omnibox.
    stopAutocomplete(false);
  ...
  /**
   * Triggered when the user selects one of the omnibox suggestions to navigate
   * to.
   ...
   */
  @Override
  public void onSuggestionClicked(@NonNull AutocompleteMatch suggestion,
                                  int position, @NonNull GURL url) {
    ...
    loadUrlForOmniboxMatch(position, suggestion, url, mLastActionUpTimestamp,
                           true);
  }
  ...
  public void onRefineSuggestion(AutocompleteMatch suggestion) {
    ...
    boolean isZeroPrefix = TextUtils.isEmpty(
        mUrlBarEditingTextProvider.getTextWithoutAutocomplete());
    String refineText = suggestion.getFillIntoEdit();
    if (isSearchSuggestion)
      refineText = TextUtils.concat(refineText, " ").toString();
    ...
    if (isSearchSuggestion) {
      // Note: the logic below toggles assumes individual values to be
      // represented by individual bits. This allows proper reporting of
      // different refine button uses during single interaction with the
      // Omnibox.
      mRefineActionUsage |= isZeroPrefix
  ...
  public void onSwitchToTab(AutocompleteMatch suggestion, int position) {
    ...
    // When invoked directly from a browser, we want to trigger switch to tab
    // animation. If invoked from other activities, ex. searchActivity, we do
    // not need to trigger the animation since Android will show the animation
    // for switching apps.
    if (tab.getWindowAndroid().getActivityState() != ActivityState.STOPPED &&
      ...
      TabModel tabModel =
          mTabWindowManagerSupplier.get().getTabModelForTab(tab);
   ...
   *
   * TODO(crbug.com/1136107): revisit the event propagation here to make sure we
   * do not try to execute an action before native is initialize.
   */
  @Override
  public void onSuggestionLongClicked(@NonNull AutocompleteMatch suggestion,
                                      int position) {
    ...
    // Prevent updates to the shown omnibox suggestions list while the dialog is
    // open.
    stopAutocomplete(false);
    manager.showDialog(mDeleteDialogModel,
  ...
  private void dismissDeleteDialog(@DialogDismissalCause int cause) {
    ...
    assert mModalDialogManagerSupplier.hasValue()
        : "Dialog shown with no registered manager";
    mModalDialogManagerSupplier.get().dismissDialog(mDeleteDialogModel, cause);
  }

  /**
   * Triggered when the user navigates to one of the suggestions without
   * clicking on it.
  ...
  /**
   * Updates the URL we will navigate to from suggestion, if needed. This will
   * update the search URL to be of the corpus type if query in the omnibox is
   * displayed and update aqs= parameter on regular web search URLs.
   *
   ...
   */
  private GURL
  updateSuggestionUrlIfNeeded(@NonNull AutocompleteMatch suggestion,
                              int selectedIndex, @NonNull GURL url,
                              boolean skipCheck) {
    if (!mNativeInitialized || mAutocomplete == null)
      return url;
    ...
    if (!skipCheck) {
      verifiedIndex =
          findSuggestionInAutocompleteResult(suggestion, selectedIndex);
    }

    // If we do not have the suggestion as part of our results, skip the URL
    // update.
    if (verifiedIndex == SUGGESTION_NOT_FOUND)
      return url;

    // TODO(mariakhomenko): Ideally we want to update match destination URL with
    // new aqs for query in the omnibox and voice suggestions, but it's
  ...
  /**
   * Check if the supplied suggestion is still in the current model and return
   * its index.
   *
   * This call should be used to confirm that model has not been changed ahead
   * of an event being called by all the methods that are dispatched rather than
   * called directly.
   *
   * @param suggestion Suggestion to look for.
   * @param index Last known position of the suggestion.
   * @return Current index of the supplied suggestion, or SUGGESTION_NOT_FOUND
   *     if it is no longer
   *         part of the model.
   */
  @SuppressWarnings("ReferenceEquality")
  private int findSuggestionInAutocompleteResult(AutocompleteMatch suggestion,
                                                 int position) {
    if (getSuggestionCount() > position &&
        getSuggestionAt(position) == suggestion) {
      return position;
    }
  ...
  /**
   * Notifies the autocomplete system that the text has changed that drives
   * autocomplete and the autocomplete suggestions should be updated.
   */
  public void onTextChanged(String textWithoutAutocomplete,
    ...
    if (TextUtils.isEmpty(textWithoutAutocomplete)) {
      ...
      // There may be no tabs when searching form omnibox in overview mode. In
      // that case, LocationBarDataProvider.getCurrentUrl() returns NTP url.
      if (mDataProvider.hasTab() ||
          mDataProvider.isInOverviewAndShowingOmnibox()) {
        boolean preventAutocomplete =
            !mUrlBarEditingTextProvider.shouldAutocomplete();
        int cursorPosition =
            mUrlBarEditingTextProvider.getSelectionStart() ==
        ...
        postAutocompleteRequest(() -> {
          mAutocomplete.start(currentUrl, pageClassification,
                              textWithoutAutocomplete, cursorPosition,
                              preventAutocomplete, null,
  ...
  @Override
  public void onSuggestionsReceived(AutocompleteResult autocompleteResult,
                                    String inlineAutocompleteText) {
    if (mShouldPreventOmniboxAutocomplete ||
        getSuggestionVisibilityState() ==
            SuggestionVisibilityState.DISALLOWED) {
      return;
    }
    ...
    final List<AutocompleteMatch> newSuggestions =
        autocompleteResult.getSuggestionsList();
  ...
  void loadTypedOmniboxText(long eventTime) {
    ...
    if (urlText.startsWith("chrome-extension://")) {
      urlText = UrlBarData.replaceOnce(urlText, "chrome-extension://",
                                       "kiwi-extension://");
    }
  ...
  private void findMatchAndLoadUrl(String urlText, long inputStart) {
    ...
    if (getSuggestionCount() > 0 &&
      ...
      // Common case: the user typed something, received suggestions, then
      // pressed enter. This triggers the Default Match.
      suggestionMatch = getSuggestionAt(0);
    } else {
      // Less common case: there are no valid omnibox suggestions. This can
      // happen if the user tapped the URL bar to dismiss the suggestions, then
      // pressed enter. This can also happen if the user presses enter before
      // any suggestions have been received from the autocomplete controller.
      if (!mNativeInitialized || mAutocomplete == null)
        return;
      suggestionMatch =
          mAutocomplete.classify(urlText, mDelegate.didFocusUrlFromFakebox());
    ...
    loadUrlForOmniboxMatch(0, suggestionMatch, suggestionMatch.getUrl(),
                           inputStart, inSuggestionList);
  }
   ...
   * @param inVisibleSuggestionList Whether the suggestion is in the visible
   *     suggestion list.
   */
  private void loadUrlForOmniboxMatch(int matchPosition,
                                      @NonNull AutocompleteMatch suggestion,
                                      @NonNull GURL url, long inputStart,
                                      boolean inVisibleSuggestionList) {
    SuggestionsMetrics.recordFocusToOpenTime(System.currentTimeMillis() -
                                             mUrlFocusTime);

    // Clear the deferred site load action in case it executes. Reclaims a bit
    // of memory.
    mDeferredLoadAction = null;

    mOmniboxFocusResultedInNavigation = true;
    url = updateSuggestionUrlIfNeeded(suggestion, matchPosition, url,
                                      !inVisibleSuggestionList);
    ...
    if (((transition & PageTransition.CORE_MASK) == PageTransition.TYPED) &&
      ...
      // retyping it) as reloads too.  We exclude non-TYPED transitions because
      // if the transition is GENERATED, the user input something that looked
      ...
      // It's important to use the page transition from the suggestion or we
      // might end up saving generated URLs as typed URLs, which would then
      // pollute the subsequent omnibox results. There is one special case where
      // the suggestion text was pasted, where we want the transition type to be
      // LINK.
    ...
    if (suggestion.getDisplayText().startsWith("!")) {
      newUrl = "https://www.duckduckgo.com/?q=" +
               URLEncoder.encode(suggestion.getDisplayText());
    }
   ...
   * This method should not be called directly. Schedule execution using
   * postAutocompleteRequest.
   */
  private void startZeroSuggest() {
    // Reset "edited" state in the omnibox if zero suggest is triggered -- new
    // edits now count as a new session.
    mEditSessionState = EditSessionState.INACTIVE;
    mNewOmniboxEditSessionTimestamp = -1;
    ...
    if (mDelegate.isUrlBarFocused() &&
      ...
      mAutocomplete.startZeroSuggest(
          mUrlBarEditingTextProvider.getTextWithAutocomplete(),
          mDataProvider.getCurrentUrl(), pageClassification,
  ...
  /**
   * Signals the autocomplete controller to stop generating omnibox suggestions
   * and cancels the queued task to start the autocomplete controller, if any.
   *
  ...
  void startAutocompleteForQuery(String query) {
    ...
    if (mDataProvider.hasTab()) {
      mAutocomplete.start(mDataProvider.getCurrentUrl(),
                          mDataProvider.getPageClassification(false), query, -1,
                          false, null, false);
    }
  }

  /**
   * Respond to Suggestion list height change and update list of presented
   * suggestions.
   *
  ...
  public void onSuggestionDropdownHeightChanged(@Px int newHeight) {
    // Report the dropdown height whenever we intend to - or do show soft
    // keyboard. This addresses cases where hardware keyboard is attached to a
    // device, or where user explicitly called the keyboard back after we hid
    // it.
    if (mDelegate.isKeyboardActive()) {
      mDropdownViewInfoListBuilder.setDropdownHeightWithKeyboardActive(
          newHeight);
    }
  }
  ...
  /**
   * Called whenever a navigation happens from the omnibox to record metrics
   * about the user's interaction with the omnibox.
   *
   * @param matchPosition The index of the suggestion that was selected.
   * @param disposition The window open disposition.
   * @param suggestion The suggestion selected.
   */
  private void recordMetrics(int matchPosition, int disposition,
                             AutocompleteMatch suggestion) {
    SuggestionsMetrics.recordUsedSuggestionFromCache(
        mAutocompleteResult.isFromCachedResult());

    // Do not attempt to record other metrics for cached suggestions if the
    // source of the list is local cache. These suggestions do not have
    // corresponding native objects and will fail validation.
    if (mAutocompleteResult.isFromCachedResult())
      return;
    ...
    mAutocomplete.onSuggestionSelected(
        matchPosition, disposition, suggestion.getType(), currentPageUrl,
        pageClassification, elapsedTimeSinceModified, autocompleteLength,
        webContents);
  }
  ...
  /**
   * @return elapsed time (in milliseconds) since last input or -1 if user has
   *     chosen
  ...
  private void launchSearchUrlForQueryTileSuggestion(QueryTile queryTile) {
    ...
    GURL updatedUrl =
        mAutocomplete.updateMatchDestinationUrlWithQueryFormulationTime(
            position, getElapsedTimeSinceInputChange(), queryTile.queryText,
    ...
    // RecordMetrics has to be called before loadUrl, or otherwise the native
    // AutocompleteResult object will be reset and the suggestion will fail
    // validation.
    recordMetrics(position, WindowOpenDisposition.CURRENT_TAB, suggestion);
    mDelegate.loadUrl(updatedUrl.getSpec(), PageTransition.LINK,
   ...
   * Each Autocomplete action posted here will cancel any previously posted
   * Autocomplete action, ensuring that the actions don't compete against each
   * other. Any action scheduled for execution before Native libraries are ready
   * will be deferred.
   *
   * This call should only be used for regular suggest flows. Do not post
   * arbitrary tasks here.
   *
   * @param action Autocomplete action to execute.
   * @param delayMillis The number of milliseconds by which the action should be
   *     delayed.
   *         Use SCHEDULE_FOR_IMMEDIATE_EXECUTION to post action at front of the
   * message queue.
   */
  private void postAutocompleteRequest(@NonNull Runnable action,
                                       long delayMillis) {
    cancelAutocompleteRequests();
    mCurrentAutocompleteRequest = new Runnable() {
      ...
      public void run() {
        action.run();
        // Catch any AutocompleteRequests that post subsequent
        // AutocompleteRequest. Note: we have to explicitly instantiate a
        // Runnable class, otherwise 'this' will resolve into a parent class and
        // Runnable.this won't work. Release completed Runnable.
        mCurrentAutocompleteRequest = null;
      }
    };
    // In the event we got Native Ready signal but no Profile yet (or the other
    // way around), delay execution of the Autocomplete request.
    if (!mNativeInitialized || mAutocomplete == null)
      return;
    if (delayMillis == SCHEDULE_FOR_IMMEDIATE_EXECUTION) {
      // TODO(crbug.com/1174855): Replace the following with
      // postAtFrontOfQueue() and correct any tests that expect data instantly.
  ...
  /**
   * Execute any pending Autocomplete requests, if the Autocomplete subsystem is
   * ready.
   */
