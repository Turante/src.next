...
/**
 * This class controls the interaction of the "edit url" suggestion item with
 * the rest of the suggestions list. This class also serves as a mediator,
 * containing logic that interacts with the rest of Chrome.
 */
public class EditUrlSuggestionProcessor extends BaseSuggestionViewProcessor {
  private final Context mContext;

  /**
   * The delegate for accessing the location bar for observation and
   * modification.
   */
  ...
  public EditUrlSuggestionProcessor(
      Context context, SuggestionHost suggestionHost,
      UrlBarDelegate locationBarDelegate,
      Supplier<LargeIconBridge> iconBridgeSupplier, Supplier<Tab> tabSupplier,
  ...
  @Override
  public boolean doesProcessSuggestion(AutocompleteMatch suggestion,
                                       int position) {
    // The what-you-typed suggestion can potentially appear as the second
    // suggestion in some cases. If the first suggestion isn't the one we want,
    // ignore all subsequent suggestions.
    if (position != 0)
      return false;

    Tab activeTab = mTabSupplier.get();
    if (activeTab == null || !activeTab.isInitialized() ||
        activeTab.isNativePage() || SadTab.isShowing(activeTab)) {
      return false;
    }
    ...
    if (!mHasClearedOmniboxForFocus) {
      ...
      if (ContextUtils.getAppSharedPreferences().getBoolean(
              "keep_address_bar_content", false) &&
          !activeTab.getUrl().getSpec().startsWith("chrome-search://")) {
        UrlBarData parsedToolbar = UrlBarData.forUrlAndText(
            suggestion.getUrl().getSpec(), suggestion.getUrl().getSpec(), null);
        mUrlBarDelegate.setOmniboxEditingText(
            parsedToolbar.getEditingOrDisplayText().toString());
      } else {
        mUrlBarDelegate.setOmniboxEditingText("");
      }
    }
  ...
  @Override
  public void populateModel(AutocompleteMatch suggestion, PropertyModel model,
                            int position) {
    ...
    setSuggestionDrawableState(
        model, SuggestionDrawableState.Builder
    ...
    setCustomActions(
        ...
        Arrays.asList(
            ...
            // TODO(https://crbug.com/1090187): do not re-use bookmark_item_edit
            // here.
            new Action(mContext,
                       SuggestionDrawableState.Builder
                           .forDrawableRes(getContext(),
                                           R.drawable.bookmark_edit_active)
                           .setLarge(true)
                           .setAllowTint(true)
                           .build(),
  ...
  @Override
  protected void onSuggestionClicked(AutocompleteMatch suggestion,
                                     int position) {
  ...
  private void onShareLink() {
    ...
    // TODO(mdjones): This should only share the displayed URL instead of the
    // background tab.
    mShareDelegateSupplier.get().share(mTabSupplier.get(), false,
  ...
  private void onCopyLink(AutocompleteMatch suggestion) {
    ...
    HistoryClustersTabHelper.onCurrentTabUrlCopied(
