...
public class TopToolbarCoordinator implements Toolbar {
  ...
  public interface UrlExpansionObserver {
     ...
     *
     * @param fraction The toolbar expansion progress. 0 indicates that the URL
     *     bar is not
     *                   expanded. 1 indicates that the URL bar is expanded to
     * the maximum width.
     */
  ...
  /**
   * The coordinator for the tab switcher mode toolbar (phones only). This will
   * be lazily created after ToolbarLayout is inflated.
   */
  private @Nullable TabSwitcherModeTTCoordinatorPhone
      mTabSwitcherModeCoordinatorPhone;
  /**
   * The coordinator for the start surface mode toolbar (phones only) if the
   * StartSurface is enabled. This will be lazily created after ToolbarLayout is
   * inflated.
   */
  private
      @Nullable StartSurfaceToolbarCoordinator mStartSurfaceToolbarCoordinator;
   ...
   * @param controlContainer The {@link ToolbarControlContainer} for the
   *     containing activity.
   * @param toolbarLayout The {@link ToolbarLayout}.
   * @param userEducationHelper Helper class for showing in-product help text
   *     bubbles.
   * @param buttonDataProviders List of classes that wish to display an optional
   *     button in the
   *         browsing mode toolbar.
   * @param layoutStateProviderSupplier Supplier of the {@link
   *     LayoutStateProvider}.
   * @param normalThemeColorProvider The {@link ThemeColorProvider} for normal
   *     mode.
   * @param overviewThemeColorProvider The {@link ThemeColorProvider} for
   *     overview mode.
   * @param tabModelSelectorSupplier Supplier of the {@link TabModelSelector}.
   * @param homepageEnabledSupplier Supplier of whether Home button is enabled.
   * @param startSurfaceAsHomepageSupplier Supplier of whether start surface
   *     should be shown as
   *         homepage.
   * @param homepageManagedByPolicySupplier Supplier of whether the homepage is
   *     managed by policy.
   * @param identityDiscStateSupplier Supplier of the state change of identity
   *     disc button.
   * @param invalidatorCallback Callback that will be invoked  when the toolbar
   *     attempts to
   *        invalidate the drawing surface.  This will give the object that
   * registers as the host for the {@link Invalidator} a chance to defer the
   * actual invalidate to sync drawing.
   * @param identityDiscButtonSupplier Supplier of Identity Disc button.
   * @param resourceManagerSupplier A supplier of a resource manager for native
   *     textures.
   * @param isProgressBarVisibleSupplier A supplier of whether the progress bar
   *     is visible.
   * @param isInconitoModeEnabledSupplier A supplier of the incognito mode being
   *     enabled or not.
   * @param isGridTabSwitcherEnabled Whether grid tab switcher is enabled via a
   *     feature flag.
   * @param isTabToGtsAnimationEnabled Whether Tab-to-GTS animation is enabled
   *     via a feature flag.
   * @param isStartSurfaceEnabled Whether start surface is enabled via a feature
   *     flag.
   * @param isTabGroupsAndroidContinuationEnabled Whether flag
   *     TabGroupsContinuationAndroid is
   *         enabled.
   * @param initializeWithIncognitoColors Whether the toolbar should be
   *     initialized with incognito
   *         colors.
   * @param shouldHideToolbarLayoutOnStart Whether to hide toolbar layout on
   *     startup.
   */
  public TopToolbarCoordinator(
    ...
    mOptionalButtonController = new OptionalBrowsingModeButtonController(
        buttonDataProviders, userEducationHelper, mToolbarLayout,
    ...
    if (mToolbarLayout instanceof ToolbarPhone) {
      if (isStartSurfaceEnabled) {
        if (shouldHideToolbarLayoutOnStart)
        ...
        mStartSurfaceToolbarCoordinator = new StartSurfaceToolbarCoordinator(
            ...
            userEducationHelper, identityDiscStateSupplier,
        ...
        mTabSwitcherModeCoordinatorPhone =
            new TabSwitcherModeTTCoordinatorPhone(
                controlContainer.getRootView().findViewById(
                    R.id.tab_switcher_toolbar_stub),
    ...
    mToolbarLayout.initialize(
        toolbarDataProvider, tabController, mMenuButtonCoordinator,
        isProgressBarVisibleSupplier, historyDelegate,
    ...
    new OneShotCallback<>(mAppMenuButtonHelperSupplier,
                          this::setAppMenuButtonHelper);
    homepageEnabledSupplier.addObserver(
        (show) -> mToolbarLayout.onHomeButtonUpdate(show));
    mToolbarLayout.setInvalidatorCallback(invalidatorCallback);
  }

  /**
   * @param appMenuButtonHelper The helper for managing menu button
   *     interactions.
   */
  ...
  /**
   * Initialize the coordinator with the components that have native
   * initialization dependencies. <p> Calling this must occur after the native
   * library have completely loaded.
   *
   * @param layoutUpdater A {@link Runnable} used to request layout update upon
   *     scene change.
   * @param tabSwitcherClickHandler The click handler for the tab switcher
   *     button.
   * @param tabSwitcherLongClickHandler The long click handler for the tab
   *     switcher button.
   ...
   * @param customTabsBackClickHandler The click handler for the custom tabs
   *     back button.
   * @param layoutManager A {@link LayoutManager} used to watch for scene
   *     changes.
   * @param tabSupplier Supplier of the activity tab.
   * @param browserControlsStateProvider {@link BrowserControlsStateProvider} to
   *     access browser
   *                                     controls offsets.
   * @param topUiThemeColorProvider {@link ThemeColorProvider} for top UI.
   */
  public void initializeWithNative(
      Runnable layoutUpdater, OnClickListener tabSwitcherClickHandler,
      OnLongClickListener tabSwitcherLongClickHandler,
      OnClickListener newTabClickHandler, OnClickListener bookmarkClickHandler,
      OnClickListener customTabsBackClickHandler, LayoutManager layoutManager,
      ObservableSupplier<Tab> tabSupplier,
      BrowserControlsStateProvider browserControlsStateProvider,
      TopUiThemeColorProvider topUiThemeColorProvider) {
    assert mTabModelSelectorSupplier.get() != null;
    if (mTabSwitcherModeCoordinatorPhone != null) {
      mTabSwitcherModeCoordinatorPhone.setOnTabSwitcherClickHandler(
          tabSwitcherClickHandler);
      mTabSwitcherModeCoordinatorPhone.setOnNewTabClickHandler(
          newTabClickHandler);
      mTabSwitcherModeCoordinatorPhone.setTabModelSelector(
          mTabModelSelectorSupplier.get());
    } else if (mStartSurfaceToolbarCoordinator != null) {
      mStartSurfaceToolbarCoordinator.setOnNewTabClickHandler(
          newTabClickHandler);
      mStartSurfaceToolbarCoordinator.setTabModelSelector(
          mTabModelSelectorSupplier.get());
      mStartSurfaceToolbarCoordinator.setTabSwitcherListener(
          tabSwitcherClickHandler);
    ...
    // If fullscreen is disabled, don't bother creating this overlay; only the
    // android view will ever be shown.
    if (DeviceClassManager.enableFullscreen()) {
      mOverlayCoordinator = new TopToolbarOverlayCoordinator(
          mToolbarLayout.getContext(), layoutManager,
          mControlContainer::getProgressBarDrawingInfo, tabSupplier,
          browserControlsStateProvider, mResourceManagerSupplier,
          topUiThemeColorProvider, mIsGridTabSwitcherEnabled);
  ...
  /**
   * @param urlExpansionObserver The observer that observes URL expansion
   *     progress change.
   */
  public void
  addUrlExpansionObserver(UrlExpansionObserver urlExpansionObserver) {
   ...
   */
  public void
  removeUrlExpansionObserver(UrlExpansionObserver urlExpansionObserver) {
  ...
  @Override
  public void getPositionRelativeToContainer(View containerView,
                                             int[] position) {
    mToolbarLayout.getPositionRelativeToContainer(containerView, position);
  }

  /**
   * Sets the {@link Invalidator} that will be called when the toolbar attempts
   * to invalidate the drawing surface.  This will give the object that
   * registers as the host for the
   * {@link Invalidator} a chance to defer the actual invalidate to sync
   * drawing.
  ...
  /**
   * @return The name of the publisher of the content if it can be reliably
   *     extracted, or null
   *         otherwise.
   */
   ...
   * back button.
   * @param canGoBack Whether or not the current tab has any history to go back
   *     to.
   */
   ...
   * Gives inheriting classes the chance to update the visibility of the
   * forward button.
   * @param canGoForward Whether or not the current tab has any history to go
   *     forward to.
   */
   ...
   * @param editingAllowed Whether or not bookmarks can be modified (added,
   *     edited, or removed).
   */
  ...
  /**
   * Gives inheriting classes the chance to respond to accessibility state
   * changes.
  ...
  /**
   * Gives inheriting classes the chance to do the necessary UI operations after
   * Chrome is restored to a previously saved state.
   */
  public void onStateRestored() { mToolbarLayout.onStateRestored(); }

  /**
   * Triggered when the current tab or model has changed.
   * <p>
   * As there are cases where you can select a model with no tabs (i.e. having
   * incognito tabs but no normal tabs will still allow you to select the normal
   * model), this should not guarantee that the model's current tab is non-null.
   */
  public void onTabOrModelChanged() { mToolbarLayout.onTabOrModelChanged(); }

  /**
   * For extending classes to override and carry out the changes related with
   * the primary color for the current tab changing.
   */
  ...
  /**
   * Sets the icon drawable that the close button in the toolbar (if any) should
   * show, or hides it if {@code drawable} is {@code null}.
   */
   ...
   * @param listener The {@link View.OnClickListener} to use for clicks to the
   *     button.
   */
  ...
  /**
   * Updates the visual appearance of a custom action button in the toolbar
   * layout, if it is supported.
   * @param index The index of the button.
   * @param drawable The icon for the button.
   * @param description The content description for the button.
   */
  public void updateCustomActionButton(int index, Drawable drawable,
                                       String description) {
  ...
  /**
   * @param attached Whether or not the web content is attached to the view
   *     heirarchy.
   */
  ...
  /**
   * Gives inheriting classes the chance to show or hide the TabSwitcher mode of
   * this toolbar.
   * @param inTabSwitcherMode Whether or not TabSwitcher mode should be shown or
   *     hidden.
   * @param showToolbar    Whether or not to show the normal toolbar while
   *     animating.
   * @param delayAnimation Whether or not to delay the animation until after the
   *     transition has
  ...
  public void setTabSwitcherMode(boolean inTabSwitcherMode, boolean showToolbar,
    ...
    mToolbarLayout.setTabSwitcherMode(inTabSwitcherMode, showToolbar,
                                      delayAnimation, mMenuButtonCoordinator);
    if (mTabSwitcherModeCoordinatorPhone != null) {
  ...
  /**
   * Gives inheriting classes the chance to update their state when the
   * TabSwitcher transition has finished.
   */
   ...
   * @param tabCountProvider The {@link TabCountProvider} subclasses can
   *     observe.
   */
  ...
  /**
   * Gives inheriting classes the chance to update themselves based on default
  ...
  /**
   * Returns the elapsed realtime in ms of the time at which first draw for the
   * toolbar occurred.
   */
  public long getFirstDrawTime() { return mToolbarLayout.getFirstDrawTime(); }
   ...
   */
  public void finishAnimations() { mToolbarLayout.finishAnimations(); }
   ...
   * @param requestToShow Whether or not request showing the start surface
   *     toolbar.
   * @param toolbarHeight The height of start surface toolbar.
   */
  public void updateStartSurfaceToolbarState(@StartSurfaceState int newState,
    ...
    mStartSurfaceToolbarCoordinator.onStartSurfaceStateChanged(newState,
                                                               requestToShow);
  ...
  public void onStartSurfaceHeaderOffsetChanged(int verticalOffset,
                                                int toolbarHeight) {
    if (mStartSurfaceToolbarCoordinator != null) {
      mStartSurfaceToolbarCoordinator.onStartSurfaceHeaderOffsetChanged(
          verticalOffset);
      updateToolbarLayoutVisibility(toolbarHeight);
    }
  }
  ...
  /**
   * This method should be called when there is a possibility that logo became
   * available or was changed.
  ...
  public void onLogoAvailable(Bitmap logoImage, String contentDescription) {
    if (mStartSurfaceToolbarCoordinator != null) {
      mStartSurfaceToolbarCoordinator.onLogoImageAvailable(logoImage,
                                                           contentDescription);
    }
  }
  ...
  @VisibleForTesting
  public OptionalBrowsingModeButtonController
  getOptionalButtonControllerForTesting() {
