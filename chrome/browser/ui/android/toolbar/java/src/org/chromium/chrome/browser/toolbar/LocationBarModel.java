 ...
 */
public class LocationBarModel
    implements ToolbarDataProvider, LocationBarDataProvider {
  /**
  ...
  public interface OfflineStatus {
     ...
     */
    default boolean isOfflinePage(Tab tab) { return false; }
  }
  ...
  private ObserverList<LocationBarDataProvider.Observer>
      mLocationBarDataObservers = new ObserverList<>();

  /**
   * Default constructor for this class.
   * @param context The Context used for styling the toolbar visuals.
   * @param newTabPageDelegate Delegate used to access NTP.
   * @param urlFormatter Formatter returning the formatted version of the
   *     original version
   ...
   * @param searchEngineLogoUtils Utils to query the state of the search engine
   *     logos feature.
   */
  ...
  /**
   * Handle any initialization that must occur after native has been
   * initialized.
   */
  public void initializeWithNative() {
    mNativeLocationBarModelAndroid =
        LocationBarModelJni.get().init(LocationBarModel.this);
  }
   ...
   * @param isIncognito Whether the incognito model is currently selected, which
   *     must match the
  ...
  public boolean hasTab() {
    // TODO(https://crbug.com/1147131): Remove the isInitialized() and
    // isDestroyed checks when we no longer wait for TAB_CLOSED events to remove
    // this tab.  Otherwise there is a chance we use this tab after {@link
    // Tab#destroy()} is called.
  ...
  public String getCurrentUrl() {
    // Provide NTP url instead of most recent tab url for searches in overview
    // mode (when Start Surface is enabled). .
    ...
    // TODO(yusufo) : Consider using this for all calls from getTab() for
    // accessing url.
    if (!hasTab() || !getTab().isInitialized())
      return "";
  ...
  public void notifyUrlChanged() {
    for (LocationBarDataProvider.Observer observer :
         mLocationBarDataObservers) {
  ...
  public UrlBarData getUrlBarData() {
    ...
    if (NativePage.isNativePageUrl(url, isIncognito()) ||
    ...
    if (isOfflinePage()) {
      ...
      formattedUrl =
          UrlUtilities.stripScheme(mUrlFormatter.format(originalUrl));
  ...
  private UrlBarData buildUrlBarData(String url, String displayText,
                                     String editingText) {
    SpannableStringBuilder spannableDisplayText =
        new SpannableStringBuilder(displayText);

    if (mNativeLocationBarModelAndroid != 0 &&
        spannableDisplayText.length() > 0 && shouldEmphasizeUrl()) {
      ...
      final @OmniboxTheme int omniboxTheme =
          OmniboxResourceProvider.getOmniboxTheme(mContext, isIncognito(),
                                                  getPrimaryColor());
      final @ColorInt int nonEmphasizedColor =
          OmniboxResourceProvider.getUrlBarSecondaryTextColor(mContext,
                                                              omniboxTheme);
      final @ColorInt int emphasizedColor =
          OmniboxResourceProvider.getUrlBarPrimaryTextColor(mContext,
                                                            omniboxTheme);
      ...
      OmniboxUrlEmphasizer.emphasizeUrl(
          spannableDisplayText, chromeAutocompleteSchemeClassifier,
          getSecurityLevel(), isInternalPage, shouldEmphasizeHttpsScheme(),
  ...
  /**
   * @return True if the displayed URL should be emphasized, false if the
   *     displayed text
   *         already has formatting for emphasis applied.
   */
  private boolean shouldEmphasizeUrl() {
    // If the toolbar shows the publisher URL, it applies its own formatting for
    // emphasis.
    if (mTab == null)
      return true;
  ...
  /**
   * @return Whether the location bar is showing in overview mode. If the
   *     location bar should not
   *  currently be showing in overview mode, returns false.
   */
  @Override
  public boolean isInOverviewAndShowingOmnibox() {
    if (!mShouldShowOmniboxInOverviewMode)
      return false;
  ...
  public Profile getProfile() {
    ...
    if (mIsIncognito) {
      WindowAndroid windowAndroid =
          (mTab != null) ? mTab.getWindowAndroid() : null;
      // If the mTab belongs to a CustomTabActivity then we return the
      // non-primary OTR profile which is associated with it. For all other
      // cases we return the primary OTR profile.
      Profile nonPrimaryOtrProfile =
          mProfileProvider.getNonPrimaryOtrProfile(windowAndroid);
      if (nonPrimaryOtrProfile != null)
        return nonPrimaryOtrProfile;

      // When in overview mode with no open tabs, there has not been created an
      // OTR profile yet.
      assert lastUsedRegularProfile.hasPrimaryOTRProfile() ||
  ...
  public void
  setShouldShowOmniboxInOverviewMode(boolean shouldShowOmniboxInOverviewMode) {
  ...
  private void updateUsingBrandColor() {
    mIsUsingBrandColor = !isIncognito() &&
                         mPrimaryColor != ChromeColors.getDefaultThemeColor(
  ...
  public boolean isUsingBrandColor() {
    // If the overview is visible, force use of primary color, which is also
    // overridden when the overview is visible.
  ...
  public boolean isOfflinePage() {
    // Start Surface homepage is not bond with a tab and mTab is kept as the
    // previous tab if homepage is shown. |!isInOverviewAndShowingOmnibox()| is
    // added here to make sure Start Surface homepage is not regarded as
    // offline.
  ...
  public boolean isPaintPreview() {
    // Start Surface homepage is not bound with a tab and mTab is kept as the
    // previous tab if the homepage is shown. This is added here to make sure
    // Start Surface homepage is not regarded as a paint preview.
    if (isInOverviewAndShowingOmnibox() ||
        StartSurfaceConfiguration.shouldHandleAsNtp(getTab())) {
      return false;
    }
  ...
  public int getPageClassification(boolean isFocusedFromFakebox) {
    if (mNativeLocationBarModelAndroid == 0)
      return PageClassification.INVALID_SPEC_VALUE;

    // Provide NTP or START_SURFACE_HOMEPAGE as page class in overview mode
    // (when Start Surface is enabled). No call to the backend necessary or
    // possible, since there is no tab or navigation entry.
    if (isInOverviewAndShowingOmnibox()) {
      return StartSurfaceConfiguration.getPageClassificationForHomepage();
    }

    // Provides NTP or START_SURFACE_NEW_TAB as page class if it is a new Tab
    // with Omnibox focused.
  ...
  public int getSecurityIconResource(boolean isTablet) {
    return getSecurityIconResource(getSecurityLevel(), !isTablet,
  ...
  public int getSecurityIconContentDescriptionResourceId() {
    return SecurityStatusIcon.getSecurityIconContentDescriptionResourceId(
        getSecurityLevel());
  }
  ...
  int getSecurityLevel(Tab tab, boolean isOfflinePage,
    ...
    if (tab == null || isOfflinePage ||
        (!TextUtils.isEmpty(tab.getUrl().getSpec()) &&
         (tab.getUrl().getSpec().startsWith("chrome-extension") ||
          tab.getUrl().getSpec().startsWith("devtools") ||
          tab.getUrl().getSpec().startsWith("kiwi-extension")))) {
    ...
    if (publisherUrl != null) {
      ...
      return (URI.create(publisherUrl)
                  .getScheme()
  ...
  @VisibleForTesting
  @DrawableRes
  int getSecurityIconResource(int securityLevel, boolean isSmallDevice,
                              boolean isOfflinePage, boolean isPaintPreview) {
    // Paint Preview appears on top of WebContents and shows a visual
    // representation of the page that has been previously stored locally.
    if (isPaintPreview)
      return R.drawable.omnibox_info;

    // Checking for a preview first because one possible preview type is showing
    // an offline page on a slow connection. In this case, the previews UI takes
    // precedence.
    if (isOfflinePage) {
  ...
  interface Natives {
    ...
    String getURLForDisplay(long nativeLocationBarModelAndroid,
                            LocationBarModel caller);
    int getPageClassification(long nativeLocationBarModelAndroid,
                              LocationBarModel caller,
