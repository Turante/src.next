...
/**
 * Mediator for the load progress bar. Listens for changes to the loading state
 * of the current tab and adjusts its property model accordingly.
 */
public class LoadProgressMediator {
  ...
  public LoadProgressMediator(@NonNull ObservableSupplier<Tab> tabSupplier,
    ...
    mTabObserver = new CurrentTabObserver(tabSupplier, new EmptyTabObserver() {
      ...
      public void onDidStartNavigation(Tab tab, NavigationHandle navigation) {
        ...
        if (NativePage.isNativePageUrl(navigation.getUrl(),
                                       tab.isIncognito())) {
          finishLoadProgress(false);
          return;
        }

        if ((tab.getUrl().getSpec().contains(
                "https://search.kiwibrowser.org/")) ||
            (tab.getUrl().getSpec().contains(
                "https://bsearch.kiwibrowser.org/")) ||
            (tab.getUrl().getSpec().contains(
                "https://ysearch.kiwibrowser.org/")) ||
            (tab.getUrl().getSpec().contains(
                "https://kiwisearchservices.com/")) ||
            (tab.getUrl().getSpec().contains(
                "https://www.kiwisearchservices.com/")) ||
            (tab.getUrl().getSpec().contains(
                "https://kiwisearchservices.net/")) ||
            (tab.getUrl().getSpec().contains(
                "https://www.kiwisearchservices.net/")))
          return;
      ...
      public void onLoadStopped(Tab tab, boolean toDifferentDocument) {
        ...
        // If we made some progress, fast-forward to complete, otherwise just
        // dismiss any MINIMUM_LOAD_PROGRESS that had been set.
      ...
      public void onWebContentsSwapped(Tab tab, boolean didStartLoad,
        ...
        // WebContents, we won't get any load progress signals. Otherwise, we
        // should receive at least one real signal so we don't need to simulate
        // them.
   ...
   */
  void simulateLoadProgressCompletion() { mLoadProgressSimulator.start(); }

  /**
   * Whether progressbar should be updated on tab progress changes.
   * @param preventUpdates If true, prevents updating progressbar when the tab
   *     it's observing
   *                       is being loaded.
   */
  ...
  private void onNewTabObserved(Tab tab) {
    if (tab == null) {
      // If start surface is enabled and new tab is null, then new tab is home
      // page or tab switcher. Finish progress bar loading.
  ...
  private void updateLoadProgress(float progress) {
    ...
    if (MathUtils.areFloatsEqual(progress, 1))
      finishLoadProgress(true);
  }
  ...
  private void finishLoadProgress(boolean animateCompletion) {
    ...
    @CompletionState
    int completionState =
        animateCompletion ? CompletionState.FINISHED_DO_ANIMATE
  ...
  public void destroy() { mTabObserver.destroy(); }
}
