...
/**
 * A SceneLayer to render the top toolbar. This is the "view" piece of the top
 * toolbar overlay.
 */
...
class TopToolbarSceneLayer extends SceneOverlayLayer {
  ...
  public static void bind(PropertyModel model, TopToolbarSceneLayer view,
                          PropertyKey key) {
    view.pushProperties(model);
  }

  /**
   * @param resourceManagerSupplier Access to a {@link ResourceManager} for
   *     textures.
   */
  ...
  private void pushProperties(PropertyModel model) {
    if (mResourceManagerSupplier.get() == null)
      return;
    TopToolbarSceneLayerJni.get().updateToolbarLayer(
        mNativePtr, TopToolbarSceneLayer.this, mResourceManagerSupplier.get(),
    ...
    DrawingInfo progressInfo =
        model.get(TopToolbarOverlayProperties.PROGRESS_BAR_INFO);
    if (progressInfo == null)
      return;

    TopToolbarSceneLayerJni.get().updateProgressBar(
        mNativePtr, TopToolbarSceneLayer.this,
        progressInfo.progressBarRect.left, progressInfo.progressBarRect.top,
        progressInfo.progressBarRect.width(),
        progressInfo.progressBarRect.height(), progressInfo.progressBarColor,
  ...
  protected void initializeNative() {
    if (mNativePtr == 0) {
      mNativePtr =
          TopToolbarSceneLayerJni.get().init(TopToolbarSceneLayer.this);
    }
    assert mNativePtr != 0;
  }
  ...
  interface Natives {
    ...
    void setContentTree(long nativeTopToolbarSceneLayer,
                        TopToolbarSceneLayer caller, SceneLayer contentTree);
    void updateToolbarLayer(long nativeTopToolbarSceneLayer,
                            TopToolbarSceneLayer caller,
                            ResourceManager resourceManager, int resourceId,
                            int toolbarBackgroundColor, int urlBarResourceId,
                            int urlBarColor, float contentOffset,
                            boolean showShadow, boolean visible,
                            boolean enableBottomToolbar);
    void updateProgressBar(
        long nativeTopToolbarSceneLayer, TopToolbarSceneLayer caller,
        ...
        int progressBarBackgroundY, int progressBarBackgroundWidth,
