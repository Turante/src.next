...
public class TopToolbarOverlayMediator {
  ...
  private final Callback<ClipDrawableProgressBar.DrawingInfo>
      mProgressInfoCallback;
  ...
  /**
   * Whether the active layout has its own toolbar to display instead of this
   * one.
   */
  ...
  TopToolbarOverlayMediator(
      PropertyModel model, Context context,
    ...
    mSceneChangeObserver = new LayoutStateObserver() {
      ...
      public void onStartedShowing(@LayoutType int layout,
                                   boolean showToolbar) {
        // TODO(1100332): Once ToolbarSwipeLayout uses a SceneLayer that does
        // not include
        //                its own toolbar, only check for the vertical tab
        //                switcher.
    ...
    // Keep an observer attached to the visible tab (and only the visible tab)
    // to update properties including theme color.
    ...
    mBrowserControlsObserver = new BrowserControlsStateProvider.Observer() {
      ...
      public void onControlsOffsetChanged(
          int topOffset, int topControlsMinHeightOffset, int bottomOffset,
          int bottomControlsMinHeightOffset, boolean needsAnimate) {
        // The toolbar layer is positioned below the minimum height (i.e. the
        // top of the toolbar layer is set to the bottom of minimum height).
        // Hence, the offset consists of the top controls offset plus top
        // controls minimum height.
        int yOffset =
            topOffset + mBrowserControlsStateProvider.getTopControlsMinHeight();
  ...
  /**
   * Compute whether the texture's shadow should be visible. The shadow is
   * visible whenever the android view is not shown.
   */
  private void updateShadowState() {
    boolean drawControlsAsTexture = BrowserControlsUtils.drawControlsAsTexture(
  ...
  private void updateThemeColor(Tab tab) {
    ...
    mModel.set(TopToolbarOverlayProperties.URL_BAR_COLOR,
               getUrlBarBackgroundColor(tab, color));
  }
  ...
  private int getToolbarBackgroundColor(Tab tab) {
    if (sToolbarBackgroundColorForTesting != null)
      return sToolbarBackgroundColorForTesting;
  ...
  private int getUrlBarBackgroundColor(Tab tab, @ColorInt int backgroundColor) {
    if (sUrlBarColorForTesting != null)
      return sUrlBarColorForTesting;
  ...
  private void updateProgress() {
    ...
    // Update and set the progress info to trigger an update; the
    // PROGRESS_BAR_INFO property skips the object equality check.
    mProgressInfoCallback.onResult(
  ...
  boolean shouldBeAttachedToTree() { return true; }
