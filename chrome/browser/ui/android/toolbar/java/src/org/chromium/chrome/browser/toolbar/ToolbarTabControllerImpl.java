...
public class ToolbarTabControllerImpl implements ToolbarTabController {
  ...
  private final Supplier<BottomControlsCoordinator>
   ...
   * @param overrideHomePageSupplier Supplier that returns true if it overrides
   *     the default
   *         homepage behavior.
   * @param trackerSupplier Supplier for the current profile tracker.
   * @param homepageUrlSupplier Supplier for the homepage URL.
   * @param onSuccessRunnable Runnable that is invoked when the active tab is
   *     asked to perform the
   *         corresponding ToolbarTabController action; it is not invoked if the
   * tab cannot perform the action or for openHompage.
   */
  public ToolbarTabControllerImpl(
      Supplier<Tab> tabSupplier, Supplier<Boolean> overrideHomePageSupplier,
  ...
  @Override
  public boolean back() {
    BottomControlsCoordinator controlsCoordinator =
  ...
  public void openHomepage() {
    ...
    if (mOverrideHomePageSupplier.get()) {
      if (ChromeFeatureList.isEnabled(ChromeFeatureList.TOOLBAR_IPH_ANDROID)) {
        // While some other element is handling the routing of this click event,
        // something still needs to notify the event. This approach allows
        // consolidation of events for the home button.
    ...
    RecordHistogram.recordBooleanHistogram("Navigation.Home.IsChromeInternal",
                                           is_chrome_internal);
  ...
  private void recordHomeButtonUserPerProfileType() {
    ...
    RecordHistogram.recordEnumeratedHistogram(
        "Android.HomeButton.PerProfileType", type,
