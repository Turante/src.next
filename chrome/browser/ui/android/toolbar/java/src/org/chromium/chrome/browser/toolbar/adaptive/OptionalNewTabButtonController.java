...
public class OptionalNewTabButtonController
  ...
  /**
   * Set of methods used to interact with dependencies which may require native
   * libraries to function. Robolectric tests can use shadows to inject
  ...
  /* package */ static class Delegate {
    ...
    /**
     * Returns a {@link TabModelSelector} used for obtaining the current tab and
     * the incognito state.
     *
     * <p>Not using {@link IncognitoStateProvider} because ISP is created in the
     * {@link ToolbarManager} and not in {@link RootUiCoordinator}.
     *
     * <p>TODO(crbug.com/1185948): Make IncognitoStateProvider available in
     * RootUiCooridnator.
     */
    @Nullable
  ...
  private final ObserverList<ButtonDataObserver> mObservers =
      new ObserverList<>();
   ...
   * @param activityLifecycleDispatcher Dispatcher for activity lifecycle
   *     events, e.g.
  ...
  public OptionalNewTabButtonController(
      Context context, Drawable buttonDrawable,
    ...
    mDelegate =
        new Delegate(tabCreatorManagerSupplier, tabModelSelectorSupplier);
    ...
    mButtonData = new ButtonDataImpl(
        /*canShow=*/false, buttonDrawable, onClickListener,
   ...
   * fist available opportunity to create and set IPHCommandBuilder. Once set
   * it's never updated.
   */
  private void maybeSetIphCommandBuilder(Tab tab) {
    if (mButtonData.getButtonSpec().getIPHCommandBuilder() != null ||
        tab == null || !FeatureList.isInitialized() ||
    ...
    IPHCommandBuilder iphCommandBuilder =
        new IPHCommandBuilder(
            ...
            R.string.adaptive_toolbar_button_new_tab_iph)
            .setHighlightParams(params);

    ButtonData.ButtonSpec currentSpec = mButtonData.getButtonSpec();
    ButtonData.ButtonSpec newSpec = new ButtonData.ButtonSpec(
        currentSpec.getDrawable(), currentSpec.getOnClickListener(),
        currentSpec.getContentDescriptionResId(),
