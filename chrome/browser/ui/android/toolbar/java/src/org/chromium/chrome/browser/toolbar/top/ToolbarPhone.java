...
public class ToolbarPhone
    extends ToolbarLayout implements OnClickListener, TabCountObserver {
  /**
   * The amount of time transitioning from one theme color to another should
   * take in ms.
   */
  public static final long THEME_COLOR_TRANSITION_DURATION =
      50; // In theory 417, since we have sped-up animation 0.6 in Kiwi, so we
          // could adjust back to make it slower
  ...
  private static final int TAB_SWITCHER_MODE_EXIT_FADE_ANIMATION_DURATION_MS =
      1;
  private static final int TAB_SWITCHER_MODE_POST_EXIT_ANIMATION_DURATION_MS =
      1;
  ...
  // This determines whether or not the toolbar draws as expected (false) or
  // whether it always draws as if it's showing the non-tabswitcher,
  // non-animating toolbar. This is used in grabbing a bitmap to use as a
  // texture representation of this view.
  @ViewDebug.ExportedProperty(category = "chrome")
  ...
  // Value that determines the amount of transition from the normal toolbar mode
  // to TabSwitcher mode.  0 = entirely in normal mode and 1.0 = entirely in
  // TabSwitcher mode.  In between values can be used for animating between the
  // two view modes.
  @ViewDebug.ExportedProperty(category = "chrome")
  protected float mTabSwitcherModeFraction;

  // Used to clip the toolbar during the fade transition into and out of
  // TabSwitcher mode.  Only used when |mAnimateNormalToolbar| is false.
  @ViewDebug.ExportedProperty(category = "chrome") private Rect mClipRect;
  ...
  /**
   * The degree to which the omnibox has expanded to full width, either because
   * it is getting focused or the NTP search box is being scrolled up. Note that
   * in the latter case, the actual width of the omnibox is not interpolated
   * linearly from this value. The value will be the maximum of {@link
   * #mUrlFocusChangeFraction} and {@link #mNtpSearchBoxScrollFraction}.
   *
  ...
  /**
   * Offset applied to the bounds of the omnibox if we are showing a New Tab
   * Page.
   */
  private final Rect mLocationBarBackgroundNtpOffset = new Rect();

  /**
   * Offsets applied to the <i>contents</i> of the omnibox if we are showing a
   * New Tab Page. This can be different from {@link
   * #mLocationBarBackgroundNtpOffset} due to the fact that we extend the
   * omnibox horizontally beyond the screen boundaries when focused, to hide its
   * rounded corners.
   */
  ...
  /**
   * Whether the toolbar has a pending request to call {@link
  ...
  /**
   * The progress fraction for the location bar width change animation that is
   * run when the optional button is shown/hidden. Animates from 1.f to 0.f when
   * showing the button and 0.f to 1.f when hiding the button, where 0.f
   * indicates the location bar width is not offset at all for the animation.
   */
  private float mLocBarWidthChangeFraction;

  /**
   * A global layout listener used to capture a new texture when the
   * experimental toolbar button is added or removed.
   */
  private ViewTreeObserver
      .OnGlobalLayoutListener mOptionalButtonLayoutListener =
      () -> requestLayoutHostUpdateForOptionalButton();

  // The following are some properties used during animation.  We use explicit
  // property classes to avoid the cost of reflection for each animation setup.
  ...
  private final Property<ToolbarPhone, Float>
      mLocBarWidthChangeFractionProperty =
          new Property<ToolbarPhone, Float>(Float.class, "") {
            @Override
            public Float get(ToolbarPhone object) {
  ...
  public void onFinishInflate() {
    try (TraceEvent te = TraceEvent.scoped("ToolbarPhone.onFinishInflate")) {
      ...
      mUrlFocusTranslationX = getResources().getDimensionPixelSize(
          R.dimen.toolbar_url_focus_translation_x);
    }
  }

  @Override
  public void
  setLocationBarCoordinator(LocationBarCoordinator locationBarCoordinator) {
  ...
  /**
   * Initializes the background, padding, margins, etc. for the location bar
   * background.
   */
  ...
  public static Drawable createModernLocationBarBackground(Context context) {
    ...
    final int tint = ChromeColors.getSurfaceColor(
        context, R.dimen.toolbar_text_box_elevation);
    // TODO(https://crbug.com/1239289): Change back to #setTint once our min API
    // level is 23.
  ...
  /**
   * Set the background color of the location bar to appropriately match the
   * theme color.
   */
  private void updateModernLocationBarColor(int color) {
    if (mCurrentLocationBarColor == color)
      return;
  ...
  /**
   * Sets up click and key listeners once we have native library available to
   * handle clicks.
   */
  @Override
  protected void onNativeLibraryReady() {
    ...
    getMenuButtonCoordinator().setOnKeyListener(
    ...
    // Calls the {@link triggerUrlFocusAnimation()} here to finish the pending
    // focus request if it has been skipped in {@link
    // onStartSurfaceStateChanged()}.
    if (mPendingTriggerUrlFocusRequest) {
      // This pending focus must be requested before native initialization when
      // instant start is enabled, whether user clicks omnibox or not.
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    // If the NTP is partially scrolled, prevent all touch events to the child
    // views.  This is to not allow a secondary touch event to trigger entering
    // the tab switcher, which can lead to really odd snapshots and transitions
    // to the switcher.
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    // Forward touch events to the NTP if the toolbar is moved away but the
    // search box hasn't reached the top of the page yet.
    if (mNtpSearchBoxTranslation.y < 0 &&
        mLocationBar.getPhoneCoordinator().getTranslationY() > 0) {
      return getToolbarDataProvider()
  ...
  public void onClick(View v) {
    ...
    if (mHomeButton != null && mHomeButton == v) {
      ...
      if (isNativeLibraryReady() &&
          mPartnerHomepageEnabledSupplier.getAsBoolean()) {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    if (!mDisableLocationBarRelayout) {
      ...
      boolean changed =
          layoutLocationBar(MeasureSpec.getSize(widthMeasureSpec));
  ...
  /**
   * @return True if layout bar's unfocused width has changed, potentially
   *     causing updates to
   *         visual elements. If this happens during measurement pass, then
   * toolbar's layout needs to be remeasured.
   */
  ...
  private boolean layoutLocationBar(int containerWidth) {
    ...
    // Chrome prevents layout_gravity="left" from being defined in XML, but it
    // simplifies the logic, so it is manually specified here.
    ...
    // Always update the unfocused layout params regardless of whether we are
    // using those in this current layout pass as they are needed for
    // animations.
    ...
    if (mLayoutLocationBarInFocusedMode ||
      ...
      int priorVisibleWidth = mLocationBar.getPhoneCoordinator()
                                  .getOffsetOfFirstVisibleFocusedView();
   ...
   * @param priorVisibleWidth The width of any visible views prior to the
   *     location bar.
   * @return The width of the location bar when it has focus.
   */
  private int getFocusedLocationBarWidth(int containerWidth,
  ...
  private int getFocusedLocationBarLeftMargin(int priorVisibleWidth) {
    ...
    if (mLocationBar.getPhoneCoordinator().getLayoutDirection() ==
   ...
   * @return The left bounds of the location bar, accounting for any buttons on
   *     the left side
   *         of the toolbar.
   */
  private int getViewBoundsLeftOfLocationBar(@VisualState int visualState) {
    // Uses getMeasuredWidth()s instead of getLeft() because this is called in
    // onMeasure and the layout values have not yet been set.
  ...
  /**
   * @return The left bounds of the location bar after accounting for any
   *     visible left buttons.
   */
  private int getBoundsAfterAccountingForLeftButton() {
    int padding = mToolbarSidePadding;

    // If home button is visible, or it's now in overview and toolbar is not
    // shown (url bar shouldn't be focused), mHomeButton.getMeasuredWidth()
    // should be returned as the left bound.
    if (mHomeButton != null && (mHomeButton.getVisibility() != GONE ||
                                isInOverviewAndToolbarInvisible())) {
  ...
  /**
   * @param visualState The current {@link VisualState} of the toolbar.
   * @return The right bounds of the location bar, accounting for any buttons on
   *     the right side
   *         of the toolbar.
   */
  private int getViewBoundsRightOfLocationBar(@VisualState int visualState) {
    // Uses getMeasuredWidth()s instead of getRight() because this is called in
    // onMeasure and the layout values have not yet been set.
    if (visualState == VisualState.NEW_TAB_NORMAL &&
        mTabSwitcherState == STATIC_TAB) {
  ...
  /**
   * @return The right bounds of the location bar after accounting for any
   *     visible right buttons.
   */
  private int getBoundsAfterAccountingForRightButtons() {
    if (mStartSurfaceScrollFraction == 1.0f)
      return mToolbarSidePadding;
  ...
  /**
   * Returns whether it's on overview mode (on start surface homepage or tab
   * switcher surface) and toolbar phone is not shown.
   */
  ...
  private void updateToolbarBackground(int color) {
    if (mToolbarBackground.getColor() == color)
      return;
  ...
  private int getToolbarColorForVisualState(final
    ...
    case VisualState.NEW_TAB_NORMAL:
      ...
      // transparent. When the location bar reaches the top of the screen (i.e.
      // location bar is fully expanded), the background needs to change back to
      // the default toolbar color so that the NTP content is not visible
      // beneath the toolbar. In between the transition, we set a translucent
      // default toolbar color based on the expansion progress of the toolbar.
  ...
  protected void dispatchDraw(Canvas canvas) {
    ...
    if (!mTextureCaptureMode &&
        mToolbarBackground.getColor() != Color.TRANSPARENT) {
    ...
    if (mTextureCaptureMode) {
      ...
      if (mTabSwitcherModeAnimation != null) {
        ...
        // Perform the fade logic before super.dispatchDraw(canvas) so that we
        // can properly set the values before the draw happens.
        if (!mAnimateNormalToolbar) {
          drawTabSwitcherFadeAnimation(tabSwitcherAnimationFinished,
                                       mTabSwitcherModeFraction);
        }
      }
      ...
      if (mTabSwitcherModeAnimation != null) {
        // Perform the overlay logic after super.dispatchDraw(canvas) as we need
        // to draw on top of the current views.
        if (mAnimateNormalToolbar) {
          drawTabSwitcherAnimationOverlay(canvas, mTabSwitcherModeFraction);
        }

        // Clear the animation.
        if (tabSwitcherAnimationFinished)
  ...
  /**
   * @return True if the toolbar is showing tab switcher assets, including
   *     during transitions.
   */
  ...
  /**
   * Calculate the bounds for the location bar background and set them to {@code
   * out}.
   */
  private void updateLocationBarBackgroundBounds(Rect out,
                                                 @VisualState int visualState) {
    ...
    int rightViewPosition =
        getRightPositionOfLocationBarBackground(visualState);
    ...
    // - The bottom of the viewport is aligned with the bottom of the location
    // bar. Additional padding can be applied for use during animations.
   ...
   */
  private int
  getLeftPositionOfLocationBarBackground(@VisualState int visualState) {
    float expansion = getExpansionFractionForVisualState(visualState);
  ...
  /**
   * @return The left drawing position for the location bar background when the
   *     location bar
   *         has focus.
   */
   ...
   */
  private int
  getRightPositionOfLocationBarBackground(@VisualState int visualState) {
    float expansion = getExpansionFractionForVisualState(visualState);
  ...
  /**
   * @return The location bar background position offset, for use when the
   *     optional button
   *         show/hide animation is running.
   */
  private int getLocationBarBackgroundOffsetForOptionalButton() {
    return (int)(getLocationBarWidthOffsetForOptionalButton() *
                 mLocBarWidthChangeFraction);
  }

  /**
   * @return The difference in the location bar width when the optional button
   *     is hidden
   *         rather than showing. This is effectively the width of the optional
   * button with some adjustment to account for possible padding differences
  ...
  float getLocationBarWidthOffsetForOptionalButton() {
    ...
    // When the optional button is the only visible button after the location
    // bar and the button is hidden mToolbarSidePadding is used for the padding
    // after the location bar.
    if (!isMenuButtonPresent()) {
  ...
  /**
   * @return The right drawing position for the location bar background when the
   *     location bar
   *         has focus.
   */
  ...
  private float
  getExpansionFractionForVisualState(@VisualState int visualState) {
  ...
  private void updateUrlExpansionFraction() {
    mUrlExpansionFraction = Math.max(
  ...
  /**
   * Updates the parameters relating to expanding the location bar, as the
   * result of either a focus change or scrolling the New Tab Page.
   */
  private void updateUrlExpansionAnimation() {
    // TODO(https://crbug.com/865801): Prevent url expansion signals from
    // happening while the toolbar is not visible (e.g. in tab switcher mode).
    if (isInTabSwitcherMode())
      return;
  ...
  /**
   * Updates the location bar layout, as the result of either a focus change or
   * scrolling the New Tab Page.
   */
  private void updateLocationBarLayoutForExpansionAnimation() {
    TraceEvent.begin(
        "ToolbarPhone.updateLocationBarLayoutForExpansionAnimation");
    FrameLayout.LayoutParams locationBarLayoutParams =
    ...
    float locationBarBaseTranslationX =
        mUnfocusedLocationBarLayoutLeft - currentLeftMargin;
    if (mOptionalButtonAnimationRunning) {
      ...
      // mUnfocusedLocationBarLayoutLeft have not been updated to take into
      // account the appearance of the optional icon. The views to left of the
      // location bar will be wider than mUnfocusedlocationBarLayoutLeft in RTL,
      // so adjust the translation by that amount. When hiding the button, we
      // force a relayout without the optional toolbar button
      // (mLayoutLocationBarWithoutExtraButton). mUnfocusedLocationBarLayoutLeft
      // reflects the view bounds left of the location bar, which still includes
      // the optional button. The location bar left margin, however, has been
      // adjusted to reflect its end value when the optional button is fully
      // hidden. The locationBarBaseTranslationX above accounts for the
      // difference between mUnfocusedLocationBarLayoutLeft and the location
      // bar's current left margin.
      locationBarBaseTranslationX +=
          getViewBoundsLeftOfLocationBar(mVisualState) -
          mUnfocusedLocationBarLayoutLeft;
    }

    // When the dse icon is visible, the LocationBar needs additional
    // translation to compensate for the dse icon being laid out when focused.
    // This also affects the UrlBar, which is handled below. See comments in
    // LocationBar#getLocationBarOffsetForFocusAnimation() for implementation
    // details.
    boolean isIncognito = getToolbarDataProvider().isIncognito();
    if (SearchEngineLogoUtils.getInstance().shouldShowSearchEngineLogo(
            isIncognito)) {
      locationBarBaseTranslationX +=
          getLocationBarOffsetForFocusAnimation(hasFocus());
    }
    ...
    locationBarBaseTranslationX *=
        1f - (mOptionalButtonAnimationRunning ? mLocBarWidthChangeFraction
                                              : mUrlExpansionFraction);
    ...
    if (currentTab != null) {
      ...
      if (isLocationBarShownInNTP() &&
        ...
        // Reset these values in case we transitioned to a different page during
        // the transition.
    ...
    if (isLocationBarRtl) {
      locationBarTranslationX =
          locationBarBaseTranslationX + mLocationBarNtpOffsetRight;
    } else {
      locationBarTranslationX =
          locationBarBaseTranslationX + mLocationBarNtpOffsetLeft;
    }

    mLocationBar.getPhoneCoordinator().setTranslationX(locationBarTranslationX);

    // When the dse icon is enabled, the UrlBar needs additional translation to
    // compensate for the additional translation applied to the LocationBar. See
    // comments in LocationBar#getUrlBarTranslationXForToolbarAnimation() for
    // implementation details.
    if (SearchEngineLogoUtils.getInstance().shouldShowSearchEngineLogo(
            isIncognito)) {
      mUrlBar.setTranslationX(getUrlBarTranslationXForToolbarAnimation(
    ...
    if (!mOptionalButtonAnimationRunning) {
      ...
      // Only transition theme colors if in static tab mode that is not the NTP.
      // In practice this only runs when you focus the omnibox on a web page.
      if (!isLocationBarShownInNTP() && mTabSwitcherState == STATIC_TAB) {
        ...
        int defaultLocationBarColor =
            getLocationBarColorForToolbarColor(defaultColor);
        int primaryColor = getToolbarDataProvider().getPrimaryColor();
        int themedLocationBarColor =
            getLocationBarColorForToolbarColor(primaryColor);
        ...
        updateModernLocationBarColor(ColorUtils.getColorWithOverlay(
            themedLocationBarColor, defaultLocationBarColor,
            mUrlFocusChangeFraction));
      }
    }

    // Force an invalidation of the location bar to properly handle the clipping
    // of the URL bar text as a result of the URL action container translations.
  ...
  /**
   * Calculates the translation X for the URL actions container for use in the
   * URL expansion animation.
   *
   * @param isLocationBarRtl Whether the location bar layout is RTL.
   * @param locationBarBaseTranslationX The base location bar translation for
   *     the URL expansion
   *                                    animation.
  ...
  private float getUrlActionsTranslationXForExpansionAnimation(
    ...
    if (!isLocationBarRtl || isRtl) {
      // Negate the location bar translation to keep the URL action container in
      // the same place during the focus expansion.
      urlActionsTranslationX = -locationBarBaseTranslationX;
    }

    if (isRtl) {
      urlActionsTranslationX +=
          mLocationBarNtpOffsetLeft - mLocationBarNtpOffsetRight;
    } else {
      urlActionsTranslationX +=
          mLocationBarNtpOffsetRight - mLocationBarNtpOffsetLeft;
    }
  ...
  /**
   * Reset the parameters for the New Tab Page transition animation (expanding
   * the location bar as a result of scrolling the New Tab Page) to their
   * default values.
   */
  ...
  /**
   * Updates the parameters of the New Tab Page transition animation (expanding
   * the location bar as a result of scrolling the New Tab Page).
   */
  private void updateNtpTransitionAnimation() {
    // Skip if in or entering tab switcher mode.
    if (mTabSwitcherState == TAB_SWITCHER ||
    ...
    ntpDelegate.getSearchBoxBounds(mNtpSearchBoxBounds,
                                   mNtpSearchBoxTranslation);
    int locationBarTranslationY =
        Math.max(0, (mNtpSearchBoxBounds.top -
                     mLocationBar.getPhoneCoordinator().getTop()));
    mLocationBar.getPhoneCoordinator().setTranslationY(locationBarTranslationY);

    updateButtonsTranslationY();

    // Linearly interpolate between the bounds of the search box on the NTP and
    // the omnibox background bounds. |shrinkage| is the scaling factor for the
    // offset -- if it's 1, we are shrinking the omnibox down to the size of the
    // search box.
    float shrinkage =
        1f - Interpolators.FAST_OUT_SLOW_IN_INTERPOLATOR.getInterpolation(
                 mUrlExpansionFraction);
    ...
    mLocationBarBackgroundNtpOffset.set(
        Math.round(leftBoundDifference * shrinkage), locationBarTranslationY,
        Math.round(rightBoundDifference * shrinkage), locationBarTranslationY);
    ...
    // The search box on the NTP is visible if our omnibox is invisible, and
    // vice-versa.
  ...
  /**
   * Update the y translation of the buttons to make it appear as if they were
   * scrolling with the new tab page.
   */
  private void updateButtonsTranslationY() {
    int transY = mTabSwitcherState == STATIC_TAB
                     ? Math.min(mNtpSearchBoxTranslation.y, 0)
                     : 0;
    ...
    if (mHandButton != null)
      mHandButton.setTranslationY(transY);
  }
  ...
  private void drawTabSwitcherFadeAnimation(boolean animationFinished,
                                            float progress) {
    setAlpha(progress);
    ...
    if (mClipRect != null)
      mClipRect.set(0, 0, getWidth(), (int)(getHeight() * progress));
  }

  /**
   * When entering and exiting the TabSwitcher mode, we fade out or fade in the
   * browsing mode of the toolbar on top of the TabSwitcher mode version of it.
   * We do this by drawing all of the browsing mode views on top of the android
   * view.
   */
  @VisibleForTesting
  void drawTabSwitcherAnimationOverlay(Canvas canvas, float animationProgress) {
    ...
    if (mLocationBar.getPhoneCoordinator().getAlpha() != 0 &&
        isLocationBarCurrentlyShown()) {
    ...
    if (mOptionalButton != null &&
      ...
      ViewUtils.translateCanvasToView(mToolbarButtonsContainer, mOptionalButton,
                                      canvas);
      ...
      int backgroundLeft =
          (mOptionalButton.getWidth() - mOptionalButton.getPaddingLeft() -
           mOptionalButton.getPaddingRight() - backgroundWidth) /
          2;
      backgroundLeft += mOptionalButton.getPaddingLeft();
      int backgroundTop =
          (mOptionalButton.getHeight() - mOptionalButton.getPaddingTop() -
    ...
    if (mHandButton != null && mHandButton.getVisibility() != View.GONE) {
      ...
      ViewUtils.translateCanvasToView(mToolbarButtonsContainer, mHandButton,
                                      canvas);
    ...
    if (mTabSwitcherAnimationTabStackDrawable != null &&
        mToggleTabStackButton != null && mUrlExpansionFraction != 1f) {
      // Draw the tab stack button image.
      canvas.save();
      ViewUtils.translateCanvasToView(mToolbarButtonsContainer,
                                      mToggleTabStackButton, canvas);

      int backgroundWidth =
          mToggleTabStackButton.getDrawable().getIntrinsicWidth();
      int backgroundHeight =
          mToggleTabStackButton.getDrawable().getIntrinsicHeight();
      int backgroundLeft =
    ...
    final MenuButtonCoordinator menuButtonCoordinator =
  ...
  protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
    ...
    if (mLocationBarBackground != null) {
      ...
      int translationY =
          (int)mLocationBar.getPhoneCoordinator().getTranslationY();
      ...
      if (mUrlExpansionFraction != 0f && clipTop < child.getBottom()) {
        ...
        // Hardware mode does not support unioned clip regions, so clip using
        // the appropriate bounds based on whether the child is to the left or
  ...
  private boolean isChildLeft(View child) {
    return (mHomeButton != null && child == mHomeButton) ^
        LocalizationUtils.isLayoutRtl();
  }

  /**
   * @return Whether or not the location bar should be drawing at any particular
   *     state of the
   *         toolbar.
   */
  ...
  private boolean drawLocationBar(Canvas canvas, long drawingTime) {
    ...
    if (shouldDrawLocationBar()) {
      ...
      if (shouldDrawLocationBarBackground()) {
        ...
        mActiveLocationBarBackground.setBounds(
            ...
            mLocationBarBackgroundBounds.top +
                mLocationBarBackgroundNtpOffset.top,
            mLocationBarBackgroundBounds.right +
                mLocationBarBackgroundNtpOffset.right,
            mLocationBarBackgroundBounds.bottom +
                mLocationBarBackgroundNtpOffset.bottom);
        mActiveLocationBarBackground.draw(canvas);
      }

      float locationBarClipLeft = mLocationBarBackgroundBounds.left +
                                  mLocationBarBackgroundNtpOffset.left;
      float locationBarClipRight = mLocationBarBackgroundBounds.right +
                                   mLocationBarBackgroundNtpOffset.right;
      float locationBarClipTop = mLocationBarBackgroundBounds.top +
                                 mLocationBarBackgroundNtpOffset.top;
      ...
      final int locationBarPaddingEnd =
          mLocationBar.getPhoneCoordinator().getPaddingEnd();
      final int locationBarDirection =
          mLocationBar.getPhoneCoordinator().getLayoutDirection();
      // When unexpanded, the location bar's visible content boundaries are
      // inset from the viewport used to draw the background.  During expansion
      // transitions, compensation is applied to increase the clip regions such
      // that when the location bar converts to the narrower collapsed layout
      // the visible content is the same.
      if (mUrlExpansionFraction != 1f && !mOptionalButtonAnimationRunning) {
        ...
        // When the defocus animation is running, the location bar padding needs
        // to be subtracted from the clip bounds so that the location bar text
        // width in the last frame of the animation matches the text width of
        // the unfocused location bar.
      ...
      // Offset the clip rect by a set amount to ensure the Google G is
      // completely inside the omnibox background when animating in.
      if (SearchEngineLogoUtils.getInstance().shouldShowSearchEngineLogo(
              isIncognito()) &&
          isLocationBarShownInNTP() && urlHasFocus() &&
      ...
      canvas.clipRect(locationBarClipLeft, locationBarClipTop,
    ...
    boolean retVal = super.drawChild(
        canvas, mLocationBar.getPhoneCoordinator().getViewForDrawing(),
        drawingTime);
  ...
  protected void onAttachedToWindow() {
    ...
    // This is a workaround for http://crbug.com/574928. Since Jelly Bean is the
    // lowest version we support now and the next deprecation target, we decided
    // to simply workaround.
  ...
  public void draw(Canvas canvas) {
    // If capturing a texture of the toolbar, ensure the alpha is set prior to
    // draw(...) being called.  The alpha is being used prior to getting to
    // draw(...), so updating the value after this point was having no affect.
    if (mTextureCaptureMode)
      assert getAlpha() == 1f;

    // mClipRect can change in the draw call, so cache this value to ensure the
    // canvas is restored correctly.
    ...
    if (shouldClip) {
      ...
      // Post an invalidate when the clip rect becomes null to ensure another
      // draw pass occurs and the full toolbar is drawn again.
      if (mClipRect == null)
        postInvalidate();
    }
  }
  ...
  public boolean setForceTextureCapture(boolean forceTextureCapture) {
    if (forceTextureCapture) {
      // Only force a texture capture if the tint for the toolbar drawables is
      // changing or if the tab count has changed since the last texture
      // capture.
  ...
  public void finishAnimations() {
    ...
    // Animator#isRunning() returns false. Sometimes this causes the progress
    // bar visibility to be set incorrectly. Update the visibility now that
    // animations are set to null. (see crbug.com/606419)
  ...
  private void removeHomeButton() { mHomeButton.setVisibility(GONE); }
  ...
  createExitTabSwitcherAnimation(final boolean animateNormalToolbar) {
    ObjectAnimator exitAnimation =
        ObjectAnimator.ofFloat(this, mTabSwitcherModeFractionProperty, 0.f);
    exitAnimation.setDuration(
        animateNormalToolbar
            ? TopToolbarCoordinator
                  .TAB_SWITCHER_MODE_NORMAL_ANIMATION_DURATION_MS
  ...
  // TODO(dtrainor): This is always true when in the tab switcher
  // (crbug.com/710750).
  ...
  public void setTabSwitcherMode(boolean inTabSwitcherMode, boolean showToolbar,
    ...
    // This method is only used for grid tab switcher with the start surface
    // disabled. When start surface is enabled, omnibox state is updated in
    // onStartSurfaceStateChanged(), which is always called before
    // setTabSwitcherMode(), so skip here.
    if (getToolbarDataProvider().shouldShowLocationBarInOverviewMode())
      return;

    // If setting tab switcher mode to true and the browser is already animating
    // or in the tab switcher skip.
    ...
    mTabSwitcherState =
        inTabSwitcherMode ? ENTERING_TAB_SWITCHER : EXITING_TAB_SWITCHER;

    // The width of location bar depends on mTabSwitcherState so layout request
    // is needed. See crbug.com/974745.
    requestLayout();
    ...
    if (inTabSwitcherMode) {
      if (mUrlFocusLayoutAnimator != null &&
          mUrlFocusLayoutAnimator.isRunning()) {
        ...
        // so that the final translation position is correct (since onMeasure
        // updates won't happen in tab switcher mode). crbug.com/518795.
  ...
  /**
   * Enables or disables the tab switcher ripple depending on whether we are in
   * or out of the tab switcher mode.
   */
  private void updateTabSwitcherButtonRipple() {
    if (mToggleTabStackButton == null)
      return;
    if (mTabSwitcherState == ENTERING_TAB_SWITCHER) {
      mToggleTabStackButton.setBackgroundColor(ApiCompatibilityUtils.getColor(
          getResources(), android.R.color.transparent));
    } else {
      // TODO(https://crbug.com/912358): This appears to no longer work.
      TypedValue outValue = new TypedValue();

      // The linked style here will have to be changed if it is updated in the
      // XML.
      getContext().getTheme().resolveAttribute(R.style.ToolbarButton, outValue,
                                               true);
  ...
  @Override
  void onStartSurfaceStateChanged(boolean shouldBeVisible,
                                  boolean isShowingStartSurface) {
    ...
    updateProgressBarVisibility();
    updateShadowVisibility();
    // Url bar should be focusable. This will be set in UrlBar#onDraw but
    // there's a delay which may cause focus to fail, so set here too.
    ...
    float startSurfaceScrollFraction =
        shouldBeVisible && isShowingStartSurface ? 1.0f : 0.0f;
  ...
  public void onTabSwitcherTransitionFinished() {
    ...
    // Detect what was being transitioned from and set the new state
    // appropriately.
  ...
  private Property<TextView, Integer>
  buildUrlScrollProperty(final View containerView,
                         final boolean isContainerRtl) {
    // If the RTL-ness of the container view changes during an animation, the
    // scroll values become invalid.  If that happens, snap to the ending
    // position and no longer update.
    return new Property<TextView, Integer>(Integer.class, "scrollX") {
      ...
      public void set(TextView view, Integer scrollX) {
        if (mRtlStateInvalid)
          return;
        boolean rtl =
            containerView.getLayoutDirection() == LAYOUT_DIRECTION_RTL;
  ...
  private void populateUrlExpansionAnimatorSet(List<Animator> animators) {
    ...
    Animator animator =
        ObjectAnimator.ofFloat(this, mUrlFocusChangeFractionProperty, 1f);
    ...
    if (mToggleTabStackButton != null) {
      animator = ObjectAnimator.ofFloat(mToggleTabStackButton, TRANSLATION_X,
    ...
    if (mOptionalButton != null &&
        mOptionalButton.getVisibility() != View.GONE) {
      animator = ObjectAnimator.ofFloat(mOptionalButton, TRANSLATION_X,
                                        toolbarButtonTranslationX);
      animator.setDuration(URL_FOCUS_TOOLBAR_BUTTONS_DURATION_MS);
      animator.setInterpolator(BakedBezierInterpolator.FADE_OUT_CURVE);
      animators.add(animator);
    ...
    if (mToolbarShadow != null) {
      animator =
          ObjectAnimator.ofFloat(mToolbarShadow, ALPHA, urlHasFocus() ? 0 : 1);
      animator.setDuration(URL_FOCUS_CHANGE_ANIMATION_DURATION_MS);
      animator.setInterpolator(BakedBezierInterpolator.TRANSFORM_CURVE);
      animators.add(animator);
    }
  ...
  private void populateUrlClearExpansionAnimatorSet(List<Animator> animators) {
    Animator animator =
        ObjectAnimator.ofFloat(this, mUrlFocusChangeFractionProperty, 0f);
    animator.setDuration(URL_FOCUS_CHANGE_ANIMATION_DURATION_MS);
    animator.setInterpolator(BakedBezierInterpolator.TRANSFORM_CURVE);
    animators.add(animator);
    ...
    if (mToggleTabStackButton != null) {
      animator =
          ObjectAnimator.ofFloat(mToggleTabStackButton, TRANSLATION_X, 0);
      animator.setDuration(URL_FOCUS_TOOLBAR_BUTTONS_DURATION_MS);
    ...
    if (mOptionalButton != null &&
        mOptionalButton.getVisibility() != View.GONE) {
      // TODO(twellington): it's possible that the optional button was shown
      // while the url bar was focused, in which case the translation x and
      // alpha animators are a no-op. Account for this case.
    ...
    mLocationBar.getPhoneCoordinator().populateFadeAnimations(
        animators, URL_FOCUS_TOOLBAR_BUTTONS_DURATION_MS,
        URL_CLEAR_FOCUS_MENU_DELAY_MS, 1);

    if (isLocationBarShownInNTP() && mNtpSearchBoxScrollFraction == 0f)
      return;
  ...
  public void onTabCountChanged(int numberOfTabs, boolean isIncognito) {
    ...
    if (mTabSwitcherAnimationTabStackDrawable != null) {
      mTabSwitcherAnimationTabStackDrawable.updateForTabCount(numberOfTabs,
                                                              isIncognito);
    }
  }

  /**
   * Get the theme color for the currently active tab. This is not affected by
   * the tab switcher's theme color.
   * @return The current tab's theme color.
   */
  private int getTabThemeColor() {
    if (getToolbarDataProvider() != null)
      return getToolbarDataProvider().getPrimaryColor();
    return getToolbarColorForVisualState(isIncognito() ? VisualState.INCOGNITO
                                                       : VisualState.NORMAL);
  }
  ...
  isVisualStateValidForBrandColorTransition(@VisualState int state) {
  ...
  public void onPrimaryColorChanged(boolean shouldAnimate) {
    ...
    final int initialLocationBarColor =
        getLocationBarColorForToolbarColor(initialColor);
    final int finalLocationBarColor =
        getLocationBarColorForToolbarColor(finalColor);
    ...
    boolean shouldUseOpaque =
        ColorUtils.shouldUseOpaqueTextboxBackground(finalColor);
    ...
    mBrandColorTransitionAnimation = ValueAnimator.ofFloat(0, 1).setDuration(
        THEME_COLOR_TRANSITION_DURATION);
    mBrandColorTransitionAnimation.setInterpolator(
        ...
        new AnimatorUpdateListener() {
          ...
          public void onAnimationUpdate(ValueAnimator animation) {
            ...
            if (shouldAnimateAlpha) {
              mLocationBarBackgroundAlpha = (int)(MathUtils.interpolate(
                  initialAlpha, finalAlpha, fraction));
            }
            updateToolbarBackground(ColorUtils.getColorWithOverlay(
                initialColor, finalColor, fraction));
            updateModernLocationBarColor(ColorUtils.getColorWithOverlay(
  ...
  private void updateNtpAnimationState() {
    ...
    if (ntpDelegate.isLocationBarShown()) {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        NtpSearchBoxDrawable ntpSearchBox =
            new NtpSearchBoxDrawable(getContext(), this);
      ...
      // Convert the previous NTP scroll progress to URL focus progress because
      // that will give a nicer transition animation from the expanded NTP
      // omnibox to the collapsed normal omnibox on other non-NTP pages.
  ...
  public void onDefaultSearchEngineChanged() {
    ...
    // Post an update for the toolbar state, which will allow all other
    // listeners for the search engine change to update before we check on the
    // state of the world for a UI update.
    // TODO(tedchoc): Move away from updating based on the search engine change
    // and instead
    //                add the toolbar as a listener to the NewTabPage and udpate
    //                only when it notifies the listeners that it has changed
    //                its state.
    post(new Runnable() {
      @Override
      public void run() {
  ...
  public void handleFindLocationBarStateChange(boolean showing) {
    setVisibility(showing                           ? View.GONE
                  : mTabSwitcherState == STATIC_TAB ? View.VISIBLE
                                                    : View.INVISIBLE);
  }
  ...
  private void updateShadowVisibility() {
    ...
    if (mToolbarShadow != null &&
        mToolbarShadow.getVisibility() != shadowVisibility) {
  ...
  private boolean shouldDrawShadow() {
    // TODO(twellington): Move this shadow state information to
    // ToolbarDataProvider and show shadow when incognito NTP is scrolled.
    return mTabSwitcherState == STATIC_TAB && !mForceHideShadow &&
        !hideShadowForIncognitoNtp() && !hideShadowForInterstitial() &&
  ...
  private @VisualState int computeVisualState() {
    if (isLocationBarShownInNTP())
      return VisualState.NEW_TAB_NORMAL;
    if (isIncognito())
      return VisualState.INCOGNITO;
    if (getToolbarDataProvider().isUsingBrandColor())
      return VisualState.BRAND_COLOR;
  ...
  private void updateVisualsForLocationBarState() {
    ...
    // These are used to skip setting state unnecessarily while in the tab
    // switcher.
    ...
    // If we are navigating to or from a brand color, allow the transition
    // animation to run to completion as it will handle the triggering this path
    // again and committing the proper visual state when it finishes.  Brand
    // color transitions are only valid between normal non-incognito pages and
    // brand color pages, so if the visual states do not match then cancel the
    // animation below.
    if (mBrandColorTransitionActive &&
    ...
    // If The page is native force the use of the standard theme for the
    // progress bar.
    if (getToolbarDataProvider() != null &&
        getToolbarDataProvider().getTab() != null &&
    ...
    if (mVisualState == VisualState.BRAND_COLOR && !visualStateChanged) {
      ...
      if (unfocusedLocationBarUsesTransparentBg !=
    ...
    // This exception is to prevent early change of theme color when exiting the
    // tab switcher since currently visual state does not map correctly to tab
    // switcher state. See https://crbug.com/832594 for more info.
    if (mTabSwitcherState != EXITING_TAB_SWITCHER) {
      updateToolbarBackgroundFromState(mVisualState);
    }
    ...
    updateModernLocationBarColor(
        getLocationBarColorForToolbarColor(currentPrimaryColor));

    mLocationBar.updateVisualsForState();

    // We update the alpha before comparing the visual state as we need to
    // change its value when entering and exiting TabSwitcher mode.
  ...
  void updateOptionalButton(ButtonData buttonData) {
    if (mOptionalButton == null) {
      ...
      if (getLayoutDirection() == LAYOUT_DIRECTION_RTL)
        mOptionalButtonTranslation *= -1;
    } else if (mOptionalButtonAnimationRunning) {
      // TODO(https://crbug.com/865801): refine this logic to allow for
      // same-button updates, e.g. swapping in a new drawable part way through
      // the animation.
      mOptionalButtonAnimator.end();
    }
    ...
    if (buttonSpec.getOnLongClickListener() == null) {
      ...
      mOptionalButton.setOnLongClickListener(
          buttonSpec.getOnLongClickListener());
    }
    mOptionalButton.setImageDrawable(buttonSpec.getDrawable());
    mOptionalButton.setContentDescription(getContext().getResources().getString(
        buttonSpec.getContentDescriptionResId()));
    ...
    if (mTabSwitcherState == STATIC_TAB) {
      ...
      getViewTreeObserver().addOnGlobalLayoutListener(
          mOptionalButtonLayoutListener);
    }
  }

  @Override
  void hideOptionalButton() {
    // mLayoutLocationBarWithoutExtraButton implies that the hide animation is
    // currently running.
    if (mOptionalButton == null ||
        mOptionalButton.getVisibility() == View.GONE ||
    ...
    if (mTabSwitcherState == STATIC_TAB && !mUrlFocusChangeInProgress &&
  ...
  /**
   * Whether the menu button is visible. Used as a proxy for whether there are
   * end toolbar buttons besides the optional button.
   */
  ...
  /**
   * Runs an animation that fades in the optional button while shortening the
  ...
  private void runShowOptionalButtonAnimation() {
    ...
    buttonTranslationAnimator.setInterpolator(
        BakedBezierInterpolator.TRANSFORM_CURVE);
    buttonTranslationAnimator.setStartDelay(
        EXPERIMENTAL_ICON_ANIMATION_DELAY_MS);
    buttonTranslationAnimator.setDuration(
        EXPERIMENTAL_ICON_ANIMATION_DURATION_MS);
  ...
  /**
   * Runs an animation that fades out the optional button while lengthening the
   * location bar background.
   */
  private void runHideOptionalButtonsAnimators() {
    ...
    ObjectAnimator buttonAnimator =
        ObjectAnimator.ofFloat(mOptionalButton, View.ALPHA, 0.f);
  ...
  /**
   * Resets the alpha and translation X for all views affected by the animations
   * for showing or hiding buttons.
   */
  ...
  /**
   * Custom drawable that allows sharing the NTP search box drawable between the
   * toolbar and the NTP.  This allows animations to continue as the drawable is
   * switched between the two owning views.
   */
  private static class NtpSearchBoxDrawable extends DrawableWrapper {
    ...
    /**
     * Reset the bounds of the drawable to the last bounds received that was not
     * marked from the toolbar.
     */
    ...
    public boolean setVisible(boolean visible, boolean restart) {
      // Ignore visibility changes.  The NTP can toggle the visibility based on
      // the scroll position of the page, so we simply ignore all of this as we
      // expect the drawable to be visible at all times of the NTP.
      return false;
    }
  ...
  private void cancelAnimations() {
    ...
    if (mBrandColorTransitionAnimation != null &&
        mBrandColorTransitionAnimation.isRunning()) {
      mBrandColorTransitionAnimation.cancel();
    }

    if (mOptionalButtonAnimator != null &&
        mOptionalButtonAnimator.isRunning()) {
      mOptionalButtonAnimator.cancel();
    }
  }

  /**
   * Calculates the offset required for the focused LocationBar to appear as if
   * it's still unfocused so it can animate to a focused state.
   *
   * @param hasFocus True if the LocationBar has focus, this will be true
   *     between the focus
  ...
  private int getLocationBarOffsetForFocusAnimation(boolean hasFocus) {
    ...
    // For LTR, the value is negative because we are making space on the
    // left-hand side. For RTL, the value is positive because we are pushing the
    // icon further to the right-hand side.
    int offset =
        statusCoordinator.getStatusIconWidth() - getAdditionalOffsetForNTP();
    return mLocationBar.isLayoutRtl() ? offset : -offset;
  }

  /**
   * Function used to position the url bar inside the location bar during
   * omnibox animation.
   *
   * @param urlExpansionPercent The current expansion percent, 1 is fully
   *     focused and 0 is
   ...
   */
  private float
  getUrlBarTranslationXForToolbarAnimation(float urlExpansionPercent,
                                           boolean hasFocus) {
    StatusCoordinator statusCoordinator = mLocationBar.getStatusCoordinator();
    if (statusCoordinator == null)
      return 0;
    ...
    // The calculation here is: the difference in padding between the focused vs
    // unfocused states and also accounts for the translation that the status
    // icon will do. In the end, this translation will be the distance that the
    // url bar needs to travel to arrive at the desired padding when focused.
    float translation = urlExpansionPercent *
                        statusCoordinator.getEndPaddingPixelSizeOnFocusDelta();

    boolean scrollingOnNtp =
        !hasFocus && statusCoordinator.isSearchEngineStatusIconVisible() &&
    ...
    if (scrollingOnNtp) {
      ...
      //
      // This branch calculates the negative distance the URL bar needs to
      // travel to completely overlap the status icon and end up in a state that
      // matches the fakebox.
      float overStatusIconTranslation =
          translation -
          (1f - urlExpansionPercent) * (statusCoordinator.getStatusIconWidth() -
                                        getAdditionalOffsetForNTP());
      // For LTR, the value is negative because the status icon is left of the
      // url bar on the x/y plane. For RTL, the value is positive because the
      // status icon is right of the url bar on the x/y plane.
