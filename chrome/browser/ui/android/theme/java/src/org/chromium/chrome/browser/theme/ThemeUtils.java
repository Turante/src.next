...
public class ThemeUtils {
  ...
  // This param is used to split the dynamic colors launch into a minimal launch
  // that is primarily activity specific, and then a later launch that will be
  // app wide.
  private static final String FULL_DYNAMIC_COLORS_PARAM = "dynamic_color_full";
  public static final BooleanCachedFieldTrialParameter
      ENABLE_FULL_DYNAMIC_COLORS = new BooleanCachedFieldTrialParameter(
          ChromeFeatureList.DYNAMIC_COLOR_ANDROID, FULL_DYNAMIC_COLORS_PARAM,
          false);

  /**
   * The background color to use for a given {@link Tab}. This will either be
   * the color specified by the associated web content or a default color if not
   * specified.
  ...
  public static int getBackgroundColor(Tab tab) {
    ...
    boolean isLocalNTP =
        tab != null && tab.getUrl().getSpec().contains("/local-ntp/");
    if (isIncognito && isLocalNTP)
      return Color.BLACK;

    WebContents tabWebContents = tab.getWebContents();
    RenderWidgetHostView rwhv = tabWebContents == null
                                    ? null
                                    : tabWebContents.getRenderWidgetHostView();
  ...
  int getTextBoxColorForToolbarBackground(Context context, @Nullable Tab tab,
   ...
   */
  public static @ColorInt
  int getTextBoxColorForToolbarBackgroundInNonNativePage(Context context,
                                                         @ColorInt int color,
                                                         boolean isIncognito) {
    // Text box color on default toolbar background in incognito mode is a
    // pre-defined color. We calculate the equivalent opaque color from the
    // pre-defined translucent color.
    if (isIncognito) {
      ...
      return ColorUtils.getColorWithOverlay(color, overlayColorOpaque,
                                            overlayColorAlpha);
    }

    // Text box color on default toolbar background in standard mode is a
    // pre-defined color instead of a calculated color.
    if (ThemeUtils.isUsingDefaultToolbarColor(context, false, color)) {
      return ChromeColors.getSurfaceColor(context,
  ...
  /**
   * Returns the icon tint for based on the given parameters. Does not adjust
   * color based on night mode as this may conflict with toolbar theme colors.
   ...
   */
  public static ColorStateList getThemedToolbarIconTint(Context context,
                                                        boolean useLight) {
    return AppCompatResources.getColorStateList(
        context, getThemedToolbarIconTintRes(useLight));
  }
  ...
  public static @ColorRes int getThemedToolbarIconTintRes(boolean useLight) {
    // Light toolbar theme colors may be used in night mode, so use
    // toolbar_icon_tint_dark which is not overridden in night- resources.
   ...
   * @param isIncognito Whether to retrieve the default theme color for
   *     incognito mode.
   * @param color The color that the toolbar is using.
   * @return If the color is the default toolbar color.
   */
  public static boolean
  isUsingDefaultToolbarColor(Context context, boolean isIncognito, int color) {
   ...
   */
  public static @ColorInt
  int getToolbarHairlineColor(Context context, @ColorInt int toolbarColor,
                              boolean isIncognito) {
    ...
    if (isUsingDefaultToolbarColor(context, isIncognito, toolbarColor)) {
      return isIncognito
          ...
          : MaterialColors.getColor(context,
                                    R.attr.divider_line_bg_color_dynamic, TAG);
    }

    final float alpha =
        ResourcesCompat.getFloat(res, R.dimen.toolbar_hairline_overlay_alpha);
    final int hairlineColorOpaque =
        res.getColor(R.color.toolbar_hairline_overlay_opaque) & 0xFF000000;
    return ColorUtils.getColorWithOverlay(toolbarColor, hairlineColorOpaque,
                                          alpha);
  }
}
