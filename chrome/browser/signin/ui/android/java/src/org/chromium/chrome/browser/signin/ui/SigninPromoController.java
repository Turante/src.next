...
/**
 * A controller for configuring the sign in promo. It sets up the sign in promo
 * depending on the context: whether there are any Google accounts on the device
 * which have been previously signed in or not. The controller also takes care
 * of counting impressions, recording signin related user actions and
 * histograms.
 */
public class SigninPromoController {
   ...
   * @param accessPoint The access point for which the impression limit is being
   *     checked.
   */
  ...
  private static boolean canShowBookmarkPromo() {
    ...
    return getSigninPromoImpressionsCountBookmarks() <
        MAX_IMPRESSIONS_BOOKMARKS &&
        !isPromoDismissed;
  }

  /**
   * Expires the promo card if Chrome has been in the background long enough.
   * We're using the "Start Surface" concept of session here which is if the app
   * has been in the background for X amount of time. Called from
   * #onStartWithNative, after which the time stored in {@link
   * ChromeInactivityTracker} is expired.
   *
   * @param timeSinceLastBackgroundedMs The time since Chrome has sent into the
   *     background.
   */
  public static void maybeExpireNTPPromo(long timeSinceLastBackgroundedMs) {
    long timeSinceLastBackgroundedLimitMs =
        StartSurfaceConfiguration
            .SIGN_IN_PROMO_SHOW_SINCE_LAST_BACKGROUNDED_LIMIT_MS.getValue();
    // If |timeSinceLastBackgroundedLimitMs| is not -1, which means we have a
    // limit on last background time, check and record whether the current time
    // is expired.
  ...
  private static boolean canShowNTPPromo() {
    ...
    return visibleAccount == null ||
        ...
        accountManagerFacade.canOfferExtendedSyncPromos(visibleAccount)
            .or(false);
  }
  ...
  private static @Nullable Account getVisibleAccount() {
    ...
    if (visibleAccount == null) {
      visibleAccount = AccountUtils.getDefaultAccountIfFulfilled(
          accountManagerFacade.getAccounts());
    }
   ...
   * @param accessPoint Specifies the AccessPoint from which the promo is to be
   *     shown.
  ...
  public SigninPromoController(
    ...
    case SigninAccessPoint.BOOKMARK_MANAGER:
      ...
      // On Android, the promo does not have a button to add and account when
      // there is already an account on the device. Always use the
      // NoExistingAccount variant.
    ...
    case SigninAccessPoint.RECENT_TABS:
      ...
      mSigninWithDefaultUserActionName =
          "Signin_SigninWithDefault_FromRecentTabs";
      mSigninNotDefaultUserActionName =
          "Signin_SigninNotDefault_FromRecentTabs";
      // On Android, the promo does not have a button to add and account when
      // there is already an account on the device. Always use the
      // NoExistingAccount variant.
      mSigninNewAccountUserActionName =
    ...
    case SigninAccessPoint.SETTINGS:
      ...
      mSigninWithDefaultUserActionName =
          "Signin_SigninWithDefault_FromSettings";
      ...
      break;
    default:
      throw new IllegalArgumentException("Unexpected value for access point: " +
                                         mAccessPoint);
    }
  }
   ...
   * @param profileDataCache The {@link ProfileDataCache} that stores profile
   *     data.
   * @param view The {@link PersonalizedSigninPromoView} that should be set up.
   * @param listener The {@link SigninPromoController.OnDismissListener} to be
   *     set to the view.
   */
  public void
  setUpSyncPromoView(ProfileDataCache profileDataCache,
                     PersonalizedSigninPromoView view,
                     SigninPromoController.OnDismissListener listener) {
    final IdentityManager identityManager =
        IdentityServicesProvider.get().getIdentityManager(
            Profile.getLastUsedRegularProfile());
    assert !identityManager.hasPrimaryAccount(ConsentLevel.SYNC)
    ...
    setupPromoView(
        view, profileDataCache.getProfileDataOrDefault(visibleAccount.name),
        listener);
  }
  ...
  public void onPromoDestroyed() {
    if (!mWasDisplayed || mWasUsed ||
        mImpressionsTilDismissHistogramName == null) {
      return;
    }
    RecordHistogram.recordCount100Histogram(mImpressionsTilDismissHistogramName,
                                            getNumImpressions());
  }

  /**
   * Configures the signin promo view and resets the impression tracker. If this
   * controller has been previously set up.
   * @param view The view in which the promo will be added.
   * @param profileData If not null, the promo will be configured to be in the
   *     hot state, using
   *         the account image, email and full name of the user to set the
   * picture and the text of the promo appropriately. Otherwise, the promo will
   * be in the cold state.
   * @param onDismissListener Listener which handles the action of dismissing
   *     the promo. A null
   *         onDismissListener marks that the promo is not dismissible and as a
   * result the close button is hidden.
   */
  ...
  /**
   * Should be called when the view is not in use anymore (e.g. it's being
   * recycled).
   */
  public void detach() {
    if (mImpressionTracker != null) {
  ...
  private void setupColdState(PersonalizedSigninPromoView view) {
    ...
    view.getPrimaryButton().setOnClickListener(
        v -> signinWithNewAccount(context));
  ...
  private void setupHotState(PersonalizedSigninPromoView view) {
    ...
    view.getPrimaryButton().setOnClickListener(
        v -> signinWithDefaultAccount(context));
    if (IdentityServicesProvider.get()
      ...
      view.getSecondaryButton().setText(
          R.string.signin_promo_choose_another_account);
      view.getSecondaryButton().setOnClickListener(
          v -> signinWithNotDefaultAccount(context));
  ...
  private void setImageSize(Context context, PersonalizedSigninPromoView view,
  ...
  @VisibleForTesting
  public static void
  setSigninPromoImpressionsCountBookmarksForTests(int count) {
  ...
  @VisibleForTesting
  public static void
  setPrefSigninPromoDeclinedBookmarksForTests(boolean isDeclined) {
    SharedPreferencesManager.getInstance().writeBoolean(
