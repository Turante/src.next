...
public final class SigninPromoUtil {
   ...
   * @param context The {@link Context} to launch the {@link
   *     SyncConsentActivity}.
   * @param syncConsentActivityLauncher launcher used to launch the {@link
   *     SyncConsentActivity}.
  ...
  public static boolean launchSigninPromoIfNeeded(
      Context context, SyncConsentActivityLauncher syncConsentActivityLauncher,
      final int currentMajorVersion) {
    if (true)
      return false;
    final SigninPreferencesManager prefManager =
  ...
  shouldLaunchSigninPromo(SigninPreferencesManager prefManager,
                          final int currentMajorVersion) {
    if (ChromeFeatureList.isEnabled(
            ChromeFeatureList.FORCE_STARTUP_SIGNIN_PROMO)) {
      return true;
    }
    final int lastPromoMajorVersion =
        prefManager.getSigninPromoLastShownVersion();
    ...
    if (TextUtils.isEmpty(UserPrefs.get(profile).getString(
            Pref.GOOGLE_SERVICES_LAST_USERNAME))) {
    ...
    if (accounts.isEmpty()) {
      // Don't show if the account list isn't available yet or there are no
      // accounts in it.
      return false;
    }
    ...
    final List<String> currentAccountNames =
        AccountUtils.toAccountNames(accounts);
    final Set<String> previousAccountNames =
        prefManager.getSigninPromoLastAccountNames();
    // Don't show if no new accounts have been added after the last time promo
    // was shown.
