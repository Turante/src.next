 ...
 *
 * <p>
 * The lifetime of this object is not tied to that of the native TabModel.  This
 * ensures the native TabModel is present when at least one incognito Tab has
 * been created and added.  When no Tabs remain, the native model will be
 * destroyed and only rebuilt when a new incognito Tab is created.
 */
class IncognitoTabModelImpl implements IncognitoTabModel {
  ...
  protected void ensureTabModelImpl() {
    ...
    if (!(mDelegateModel instanceof EmptyTabModel))
      return;
  ...
  private boolean isEmpty() { return getComprehensiveModel().getCount() == 0; }

  // Triggers IncognitoTabModelObserver.wasFirstTabCreated function. This
  // function should only be called just after the first tab is created.
  ...
  @Override
  public boolean closeTab(Tab tab, boolean animate, boolean uponExit,
                          boolean canUndo) {
    mCountOfAddingOrClosingTabs++;
  ...
  @Override
  public boolean closeTab(Tab tab, Tab recommendedNextTab, boolean animate,
                          boolean uponExit, boolean canUndo) {
    mCountOfAddingOrClosingTabs++;
    boolean retVal = mDelegateModel.closeTab(tab, recommendedNextTab, animate,
                                             uponExit, canUndo);
    mCountOfAddingOrClosingTabs--;
    destroyIncognitoIfNecessary();
    return retVal;
  ...
  public void commitAllTabClosures() {
    // Return early if no tabs are open. In particular, we don't want to destroy
    // the incognito tab model, in case we are about to add a tab to it.
    if (isEmpty())
      return;
  ...
  public void removeTab(Tab tab) {
    ...
    mCountOfAddingOrClosingTabs--;
    // Call destroyIncognitoIfNecessary() in case the last incognito tab in this
    // model is reparented to a different activity. See crbug.com/611806.
    destroyIncognitoIfNecessary();
  }
