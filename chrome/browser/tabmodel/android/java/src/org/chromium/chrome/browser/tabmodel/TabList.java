...
/**
 * A read only list of {@link Tab}s. This list understands the concept of an
 * incognito list as well as a currently selected tab (see {@link #index}).
 */
@MockedInTests
public interface TabList {
  ...
  /**
   * @return Whether this tab model contains only incognito tabs or only normal
   *     tabs.
   */
  boolean isIncognito();

  /**
   * @return The index of the current tab, or {@link #INVALID_TAB_INDEX} if
   *     there are no tabs.
   */
  int index();
   ...
   * @return The {@code Tab} at position {@code index}, or {@code null} if
   *     {@code index} < 0
   ...
   * @return      Whether or not the {@link Tab} specified by {@code tabId} has
   *     a pending
   *              closure.
   */
