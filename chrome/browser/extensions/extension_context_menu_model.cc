...
ExtensionContextMenuModel::ContextMenuAction
CommandIdToContextMenuAction(int command_id) {
...
ExtensionContextMenuModel::ExtensionContextMenuModel(
    ...
    ButtonVisibility button_visibility, PopupDelegate *delegate,
    bool can_show_icon_in_toolbar)
    : SimpleMenuModel(this), extension_id_(extension->id()),
      is_component_(Manifest::IsComponentLocation(extension->location())),
      browser_(browser), profile_(browser->profile()), delegate_(delegate),
...
ExtensionContextMenuModel::GetCurrentPageAccess(
    const Extension *extension, content::WebContents *web_contents) const {
  DCHECK(web_contents);
...
bool ExtensionContextMenuModel::IsPageAccessCommandEnabled(
    const Extension &extension, const GURL &url, int command_id) const {
...
void ExtensionContextMenuModel::HandlePageAccessCommand(
    int command_id, const Extension *extension) const {
  content::WebContents *web_contents = GetActiveWebContents();
  if (!web_contents)
    return;
...
void ExtensionContextMenuModel::LogPageAccessAction(int command_id) const {
  ...
  case PAGE_ACCESS_RUN_ON_CLICK:
    base::RecordAction(
        base::UserMetricsAction("Extensions.ContextMenu.Hosts.OnClickClicked"));
    break;
  case PAGE_ACCESS_RUN_ON_SITE:
    base::RecordAction(
        base::UserMetricsAction("Extensions.ContextMenu.Hosts.OnSiteClicked"));
    break;
  case PAGE_ACCESS_RUN_ON_ALL_SITES:
    base::RecordAction(base::UserMetricsAction(
