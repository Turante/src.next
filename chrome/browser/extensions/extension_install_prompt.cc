...
SkBitmap GetDefaultIconBitmapForMaxScaleFactor(bool is_app) {
  const gfx::ImageSkia &image =
      is_app ? extensions::util::GetDefaultAppIcon()
...
ExtensionInstallPrompt::Prompt::Prompt(PromptType type)
    : type_(type), is_requesting_host_permissions_(false), extension_(nullptr),
      average_rating_(0.0), rating_count_(0), show_user_count_(false),
...
void ExtensionInstallPrompt::Prompt::SetWebstoreData(
    const std::string &localized_user_count, bool show_user_count,
    double average_rating, int rating_count) {
...
std::u16string ExtensionInstallPrompt::Prompt::GetRetainedFilesHeading() const {
  return l10n_util::GetPluralStringFUTF16(IDS_EXTENSION_PROMPT_RETAINED_FILES,
                                          GetRetainedFileCount());
}

std::u16string
ExtensionInstallPrompt::Prompt::GetRetainedDevicesHeading() const {
  return l10n_util::GetPluralStringFUTF16(IDS_EXTENSION_PROMPT_RETAINED_DEVICES,
...
void ExtensionInstallPrompt::Prompt::AppendRatingStars(StarAppender appender,
                                                       void *data) const {
  CHECK(appender);
  CHECK(AllowWebstoreData(type_));
...
std::u16string ExtensionInstallPrompt::Prompt::GetPermissionsAsString() const {
  ...
  result = result + base::JoinString(prompt_permissions_.permissions, u"\n\n");
  result = result + base::JoinString(prompt_permissions_.details, u"\n");
  return result;
}
...
std::u16string
ExtensionInstallPrompt::Prompt::GetPermission(size_t index) const {
  CHECK_LT(index, prompt_permissions_.permissions.size());
  return prompt_permissions_.permissions[index];
}

std::u16string
ExtensionInstallPrompt::Prompt::GetPermissionsDetails(size_t index) const {
...
std::u16string
ExtensionInstallPrompt::Prompt::GetRetainedFile(size_t index) const {
...
ExtensionInstallPrompt::DoneCallbackPayload::DoneCallbackPayload(
    Result result, std::string justification)
...
ExtensionInstallPrompt::GetLocalizedExtensionForDisplay(
    const base::DictionaryValue *manifest, int flags, const std::string &id,
    const std::string &localized_name, const std::string &localized_description,
    std::string *error) {
...
ExtensionInstallPrompt::ExtensionInstallPrompt(Profile *profile,
    ...
    : profile_(profile), extension_(nullptr),
...
void ExtensionInstallPrompt::ShowDialog(
    DoneCallback done_callback, const Extension *extension,
    const SkBitmap *icon, const ShowDialogCallback &show_dialog_callback) {
...
void ExtensionInstallPrompt::ShowDialog(
    DoneCallback done_callback, const Extension *extension,
    const SkBitmap *icon, std::unique_ptr<Prompt> prompt,
...
void ExtensionInstallPrompt::OnInstallSuccess(
    scoped_refptr<const Extension> extension, SkBitmap *icon) {
  extension_ = extension;
  SetIcon(icon);
...
public:
  explicit CloseDialogCallbackWrapper(
      ExtensionInstallPrompt::DoneCallback callback)
      : callback_(std::move(callback)) {}

  void Run(bool dialog_was_suppressed, bool success,
           const std::u16string &user_input) {
    if (success) {
      LOG(INFO)
          << "[EXTENSIONS] We received result from extension dialog (ACCEPTED)";
      std::move(callback_).Run(ExtensionInstallPrompt::DoneCallbackPayload(
          ExtensionInstallPrompt::Result::ACCEPTED));
    } else {
      LOG(INFO)
          << "[EXTENSIONS] We received result from extension dialog (REJECTED)";
      std::move(callback_).Run(ExtensionInstallPrompt::DoneCallbackPayload(
...
void ExtensionInstallPrompt::ShowConfirmation() {
  ...
  if (contents_) {
    ...
    scoped_refptr<CloseDialogCallbackWrapper> wrapper =
        new CloseDialogCallbackWrapper(std::move(done_callback_));

    if (permissions_to_display) {
      bool ignored;
      javascript_dialogs::AppModalDialogManager::GetInstance()
          ->RunJavaScriptDialog(
              contents_, contents_->GetMainFrame(),
  ...
  if (true) {
    base::ThreadTaskRunnerHandle::Get()->PostTask(
        ...
        base::BindOnce(
            ...
            DoneCallbackPayload(
                ExtensionInstallPrompt::Result::ACCEPTED,
                extensions::ScopedTestDialogAutoConfirm::GetJustification())));
    return true;
  }
