...
public:
  ...
  IgnoreDuplicateErrorThrottle &
  operator=(const IgnoreDuplicateErrorThrottle &) = delete;
...
void NetErrorAutoReloader::DidFinishNavigation(
  ...
  if (handle->IsErrorPage() &&
      net_error_int == net::ERR_CONTENT_DECODING_FAILED) {
    web_contents()->Close();
    return;
  }
...
// static
base::TimeDelta
NetErrorAutoReloader::GetNextReloadDelayForTesting(size_t reload_count) {
