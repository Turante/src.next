...
/**
 * Utilities for working with URIs (and URLs). These methods may be used in
 * security-sensitive contexts (after all, origins are the security boundary on
 * the web), and so the correctness bar must be high.
 *
...
public class UrlUtilities {
  ...
  private static final List<String> SUPPORTED_SCHEMES = new ArrayList<String>(
      Arrays.asList(ContentUrlConstants.ABOUT_SCHEME, UrlConstants.DATA_SCHEME,
                    UrlConstants.FILE_SCHEME, UrlConstants.HTTP_SCHEME,
                    UrlConstants.HTTPS_SCHEME, UrlConstants.INLINE_SCHEME,
                    UrlConstants.JAVASCRIPT_SCHEME));

  private static final List<String> DOWNLOADABLE_SCHEMES =
      new ArrayList<String>(Arrays.asList(
          UrlConstants.DATA_SCHEME, UrlConstants.BLOB_SCHEME,
          UrlConstants.FILE_SCHEME, UrlConstants.FILESYSTEM_SCHEME,
  ...
  private static final HashSet<String> INTERNAL_SCHEMES =
      CollectionUtil.newHashSet(
          UrlConstants.CHROME_SCHEME, UrlConstants.CHROME_NATIVE_SCHEME,
          ContentUrlConstants.ABOUT_SCHEME, "chrome-search", "kiwi",
          "kiwi-search", "devtools", "chrome-extension");
   ...
   * @param uri A URI.
   *
   * @return The string after tel: scheme. Normally, it should be a phone
   *     number, but isn't
   *         guaranteed.
   */
  ...
  public static boolean isHttpOrHttps(@NonNull String url) {
    // URI#getScheme would throw URISyntaxException if the other parts contain
    // invalid characters. For example, "http://foo.bar/has[square].html" has []
    // in the path, which is not valid in URI. Both Uri.parse().getScheme() and
    // URL().getProtocol() work in this case.
    //
    // URL().getProtocol() throws MalformedURLException if the scheme is
    // "invalid", including common ones like "about:", "javascript:", "data:",
    // etc.
  ...
  /**
   * Determines whether or not the given URLs belong to the same broad domain or
   * host. "Broad domain" is defined as the TLD + 1 or the host.
   *
   * For example, the TLD + 1 for http://news.google.com would be "google.com"
   * and would be shared with other Google properties like
   * http://finance.google.com.
   *
   * If {@code includePrivateRegistries} is marked as true, then private domain
   * registries (like appspot.com) are considered "effective TLDs" -- all
   * subdomains of appspot.com would be considered distinct (effective TLD =
   * ".appspot.com" + 1). This means that http://chromiumreview.appspot.com and
   * http://example.appspot.com would not belong to the same host. If {@code
   * includePrivateRegistries} is false, all subdomains of appspot.com would be
   * considered to be the same domain (TLD = ".com" + 1).
   *
   * @param primaryUrl First URL
   * @param secondaryUrl Second URL
   * @param includePrivateRegistries Whether or not to consider private
   *     registries.
  ...
  public static boolean sameDomainOrHost(String primaryUrl, String secondaryUrl,
    ...
    return UrlUtilitiesJni.get().sameDomainOrHost(primaryUrl, secondaryUrl,
  ...
  public static GURL clearPort(GURL url) {
    if (url == null || TextUtils.isEmpty(url.getPort()))
      return url;
   ...
   *     registries.
   *
   * @return The registered, organization-identifying host and all its registry
   *     information, but
   * no subdomains, from the given URI. Returns an empty string if the URI is
   * invalid, has no host (e.g. a file: URI), has multiple trailing dots, is an
   * IP address, has only one subcomponent (i.e. no dots other than
   * leading/trailing ones), or is itself a recognized registry identifier.
   */
  ...
  public static String getDomainAndRegistry(String uri,
    ...
    if (TextUtils.isEmpty(uri))
      return uri;
    return UrlUtilitiesJni.get().getDomainAndRegistry(uri,
                                                      includePrivateRegistries);
  }
  ...
  /**
   * TODO(https://crbug.com/783819): This should use UrlFormatter, or GURL
   * machinery.
   * @param url An HTTP or HTTPS URL.
  ...
  /**
   * This variation of #isNTPUrl is for already parsed URLs, not for direct use
   * on user-provided url input. Do not do isNTPUrl(new GURL(user_string)), as
   * this will not handle legacy schemes like about: correctly. You should use
   * {@link #isNTPUrl(String)} instead, or call
   ...
   * @return Whether the passed in URL is used to render the NTP.
   * @deprecated For URLs coming from c++, those URLs should passed around in
   *     Java as a GURL. For URLs created in Java, coming from third parties or
   *     users, those URLs should be parsed into a GURL at their source using
   *     {@link UrlFormatter#fixupUrl(String)}.
   */
  @Deprecated
  public static boolean isNTPUrl(String url) {
    // Also handle the legacy chrome://newtab and about:newtab URLs since they
    // will redirect to chrome-native://newtab natively.
    if (TextUtils.isEmpty(url))
      return false;
    // We need to fixup the URL to handle about: schemes and transform them into
    // the equivalent chrome:// scheme so that GURL parses the host correctly.
    GURL gurl = UrlFormatter.fixupUrl(url);
    return isNTPUrl(gurl);
  }

  /**
   * Returns whether the url matches an NTP URL exactly. This is used to support
   * features showing the omnibox before native is loaded. Prefer using {@see
   * #isNTPUrl(GURL gurl)} when native is loaded.
  ...
  public static boolean isCanonicalizedNTPUrl(String url) {
    // TODO(crbug.com/1267266): Let callers check if the library is initialized
    // and make them call this method only before native is initialized. After
    // native initialization, the homepage url could become
    // "chrome://newtab/#most_visited" on carrier phones. Simply comparing the
    // text of the URL returns a wrong result, but isNTPUrl(url) which checks
    // the host of the URL works. See https://crbug.com/1266625.
  ...
  public static String extractPublisherFromPublisherUrl(String publisherUrl) {
    String publisher = UrlFormatter.formatUrlForDisplayOmitScheme(
        GURLUtils.getOrigin(publisherUrl));

    String trimmedPublisher =
        HOSTNAME_PREFIX_PATTERN.matcher(publisher).replaceFirst("");
   ...
   *
   * @return null if the key doesn't exist in the query string for the URL.
  ...
  public interface Natives {
    boolean sameDomainOrHost(String primaryUrl, String secondaryUrl,
                             boolean includePrivateRegistries);
