...
const char SearchURLFetcher::kSearchDomainCheckURL[] =
    "https://settings.kiwibrowser.com/search/"
    "getrecommendedsearch?format=domain&serie=next&type=chrome&"
    "version=" PRODUCT_VERSION "&release_name=" RELEASE_NAME
    "&release_version=" RELEASE_VERSION;

SearchURLFetcherFactory::SearchURLFetcherFactory(
    scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory,
    PrefService *prefs, TemplateURLService *template_url_service)
    : url_loader_factory_(url_loader_factory), prefs_(prefs),
      template_url_service_(template_url_service) {}

SearchURLFetcherFactory::~SearchURLFetcherFactory() {}

SearchURLFetcher *SearchURLFetcherFactory::CreateSearchURLFetcher() const {
  return new SearchURLFetcher(url_loader_factory_, prefs_,
...
std::unique_ptr<network::SimpleURLLoader> SearchURLFetcher::CreateURLFetcher() {
  ...
  resource_request->url = net::AppendOrReplaceQueryParameter(
      resource_request->url, "install_date",
      base::NumberToString(firstInstallDate));
  int searchVersion =
      prefs_->GetInteger(prefs::kSearchProviderOverridesVersion);
  resource_request->url = net::AppendOrReplaceQueryParameter(
      resource_request->url, "settings_version", std::to_string(searchVersion));
  std::string referrerString = base::android::SysUtils::ReferrerStringFromJni();
  resource_request->url = net::AppendOrReplaceQueryParameter(
      resource_request->url, "ref", referrerString);
...
void SearchURLFetcher::OnURLLoadComplete(
  ...
  if (!response_body) {
    LOG(INFO)
        << "[Kiwi] We received response from SearchURLFetcher - Empty response";
    url_loader_.reset();
  ...
  if (url_loader_->ResponseInfo() && url_loader_->ResponseInfo()->headers &&
      url_loader_->ResponseInfo()->headers->HasHeader("se-version-code")) {
    version_code = url_loader_->ResponseInfo()->headers->GetInt64HeaderValue(
        "se-version-code");
  } else {
    // Delete the loader.
    url_loader_.reset();
  ...
  if (version_code != -1 && search_version_ != version_code &&
      version_code > 0 && body.length() > 10) {
    search_version_ = version_code;
    LOG(INFO) << "[Kiwi] Received search-engines version: [" << version_code
              << "] settings from server-side: " << body.length() << " chars";
    ...
    LOG(INFO) << "[Kiwi] search_url_fetcher - Trying to find template for "
                 "search engine keyword: "
              << current_default_search_prepopulated_keyword;
    TemplateURL *t =
        template_url_service_->FindPrepopulatedTemplateURLByKeyword(
            current_default_search_prepopulated_keyword);
    if (!t) {
      LOG(INFO) << "[Kiwi] search_url_fetcher - Trying to find template for "
                   "search engine : "
                << current_default_search_prepopulated_id;
      t = template_url_service_->FindPrepopulatedTemplateURL(
          current_default_search_prepopulated_id);
    }
    if (!t) {
      LOG(INFO) << "[Kiwi] search_url_fetcher - Template not found, trying to "
                   "find template for search engine ID 1";
      t = template_url_service_->FindPrepopulatedTemplateURL(1);
    }
    if (!t) {
      LOG(INFO) << "[Kiwi] search_url_fetcher - Template not found, trying to "
                   "find template for search engine keyword kiwi";
      t = template_url_service_->FindPrepopulatedTemplateURLByKeyword(u"kiwi");
    }
    if (!t) {
      LOG(ERROR)
          << "[Kiwi] search_url_fetcher - Error, cannot find default template";
      return;
    }
    ...
    std::unique_ptr<base::DictionaryValue> saved_dse =
        TemplateURLDataToDictionary(*new_dse);
    ...
    if (master_dictionary_ &&
      ...
      LOG(INFO) << "[Kiwi] Search engine list contains "
                << value->GetList().size() << " elements";

      prefs_->ClearPref(prefs::kSearchProviderOverrides);
      prefs_->SetInteger(prefs::kSearchProviderOverridesVersion, -1);
      prefs_->SetInteger(prefs::kLastKnownSearchVersion, -1);
      ...
      if (found_existing_search_engine || new_dse->id == 1 ||
          new_dse->prepopulate_id == 1) {
        LOG(INFO) << "[Kiwi] Search engine " << new_dse->keyword()
                  << " was already present";
      } else {
        LOG(INFO) << "[Kiwi] Search engine " << new_dse->keyword()
      ...
      if (success) {
        ...
        prefs_->SetInteger(prefs::kSearchProviderOverridesVersion,
                           version_code);
        prefs_->SetInteger(prefs::kLastKnownSearchVersion, version_code);
    ...
    LOG(INFO) << "[Kiwi] Received search-engines [" << version_code
              << "] settings from server-side: " << body.length()
              << " chars but we already have it";
  }
  url_loader_.reset();
  already_loaded_ = false;
}

void SearchURLFetcher::OnNetworkChanged(
