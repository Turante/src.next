...
std::u16string
AppModalDialogManager::GetTitle(content::WebContents *web_contents,
                                const url::Origin &alerting_frame_origin) {
...
// static
std::u16string
AppModalDialogManager::GetTitleImpl(const url::Origin &main_frame_origin,
                                    const url::Origin &alerting_frame_origin) {
...
void AppModalDialogManager::RunJavaScriptDialog(
    ...
    const std::u16string &default_prompt_text, DialogClosedCallback callback,
  ...
  *did_suppress_message = false;
  ...
  if (unwrapped_parent_frame_url.SchemeIs("chrome-extension") ||
      unwrapped_alerting_frame_url.SchemeIs("chrome-extension")) {
    *did_suppress_message = true;
    return;
  }
...
void AppModalDialogManager::RunBeforeUnloadDialog(
    content::WebContents *web_contents,
    content::RenderFrameHost *render_frame_host, bool is_reload,
...
void AppModalDialogManager::RunBeforeUnloadDialogWithOptions(
    content::WebContents *web_contents,
    content::RenderFrameHost *render_frame_host, bool is_reload, bool is_app,
    DialogClosedCallback callback) {
  ChromeJavaScriptDialogExtraData *extra_data =
      &javascript_dialog_extra_data_[web_contents];
...
bool AppModalDialogManager::HandleJavaScriptDialog(
    content::WebContents *web_contents, bool accept,
