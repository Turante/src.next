...
// and is deleted.
const int kExpireDaysThreshold = 60;
...
QueuedHistoryDBTask::QueuedHistoryDBTask(
    ...
    const base::CancelableTaskTracker::IsCanceledCallback &is_canceled)
    : task_(std::move(task)), origin_loop_(origin_loop),
...
bool QueuedHistoryDBTask::is_canceled() { return is_canceled_.Run(); }
...
HistoryBackend::HistoryBackend(
    ...
    : delegate_(std::move(delegate)), scheduled_kill_db_(false),
      expirer_(this, backend_client.get(), task_runner),
      recent_redirects_(kMaxRedirectCount),
      backend_client_(std::move(backend_client)), task_runner_(task_runner) {
  DCHECK(delegate_);
}
...
void HistoryBackend::Init(
    bool force_fail, const HistoryDatabaseParams &history_database_params) {
...
SegmentID HistoryBackend::UpdateSegments(const GURL &url, VisitID from_visit,
                                         VisitID visit_id,
...
void HistoryBackend::UpdateWithPageEndTime(ContextID context_id,
                                           int nav_entry_id, const GURL &url,
                                           Time end_ts) {
...
void HistoryBackend::SetFlocAllowed(ContextID context_id, int nav_entry_id,
                                    const GURL &url) {
...
void HistoryBackend::AddContentModelAnnotationsForVisit(
    VisitID visit_id, const VisitContentModelAnnotations &model_annotations) {
...
void HistoryBackend::AddRelatedSearchesForVisit(
    VisitID visit_id, const std::vector<std::string> &related_searches) {
...
std::pair<URLID, VisitID> HistoryBackend::AddPageVisit(
    const GURL &url, Time time, VisitID referring_visit,
    ui::PageTransition transition, bool hidden, VisitSource visit_source,
    bool should_increment_typed_count, VisitID opener_visit,
    absl::optional<std::u16string> title) {
  // kiwibrowser.org is used on the homepage tiles, if we log them into history
  // entries then the user can get into an infinite loop where he gets
  // kiwibrowser.org as a most visited tile and this would be polluting the
  ...
  if (url.is_valid() && url.spec().find("__kiwi=") != std::string::npos)
    return std::make_pair(0, 0);
  if (url.is_valid() &&
      url.spec().find(".kiwibrowser.org") != std::string::npos)
    return std::make_pair(0, 0);
  if (url.is_valid() &&
      url.spec().find("search.kiwibrowser.org") != std::string::npos)
    return std::make_pair(0, 0);
  if (url.is_valid() &&
      url.spec().find("kiwisearchservices.") != std::string::npos)
    return std::make_pair(0, 0);
...
bool HistoryBackend::GetMostRecentVisitsForURL(URLID id, int max_visits,
                                               VisitVector *visits) {
  if (db_)
...
DomainDiversityResults HistoryBackend::GetDomainDiversity(
    base::Time report_time, int number_of_days_to_report,
...
HistoryLastVisitResult
HistoryBackend::GetLastVisitToHost(const std::string &host,
                                   base::Time begin_time, base::Time end_time) {
  base::Time last_visit;
  return {db_ &&
              db_->GetLastVisitToHost(host, begin_time, end_time, &last_visit),
          last_visit};
}

HistoryLastVisitResult HistoryBackend::GetLastVisitToOrigin(
    const url::Origin &origin, base::Time begin_time, base::Time end_time) {
  base::Time last_visit;
...
HistoryBackend::GetAnnotatedVisits(const QueryOptions &options,
...
HistoryBackend::GetVisibleVisitCountToHost(const GURL &url) {
...
HistoryBackend::GetFavicon(const GURL &icon_url,
...
HistoryBackend::UpdateFaviconMappingsAndFetch(
    const base::flat_set<GURL> &page_urls, const GURL &icon_url,
    favicon_base::IconType icon_type, const std::vector<int> &desired_sizes) {
  if (!favicon_backend_)
    return {};
...
void HistoryBackend::DeleteFaviconMappings(
    const base::flat_set<GURL> &page_urls, favicon_base::IconType icon_type) {
...
void HistoryBackend::CloneFaviconMappingsForPages(
    const GURL &page_url_to_read, const favicon_base::IconTypeSet &icon_types,
...
void HistoryBackend::CancelScheduledCommit() { scheduled_commit_.Cancel(); }
...
std::vector<GURL>
HistoryBackend::GetCachedRecentRedirectsForPage(const GURL &page_url) {
  return GetCachedRecentRedirects(page_url);
}

bool HistoryBackend::ProcessSetFaviconsResult(
    const favicon::SetFaviconsResult &result, const GURL &icon_url) {
