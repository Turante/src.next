...
base::Value CookieInclusionStatusNetLogParams(
    const std::string &operation, const std::string &cookie_name,
    const std::string &cookie_domain, const std::string &cookie_path,
...
net::CookieOptions
CreateCookieOptions(net::CookieOptions::SameSiteCookieContext same_site_context,
...
URLRequestHttpJob::URLRequestHttpJob(
    URLRequest *request, const HttpUserAgentSettings *http_user_agent_settings)
    : URLRequestJob(request), num_cookie_lines_left_(0),
      priority_(DEFAULT_PRIORITY), response_info_(nullptr),
      proxy_auth_state_(AUTH_STATE_DONT_NEED_AUTH),
      server_auth_state_(AUTH_STATE_DONT_NEED_AUTH), read_in_progress_(false),
      throttling_entry_(nullptr), done_(false), awaiting_callback_(false),
...
void URLRequestHttpJob::Start() {
  ...
  if (request_info_.url.host().find("addons.opera.com") != std::string::npos) {
    request_info_.extra_headers.SetHeader(
        ...
        "Gecko) Chrome/73.0.3683.86 Safari/537.36 OPR/60.0.3255.27 (Edition "
        "developer)");
  } else if (request_info_.url.host().find("chrome.google.com") !=
             std::string::npos) {
    request_info_.extra_headers.SetHeader(
        HttpRequestHeaders::kUserAgent,
        "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like "
        "Gecko) Chrome/93.0.4577.25 Safari/537.36");
  } else if (request_info_.url.host().find("web.whatsapp.com") !=
             std::string::npos) {
    request_info_.extra_headers.SetHeader(
        HttpRequestHeaders::kUserAgent,
        "Mozilla/5.0 (X11; Linux) AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/99.0.4844.66 Mobile Safari/537.36");
  } else if (request_info_.url.host().find("messenger.com") !=
             std::string::npos) {
    request_info_.extra_headers.SetHeader(
        HttpRequestHeaders::kUserAgent,
        "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:57.0) Gecko/20100101 "
        "Firefox/57.0");
  } else if (request_info_.url.host().find("roninchain.com") !=
                 std::string::npos ||
             request_info_.url.host().find("marketplace.axieinfinity.com") !=
                 std::string::npos) {
    request_info_.extra_headers.SetHeader(
        HttpRequestHeaders::kUserAgent,
  ...
  } else if (request_info_.url.host().find("facebook.com") !=
             std::string::npos) {
    request_info_.extra_headers.SetHeader(
        HttpRequestHeaders::kUserAgent,
        "Mozilla/5.0 (Linux; Android 3.0) AppleWebKit/537.36 (KHTML, like "
        "Gecko) Chrome/99.0.4844.66 Mobile Safari/537.36");
  } else if (request_info_.url.host().find("news.google.com") !=
                 std::string::npos &&
             request_info_.url.path().find(
                 "CAAqJggKIiBDQkFTRWdvSUwyMHZNRFZxYUdjU0FtVnVHZ0pWVXlnQVAB") !=
                 std::string::npos) {
    request_info_.extra_headers.SetHeader(
        HttpRequestHeaders::kUserAgent,
        "Mozilla/5.0 (Linux; Android 9; ONEPLUS A6003) AppleWebKit/537.36 "
        "(KHTML, like Gecko) Chrome/71.0.3578.99 Mobile Safari/537.36");
    request_info_.extra_headers.SetHeader(
        "Cookie", "CONSENT=YES+srp.gws-20210610-0-RC2.en+FX+320;");
  }

  if (request_info_.url.host().find("washingtonpost.com") !=
      std::string::npos) {
    request_info_.extra_headers.SetHeader("X-Forwarded-For", "1.1.1.1");
  }

  if (request_info_.url.host().find("amazon") != std::string::npos &&
      request_info_.url.query().find("kbdirect") != std::string::npos) {
    request_info_.extra_headers.RemoveHeader(HttpRequestHeaders::kReferer);
  }
...
void URLRequestHttpJob::NotifyBeforeStartTransactionCallback(
    int result, const absl::optional<HttpRequestHeaders> &headers) {
...
void URLRequestHttpJob::OnSetCookieResult(
    const CookieOptions &options, absl::optional<CanonicalCookie> cookie,
    std::string cookie_string, CookieAccessResult access_result) {
  if (request_->net_log().IsCapturing()) {
