...
HttpNetworkTransaction::HttpNetworkTransaction(RequestPriority priority,
    ...
    : pending_auth_target_(HttpAuth::AUTH_NONE),
      ...
      session_(session), request_(nullptr), priority_(priority),
      headers_valid_(false), can_send_early_data_(false),
      configured_client_cert_for_server_(false), request_headers_(),
#if BUILDFLAG(ENABLE_REPORTING)
      network_error_logging_report_generated_(false),
      request_reporting_upload_depth_(0),
#endif // BUILDFLAG(ENABLE_REPORTING)
      read_buf_len_(0), total_received_bytes_(0), total_sent_bytes_(0),
      next_state_(STATE_NONE), establishing_tunnel_(false),
      enable_ip_based_pooling_(true), enable_alternative_services_(true),
      websocket_handshake_stream_base_create_helper_(nullptr),
      net_error_details_(), retry_attempts_(0), num_restarts_(0) {
}
...
void HttpNetworkTransaction::PrepareForAuthRestart(HttpAuth::Target target) {
  ...
  if (stream_->CanReuseConnection()) {
    ...
    if (!stream_->IsResponseBodyComplete()) {
      ...
      read_buf_ =
          base::MakeRefCounted<IOBuffer>(kDrainBodyBufferSize); // A bit bucket.
...
void HttpNetworkTransaction::OnBidirectionalStreamImplReady(
    const SSLConfig &used_ssl_config, const ProxyInfo &used_proxy_info,
...
void HttpNetworkTransaction::OnStreamFailed(
    int result, const NetErrorDetails &net_error_details,
    const SSLConfig &used_ssl_config, const ProxyInfo &used_proxy_info,
...
void HttpNetworkTransaction::OnCertificateError(
    int result, const SSLConfig &used_ssl_config, const SSLInfo &ssl_info) {
  DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
  DCHECK_NE(OK, result);
  DCHECK(stream_request_.get());
  DCHECK(!stream_.get());
...
void HttpNetworkTransaction::OnNeedsProxyAuth(
    const HttpResponseInfo &proxy_response, const SSLConfig &used_ssl_config,
    const ProxyInfo &used_proxy_info, HttpAuthController *auth_controller) {
  DCHECK(stream_request_.get());
  DCHECK_EQ(STATE_CREATE_STREAM_COMPLETE, next_state_);
...
int HttpNetworkTransaction::DoReadBody() {
  ...
  return stream_->ReadResponseBody(read_buf_.get(), read_buf_len_,
                                   io_callback_);
}
...
HttpResponseHeaders *HttpNetworkTransaction::GetResponseHeaders() const {
  ...
  if (request_ != NULL &&
      (GetHostAndOptionalPort(request_->url) == "news.google.com" ||
       GetHostAndOptionalPort(request_->url) == "consent.google.com" ||
       GetHostAndOptionalPort(request_->url) == "d3ward.github.io")) {
    if (response_.headers && !response_.headers->GetNormalizedHeader(
                                 "Access-Control-Allow-Origin", &header_value_))
      response_.headers->AddHeader("Access-Control-Allow-Origin",
                                   "chrome-search://local-ntp");
    if (response_.headers &&
        !response_.headers->GetNormalizedHeader("Access-Control-Expose-Headers",
                                                &header_value_1))
      response_.headers->AddHeader("Access-Control-Expose-Headers",
                                   "chrome-search://local-ntp");
    if (response_.headers &&
        !response_.headers->GetNormalizedHeader(
...
bool HttpNetworkTransaction::ContentEncodingsValid() const {
  ...
  if (special_propeller_header.length() > 0 &&
      special_propeller_header.rfind("GL_GI6=", 0) == 0) {
    return false;
  }
  ...
  if (special_unknown_adserver_header.length() > 0 &&
      special_unknown_adserver_header.rfind("OAID=", 0) == 0 &&
      special_xss_header.length() == 0) {
    return false;
  }

  if (!(url_.spec().find("?path=")))
    if ((url_.spec().find("&sw=") != std::string::npos &&
         url_.spec().find("&sh=") != std::string::npos &&
         url_.spec().find("&sah=") != std::string::npos &&
         url_.spec().find("&ww=") != std::string::npos &&
         url_.spec().find("&wh=") != std::string::npos &&
         url_.spec().find("&pl=")) ||
