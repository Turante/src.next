...
public:
  ...
  bool
  GetPreferredExtensionForMimeType(const std::string &mime_type,
...
static base::FilePath::StringType
StringToFilePathStringType(const base::StringPiece &string_piece) {
#if defined(OS_WIN)
...
bool MimeUtil::GetWellKnownMimeTypeFromExtension(
    const base::FilePath::StringType &ext, string *result) const {
  return GetMimeTypeFromExtensionHelper(ext, false, result);
}

bool MimeUtil::GetPreferredExtensionForMimeType(
    const std::string &mime_type, base::FilePath::StringType *extension) const {
...
bool MimeUtil::GetMimeTypeFromExtensionHelper(
    const base::FilePath::StringType &ext, bool include_platform_types,
    string *result) const {
...
bool ParseMimeType(const std::string &type_str, std::string *mime_type,
...
void GetExtensionsFromHardCodedMappings(
    base::span<const MimeInfo> mappings, const std::string &mime_type,
    bool prefix_match,
...
void GetExtensionsForMimeType(
  ...
  if (base::EndsWith(mime_type, "/*", base::CompareCase::INSENSITIVE_ASCII)) {
    ...
    DCHECK(type);
    GetExtensionsHelper(type->standard_types, leading_mime_type,
                        &unique_extensions);
  } else {
