...
public class SelectionPopupControllerImpl extends ActionModeCallbackHelper
    implements ImeEventObserver, SelectionPopupController, WindowEventObserver,
               HideablePopup, ContainerViewObserver, UserData {
  private static final String TAG = "SelectionPopupCtlr"; // 20 char limit
  private static final boolean DEBUG = false;

  /**
   * Android Intent size limitations prevent sending over a megabyte of data.
   * Limit query lengths to 100kB because other things may be added to the
   * Intent.
   */
  ...
  // The readback view could be the ContainerView, which WindowAndroid has no
  // control on that. Embedders should set this properly to use the correct view
  // for readback.
  private static boolean sShouldGetReadbackViewFromWindowAndroid;

  // A flag to determine if we must only use the context from the associated web
  // contents to inflate menus. By default we use the context held by the
  // ActionMode, because this enables correct theming, but in cases where we
  // rely on the wrapping of contexts for correct resource lookup, this is not
  // correct. In that case we must directly inflate menus from the context.
  private static boolean sMustUseWebContentsContext;

  private static final class UserDataFactoryLazyHolder {
    private static final UserDataFactory<SelectionPopupControllerImpl>
        INSTANCE = SelectionPopupControllerImpl::new;
  }
  ...
  /**
   * The {@link SelectionClient} that processes textual selection, or {@code
   * null} if none exists.
   */
  ...
  /**
   * The {@link SelectionInsertionHandleObserver} that processes handle events,
   ...
   * @return {@link SelectionPopupController} object. {@code null} if not
   *     available because
   *         {@link #create()} is not called yet.
   */
  public static SelectionPopupControllerImpl
  fromWebContents(WebContents webContents) {
    return ((WebContentsImpl)webContents)
        .getOrSetUserData(SelectionPopupControllerImpl.class,
                          UserDataFactoryLazyHolder.INSTANCE);
  }

  /**
   * Create {@link SelectionPopupController} instance. Note that it will create
   * an instance with no link to native side for testing only.
   * @param webContents {@link WebContents} mocked for testing.
   * @param popupController {@link PopupController} mocked for testing.
   */
  public static SelectionPopupControllerImpl
  createForTesting(WebContents webContents, PopupController popupController) {
    return new SelectionPopupControllerImpl(webContents, popupController,
                                            false);
  }

  public static SelectionPopupControllerImpl
  createForTesting(WebContents webContents) {
  ...
  private SelectionPopupControllerImpl(WebContents webContents,
    ...
    WindowEventObserverManager manager =
    ...
    if (initializeNative) {
      mNativeSelectionPopupController =
          SelectionPopupControllerImplJni.get().init(
    ...
    mLastSelectedText = "";
    initHandleObserver();
    mAdditionalMenuItemProvider =
        ContentClassFactory.get().createAddtionalMenuItemProvider();
   ...
   */
  public SelectionClient getSelectionClient() { return mSelectionClient; }
  ...
  public void showSelectionMenu(int left, int top, int right, int bottom,
                                int handleHeight, boolean isEditable,
                                boolean isPasswordType, String selectionText,
                                int selectionStartOffset, boolean canSelectAll,
                                boolean canRichlyEdit, boolean shouldSuggest,
                                @MenuSourceType int sourceType,
                                RenderFrameHost renderFrameHost) {
    int offsetBottom = bottom;
    // Legacy action mode expects the selection rectangle not to include touch
    // handle.
    ...
    if (hasSelection()) {
      ...
      if (mSmartSelectionEventProcessor != null) {
        ...
        case MenuSourceType.MENU_SOURCE_ADJUST_SELECTION_RESET:
          mSmartSelectionEventProcessor.onSelectionAction(
              mLastSelectedText, mLastSelectionOffset,
      ...
      // Note that this won't happen if it is incognito mode or device is not
      // provisioned.
  ...
  public void showActionModeOrClearOnFailure() {
    ...
    // Just refresh non-floating action mode if it already exists to avoid
    // blinking.
    ...
    if (actionMode != null) {
      // This is to work around an LGE email issue. See crbug.com/651706 for
      // more details.
  ...
  private ActionMode startFloatingActionMode() {
    ...
    ActionMode actionMode =
        ContentApiHelperForM.startActionMode(mView, this, mCallback);
    return actionMode;
  }
  ...
  private void showContextMenuAtTouchHandle(int left, int bottom) {
    if (mWebContents.getRenderWidgetHostView() != null) {
      mWebContents.getRenderWidgetHostView().showContextMenuAtTouchHandle(
          left, bottom);
    }
  }
  ...
  private void createAndShowPastePopup() {
    ...
    if (supportsFloatingActionMode()) {
      ...
      mPastePopupMenu =
          new LegacyPastePopupMenu(windowContext, mView, delegate);
    }
    showPastePopup();
  }
  ...
  public void finishActionMode() {
    ...
    if (isActionModeValid()) {
      ...
      // Should be nulled out in case #onDestroyActionMode() is not invoked in
      // response.
      mActionMode = null;
    }
  }
  ...
  private void hideActionMode(boolean hide) {
    ...
    if (mHidden) {
      ...
      mHandler.removeCallbacks(mRepeatingHideRunnable);
      // To show the action mode that is being hidden call hide() again with a
      // short delay.
  ...
  private void hideActionModeTemporarily(long duration) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      if (isActionModeValid())
        ApiHelperForM.hideActionMode(mActionMode, duration);
    }
  }

  private boolean isFloatingActionMode() {
    return supportsFloatingActionMode() && isActionModeValid() &&
        ApiHelperForM.getActionModeType(mActionMode) ==
  ...
  /**
   * Initialize the menu by populating all the available items. Embedders should
   * remove the items that are not relevant to the input text being edited.
   */
  public static void initializeMenu(Context context, ActionMode mode,
                                    Menu menu) {
    if (!sMustUseWebContentsContext) {
      // For WebView the correct choice is to use the actionMode context because
      // webview assets have been added to its asset path. However we need to
      // fall back to using the web contents context in the case where the
      // AssetManager associated with the actionMode context does not contain
      // our assets, because not doing so will cause a crash.
      try {
   ...
   * "paste as plain text" option needs clibpoard content is rich text, and
   * editor supports rich text as well.
   */
  @VisibleForTesting
  public boolean canPasteAsPlainText() {
    ...
    // We need to show "paste as plain text" when Clipboard contains the HTML
    // text. In addition to that, on Android, Spanned could be copied to
    // Clipboard as plain_text MIME type, but in some cases, Spanned could have
    // text format, we need to show "paste as plain text" when that happens as
    // well.
  ...
  private void updateAssistMenuItem(Menu menu) {
    ...
    if (mClassificationResult != null &&
        mClassificationResult.hasNamedAction()) {
  ...
  public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
    // Actions should only happen when there is a WindowAndroid so mView should
    // not be null.
    assert mView != null;
    if (!isActionModeValid())
      return true;
    ...
    if (hasSelection() && mSmartSelectionEventProcessor != null) {
      mSmartSelectionEventProcessor.onSelectionAction(
          mLastSelectedText, mLastSelectionOffset, getActionType(id, groupId),
          mClassificationResult);
    }

    if (groupId == R.id.select_action_menu_assist_items &&
        id == android.R.id.textAssist) {
      doAssistAction();
      mode.finish();
  ...
  /**
   * Called when an ActionMode needs to be positioned on screen, potentially
   * occluding view content. Note this may be called on a per-frame basis.
   *
   * @param mode The ActionMode that requires positioning.
   * @param view The View that originated the ActionMode, in whose coordinates
   *     the Rect should
   *             be provided.
  ...
  private Rect getSelectionRectRelativeToContainingView() {
    ...
    Rect viewSelectionRect =
        new Rect((int)(mSelectionRect.left * deviceScale),
                 (int)(mSelectionRect.top * deviceScale),
                 (int)(mSelectionRect.right * deviceScale),
                 (int)(mSelectionRect.bottom * deviceScale));

    // The selection coordinates are relative to the content viewport, but we
    // need coordinates relative to the containing View.
  ...
  public void selectAll() {
    ...
    // Even though the above statement logged a SelectAll user action, we want
    // to track whether the focus was in an editable field, so log that too.
    if (isFocusedNodeEditable()) {
  ...
  public void share() {
    ...
    try {
      Intent i = Intent.createChooser(
          send, mContext.getString(R.string.actionbar_share));
   ...
   */
  private boolean isIncognito() { return mWebContents.isIncognito(); }

  /**
   * @param actionModeItem the flag for the action mode item in question. The
   *     valid flags are
  ...
  public void onReceivedProcessTextResult(int resultCode, Intent data) {
    if (mWebContents == null || resultCode != Activity.RESULT_OK ||
        data == null)
      return;

    // Do not handle the result if no text is selected or current selection is
    // not editable.
    if (!hasSelection() || !isFocusedNodeEditable())
      return;
  ...
  private void setTextHandlesTemporarilyHidden(boolean hide) {
    if (mNativeSelectionPopupController == 0)
      return;
  ...
  @VisibleForTesting
  @CalledByNative
  void onSelectionEvent(@SelectionEventType int eventType, int left, int top,
                        int right, int bottom) {
    if (DEBUG) {
      Log.i(TAG, "onSelectionEvent: " + eventType + "[(" + left + ", " + top +
                     ")-(" + right + ", " + bottom + ")]");
    }
    // Ensure the provided selection coordinates form a non-empty rect, as
    // required by the selection action mode. NOTE: the native side ensures the
    // rectangle is not empty, but that's done using floating point, which means
    // it's entirely possible for this code to receive an empty rect.
    if (left == right)
      ++right;
    if (top == bottom)
      ++bottom;
    ...
    case SelectionEventType.SELECTION_HANDLES_CLEARED:
      ...
      if (mSelectionClient != null)
        mSelectionClient.cancelAllRequests();
    ...
    case SelectionEventType.INSERTION_HANDLE_TAPPED:
      if (mWasPastePopupShowingOnInsertionDragStart) {
        ...
        showContextMenuAtTouchHandle(mSelectionRect.left,
  ...
  @VisibleForTesting
  @CalledByNative
  /* package */ void onDragUpdate(@TouchSelectionDraggableType int type,
                                  float x, float y) {
    // If this is for longpress drag selector, we can only have mangifier on S
    // and above.
    ...
    if (mHandleObserver != null) {
      ...
      x *= deviceScale;
      // The selection coordinates are relative to the content viewport, but we
      // need coordinates relative to the containing View, so adding
  ...
  public void clearSelection() {
    if (mWebContents == null || !isActionModeSupported())
      return;
  ...
  /**
   * Sets the client that implements selection augmenting functionality, or null
   * if none exists.
   */
  @Override
  ...
  @CalledByNative
  private void onSelectWordAroundCaretAck(boolean didSelect, int startAdjust,
                                          int endAdjust) {
    if (mSelectionClient != null) {
      mSelectionClient.selectWordAroundCaretAck(didSelect, startAdjust,
                                                endAdjust);
    }
  }
  ...
  public void updateSelectionState(boolean editable, boolean isPassword) {
    if (!editable)
      destroyPastePopup();
    if (editable != isFocusedNodeEditable() ||
        isPassword != isSelectionPassword()) {
      mEditable = editable;
  ...
  private class SmartSelectionCallback
    ...
    public void onClassified(SelectionClient.Result result) {
      ...
      // Do not allow classifier to shorten the selection. If the suggested
      // selection is smaller than the original we throw away classification
      // result and show the menu.
      // TODO(amaralp): This was added to fix the SelectAll problem in
      // http://crbug.com/714106. Once we know the cause of the original
      // selection we can remove this check.
      ...
      // The classificationresult is a property of the selection. Keep it even
      // the action mode has been dismissed.
      ...
      // We won't do expansion here, however, we want to 1) for starting a new
      // logging session, log non selection expansion event to match the
      // behavior of expansion case. 2) log selection handle dragging triggered
      // selection change.
      if (mSmartSelectionEventProcessor != null) {
        mSmartSelectionEventProcessor.onSelectionModified(
            mLastSelectedText, mLastSelectionOffset, mClassificationResult);
  ...
  public void setTextClassifier(TextClassifier textClassifier) {
    ...
    if (client != null)
      client.setTextClassifier(textClassifier);
  }
