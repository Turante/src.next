...
bool UrlMatchesGlobs(const std::vector<std::string> *globs, const GURL &url) {
...
enum {
  kValidUserScriptSchemes =
      URLPattern::SCHEME_CHROMEUI | URLPattern::SCHEME_CHROMESEARCH |
...
UserScript::File::File(const base::FilePath &extension_root,
                       const base::FilePath &relative_path, const GURL &url)
...
UserScript::File::File(const File &other)
    : extension_root_(other.extension_root_),
      relative_path_(other.relative_path_), url_(other.url_) {}
...
UserScript::CopyMetadataFrom(const UserScript &other) {
