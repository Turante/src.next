...
public class SysUtils {
  // A device reporting strictly more total memory in megabytes cannot be
  // considered 'low-end'.
  ...
  // A device reporting more disk capacity in gigabytes than this is considered
  // high end.
        ...
        try {
          ...
          for (;;) {
            ...
            if (totalMemoryKB <= 1024) {
              Log.w(TAG,
                    "Invalid /proc/meminfo total size in kB: " + m.group(1));
              break;
            }
  ...
  private static boolean detectLowEndDevice() {
    ...
    // If this logic changes, update the comments above
    // base::SysUtils::IsLowEndDevice.
    sAmountOfPhysicalMemoryKB = detectAmountOfPhysicalMemoryKB();
    boolean isLowEnd = true;
    if (sAmountOfPhysicalMemoryKB <= 0) {
      isLowEnd = false;
    } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      isLowEnd = sAmountOfPhysicalMemoryKB / 1024 <=
                 ANDROID_O_LOW_MEMORY_DEVICE_THRESHOLD_MB;
    } else {
      isLowEnd = sAmountOfPhysicalMemoryKB / 1024 <=
                 ANDROID_LOW_MEMORY_DEVICE_THRESHOLD_MB;
    }

    // For evaluation purposes check whether our computation agrees with Android
    // API value.
    Context appContext = ContextUtils.getApplicationContext();
    boolean isLowRam = false;
    if (appContext != null &&
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
      isLowRam = ((ActivityManager)ContextUtils.getApplicationContext()
                      .getSystemService(Context.ACTIVITY_SERVICE))
                     .isLowRamDevice();
    }
    RecordHistogram.recordBooleanHistogram("Android.SysUtilsLowEndMatches",
                                           isLowEnd == isLowRam);
  ...
  /**
   * Creates a new trace event to log the number of minor / major page faults,
  ...
  /**
   * @return Whether or not this device should be considered a high end device
   *     from a disk
  ...
  @VisibleForTesting
  public static void
  setAmountOfPhysicalMemoryKBForTesting(int physicalMemoryKB) {
    sAmountOfPhysicalMemoryKB = physicalMemoryKB;
  }

  /**
   * @return Whether this device is running Android Go. This is assumed when
   *     we're running Android
