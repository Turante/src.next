...
void BoxPainterBase::PaintInsetBoxShadowWithBorderRect(
    const PaintInfo &info, const PhysicalRect &border_rect,
    const ComputedStyle &style, PhysicalBoxSides sides_to_include) {
  if (!style.BoxShadow() || style.DarkColorScheme())
    return;
...
void BoxPainterBase::PaintInsetBoxShadowWithInnerRect(
    const PaintInfo &info, const PhysicalRect &inner_rect,
    const ComputedStyle &style) {
  if (!style.BoxShadow() || style.DarkColorScheme())
    return;
...
bool BoxPainterBase::ShouldForceWhiteBackgroundForPrintEconomy(
    const Document &document, const ComputedStyle &style) {
...
BoxPainterBase::FillLayerInfo::FillLayerInfo(
    const Document &doc, const ComputedStyle &style, bool is_scroll_container,
    Color bg_color, const FillLayer &layer,
    BackgroundBleedAvoidance bleed_avoidance,
    RespectImageOrientationEnum respect_image_orientation,
    PhysicalBoxSides sides_to_include, bool is_inline,
    bool is_painting_background_in_contents_space)
    : image(layer.GetImage()), color(bg_color),
      respect_image_orientation(respect_image_orientation),
      sides_to_include(sides_to_include), is_bottom_layer(!layer.Next()),
...
absl::optional<gfx::RectF>
OptimizeToSingleTileDraw(const BackgroundImageGeometry &geometry,
                         const PhysicalRect &dest_rect, Image *image,
...
//   sprite computation.
void DrawTiledBackground(GraphicsContext &context, Image *image,
                         const BackgroundImageGeometry &geometry,
                         SkBlendMode op, const AutoDarkMode &auto_dark_mode,
                         RespectImageOrientationEnum respect_orientation) {
...
bool PaintBGColorWithPaintWorklet(const Document *document,
                                  ...
                                  Node *node, const ComputedStyle &style,
...
void DidDrawImage(
    Node *node, const Image &image, const StyleImage &style_image,
...
inline bool PaintFastBottomLayer(
    const Document *document, Node *node, const ComputedStyle &style,
    GraphicsContext &context, const BoxPainterBase::FillLayerInfo &info,
    const PhysicalRect &rect, const FloatRoundedRect &border_rect,
    BackgroundImageGeometry &geometry, Image *image, SkBlendMode composite_op) {
...
FloatRoundedRect BackgroundRoundedRectAdjustedForBleedAvoidance(
    const ComputedStyle &style, const PhysicalRect &border_rect,
    bool object_has_multiple_boxes, PhysicalBoxSides sides_to_include,
...
FloatRoundedRect RoundedBorderRectForClip(
    const ComputedStyle &style, const BoxPainterBase::FillLayerInfo &info,
    const FillLayer &bg_layer, const PhysicalRect &rect,
    bool object_has_multiple_boxes, const PhysicalSize &flow_box_size,
    BackgroundBleedAvoidance bleed_avoidance,
...
void PaintFillLayerBackground(const Document *document,
                              ...
                              const BoxPainterBase::FillLayerInfo &info,
                              Node *node, const ComputedStyle &style,
                              Image *image, SkBlendMode composite_op,
                              const BackgroundImageGeometry &geometry,
...
LayoutRectOutsets
AdjustOutsetsForEdgeInclusion(const LayoutRectOutsets outsets,
                              const BoxPainterBase::FillLayerInfo &info) {
...
LayoutRectOutsets
BoxPainterBase::AdjustedBorderOutsets(const FillLayerInfo &info) const {
  return AdjustOutsetsForEdgeInclusion(ComputeSnappedBorders(), info);
}

void BoxPainterBase::PaintFillLayer(
    const PaintInfo &paint_info, const Color &color, const FillLayer &bg_layer,
    const PhysicalRect &rect, BackgroundBleedAvoidance bleed_avoidance,
    BackgroundImageGeometry &geometry, bool object_has_multiple_boxes,
  ...
  case EFillBox::kContent: {
    ...
    clip_rect.Contract(AdjustOutsetsForEdgeInclusion(border, fill_layer_info));
    if (bg_layer.Clip() == EFillBox::kContent) {
      clip_rect.Contract(
...
void BoxPainterBase::PaintFillLayerTextFillBox(
    ...
    Image *image, SkBlendMode composite_op,
    const BackgroundImageGeometry &geometry, const PhysicalRect &rect,
    const PhysicalRect &scrolled_paint_rect, bool object_has_multiple_boxes) {
...
void BoxPainterBase::PaintBorder(const ImageResourceObserver &obj,
                                 const Document &document, Node *node,
