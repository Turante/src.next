...
LocalFrameView::LocalFrameView(LocalFrame &frame, IntRect frame_rect)
    : FrameView(frame_rect), frame_(frame), can_have_scrollbars_(true),
      has_pending_layout_(false), layout_scheduling_enabled_(true),
      layout_count_for_testing_(0), lifecycle_update_count_for_testing_(0),
      ...
      update_plugins_timer_(frame.GetTaskRunner(TaskType::kInternalLoading),
                            this, &LocalFrameView::UpdatePluginsTimerFired),
      first_layout_(true), base_background_color_(Color::kWhite),
      media_type_(media_type_names::kScreen),
      safe_to_propagate_scroll_to_parent_(true),
      visually_non_empty_character_count_(0),
      visually_non_empty_pixel_count_(0), is_visually_non_empty_(false),
      sticky_position_object_count_(0), layout_size_fixed_to_frame_size_(true),
      needs_update_geometries_(false), root_layer_did_scroll_(false),
      ...
      needs_forced_compositing_update_(false), needs_focus_on_fragment_(false),
      main_thread_scrolling_reasons_(0), forced_layout_stack_depth_(0),
      forced_layout_start_time_(base::TimeTicks()), paint_frame_count_(0),
...
#if DCHECK_IS_ON()
      ,
      is_updating_descendant_dependent_flags_(false), is_updating_layout_(false)
#endif
{
...
bool LocalFrameView::DidFirstLayout() const { return !first_layout_; }
...
Page *LocalFrameView::GetPage() const { return GetFrame().GetPage(); }
...
LocalFrameView::GetCompositorAnimationTimeline() const {
...
void LocalFrameView::AddViewportConstrainedObject(
    LayoutObject &object, ViewportConstrainedType constrained_reason) {
...
void LocalFrameView::MarkViewportConstrainedObjectsForLayout(
    bool width_changed, bool height_changed) {
...
void LocalFrameView::ComputePostLayoutIntersections(
    unsigned parent_flags, absl::optional<base::TimeTicks> &monotonic_time) {
  if (ShouldThrottleRendering())
    return;
...
LocalFrameView::CompositedLayersAsJSON(LayerTreeFlags flags) {
...
void LocalFrameView::ForceLayoutForPagination(
    const gfx::SizeF &page_size, const gfx::SizeF &original_page_size,
...
gfx::Point
LocalFrameView::RootFrameToDocument(const gfx::Point &point_in_root_frame) {
  return gfx::ToFlooredPoint(
      RootFrameToDocument(gfx::PointF(point_in_root_frame)));
}

gfx::PointF
LocalFrameView::RootFrameToDocument(const gfx::PointF &point_in_root_frame) {
  ScrollableArea *layout_viewport = LayoutViewport();
  if (!layout_viewport)
...
DoublePoint
LocalFrameView::DocumentToFrame(const DoublePoint &point_in_document) const {
  ScrollableArea *layout_viewport = LayoutViewport();
  if (!layout_viewport)
...
gfx::Point
LocalFrameView::DocumentToFrame(const gfx::Point &point_in_document) const {
  return ToFlooredPoint(DocumentToFrame(DoublePoint(point_in_document)));
}

gfx::PointF
LocalFrameView::DocumentToFrame(const gfx::PointF &point_in_document) const {
...
PhysicalRect
LocalFrameView::DocumentToFrame(const PhysicalRect &rect_in_document) const {
  return PhysicalRect(DocumentToFrame(rect_in_document.offset),
                      rect_in_document.size);
}

gfx::Point
LocalFrameView::FrameToDocument(const gfx::Point &point_in_frame) const {
  return ToFlooredPoint(FrameToDocument(PhysicalOffset(point_in_frame)));
}

PhysicalOffset
LocalFrameView::FrameToDocument(const PhysicalOffset &offset_in_frame) const {
  ScrollableArea *layout_viewport = LayoutViewport();
  if (!layout_viewport)
...
PhysicalRect
LocalFrameView::FrameToDocument(const PhysicalRect &rect_in_frame) const {
...
ScrollableArea *
LocalFrameView::ScrollableAreaWithElementId(const CompositorElementId &id) {
...
PhysicalOffset
LocalFrameView::ViewportToFrame(const PhysicalOffset &point_in_viewport) const {
...
gfx::PointF
LocalFrameView::ViewportToFrame(const gfx::PointF &point_in_viewport) const {
...
gfx::Point
LocalFrameView::ViewportToFrame(const gfx::Point &point_in_viewport) const {
...
gfx::Point
LocalFrameView::FrameToViewport(const gfx::Point &point_in_frame) const {
...
void LocalFrameView::PaintOutsideOfLifecycle(
    GraphicsContext &context, const GlobalPaintFlags global_paint_flags,
    const CullRect &cull_rect) {
...
gfx::Point
LocalFrameView::ConvertToRootFrame(const gfx::Point &local_point) const {
  return ToRoundedPoint(ConvertToRootFrame(PhysicalOffset(local_point)));
}

PhysicalOffset
LocalFrameView::ConvertToRootFrame(const PhysicalOffset &local_offset) const {
  if (LocalFrameView *parent = ParentFrameView()) {
...
gfx::PointF
LocalFrameView::ConvertToRootFrame(const gfx::PointF &local_point) const {
  if (LocalFrameView *parent = ParentFrameView()) {
...
PhysicalRect
LocalFrameView::ConvertToRootFrame(const PhysicalRect &local_rect) const {
  if (LocalFrameView *parent = ParentFrameView()) {
    PhysicalOffset parent_offset =
...
IntRect
LocalFrameView::ConvertFromRootFrame(const IntRect &rect_in_root_frame) const {
  if (LocalFrameView *parent = ParentFrameView()) {
...
void LocalFrameView::CollectAnnotatedRegions(
    LayoutObject &layout_object, Vector<AnnotatedRegionValue> &regions) const {
...
unsigned
LocalFrameView::GetIntersectionObservationFlags(unsigned parent_flags) const {
  unsigned flags = 0;
...
LocalFrameView::MainThreadScrollingReasonsPerFrame() const {
...
MainThreadScrollingReasons
LocalFrameView::GetMainThreadScrollingReasons() const {
  MainThreadScrollingReasons reasons =
      static_cast<MainThreadScrollingReasons>(0);
...
bool LocalFrameView::MapToVisualRectInRemoteRootFrame(
    PhysicalRect &rect, bool apply_overflow_clip) {
  DCHECK(frame_->IsLocalRoot());
...
void LocalFrameView::NotifyVideoIsDominantVisibleStatus(
    HTMLVideoElement *element, bool is_dominant) {
  if (is_dominant) {
