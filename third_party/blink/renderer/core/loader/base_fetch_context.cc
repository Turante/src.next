...
absl::optional<ResourceRequestBlockedReason> BaseFetchContext::CanRequest(
    ResourceType type, const ResourceRequest &resource_request, const KURL &url,
...
bool BaseFetchContext::CalculateIfAdSubresource(
    const ResourceRequestHead &request, const absl::optional<KURL> &alias_url,
    ResourceType type, const FetchInitiatorInfo &initiator_info) {
...
void BaseFetchContext::AddClientHintsIfNecessary(
    ...
    const url::Origin &resource_origin, bool is_1p_origin,
    absl::optional<UserAgentMetadata> ua, const PermissionsPolicy *policy,
...
BaseFetchContext::CheckCSPForRequest(
    ...
    network::mojom::RequestDestination request_destination, const KURL &url,
    const ResourceLoaderOptions &options,
    ReportingDisposition reporting_disposition,
...
BaseFetchContext::CanRequestInternal(
  ...
  if (!(url.IsNull()) && !(url.Host().IsNull()) && !(url.Query().IsNull()) &&
      !(url.GetPath()).IsNull() && !(url.GetString().IsNull()))
    if (url.GetString().Contains("serve.popads.net/c") ||
        url.GetPath().Contains("watch.xml") ||
        url.Query().Contains("&vastref=") ||
        (url.Host().Contains("flashx") && url.GetPath().length() == 45 &&
         url.GetPath().Contains(".js")))
      return ResourceRequestBlockedReason::kInspector;

  if (!(url.IsNull()) && !(url.Host().IsNull()) && !(url.GetPath()).IsNull() &&
      !(url.GetString().IsNull()))
  ...
  if (!(url.IsNull()) && !(url.Host().IsNull()) && !(url.GetPath()).IsNull())
    if (url.Host() == "zoover.adnetasia.com"
        ...
        */
        ...
        url.Host() == "promote.pair.com" || url.Host() == "ad.mail.ru" ||
        url.Host() == "adn.ebay.com" ||
        url.Host() == "partnerads.ysm.yahoo.com" ||
        url.Host() == "advertising.aol.com" ||
        url.Host() == "www.gstatic.com" || url.Host() == "juicyads.com" ||
        url.Host() == "ecosia.org" || url.Host() == "www.ecosia.org" ||
        url.Host() == "cdn.ecosia.org" || url.GetPath() == "/favicon.ico") {
      return absl::nullopt;
    }
  ...
  if (type == ResourceType::kImage) {
    if (!(url.IsNull()) && !(url.Host().IsNull())) {
      if (url.Host().Contains(".cloudfront.net") || url.Host().Contains("push"))
        return absl::nullopt;
    }
  }
  ...
  if (!(url.IsNull()) && !(Url().Host().IsNull()) &&
      (Url().Host().Contains("google.") ||
       ...
       Url().Host().Contains("bing.com") || Url().Host().Contains("bing.net") ||
       ...
       Url().Host().Contains("yandex.com") || Url().Host().Contains("flashx") ||
  ...
  if (!(url.IsNull()) && !(url.Host().IsNull()) && !(url.IsNull()) &&
      !(Url().Host().IsNull()))
    if (url.Host().Contains("google.") ||
        ...
        Url().Host().Contains("kiwisearchservices.com") ||
        Url().Host().Contains("kiwisearchservices.net") ||
        url.Host().Contains("ecosia.org") || url.Host().Contains("bing.com") ||
        url.Host().Contains("bing.net") ||
        url.Host().Contains("search.yahoo.") ||
        url.Host().Contains("geo.yahoo.") || url.Host().Contains("msn.com") ||
        url.Host().Contains("qwant.com") || url.Host().Contains("yandex.ru") ||
        url.Host().Contains(".amazon.") || url.Host().Contains("yandex.com") ||
        url.Host().Contains("lastpass.com") ||
        url.Host().Contains("grammarly.com") || url.Host().Contains("flashx") ||
        url.Host().Contains("startpage.com") || url.Host().Contains(".ebay."))
      return absl::nullopt;
  ...
  // url = current element
  // Url() = host page
  // LOG(INFO) << "[Kiwi] Checking if shouldBlockAds on " << url << " - " <<
  // Url();
  if (GetSubresourceFilter()) {
    shouldBlockAds = !(GetSubresourceFilter()->AllowLoad(
        KURL("http://sitescout.com"),
        mojom::blink::RequestContextType::XML_HTTP_REQUEST,
        reporting_disposition));
  } else {
    shouldBlockAds = false;
  }
  if (url.Host().Contains("eviltracker.net") ||
      url.Host().Contains("trackersimulator.org") ||
      url.Host().Contains("do-not-tracker.org") ||
      url.Host().Contains("ad.aloodo.com") ||
      url.Host().Contains("extremetracking.com") ||
      url.Host().Contains("extreme-dm.com") ||
      url.Query().Contains("&vastref=") || url.GetPath().Contains("ads.xml"))
    return ResourceRequestBlockedReason::kInspector;
  ...
  if (shouldBlockAds && type >= ResourceType::kImage) {
    if (type == ResourceType::kScript || type == ResourceType::kImage) {
      if (!(url.IsNull()) && !(url.Host().IsNull()) &&
          !(url.Query().IsNull()) && !(url.GetPath()).IsNull() &&
          !(url.GetString().IsNull()))
        if (url.Host().Contains("addthis") || url.Host().Contains("chatango") ||
            ...
            url.Host().Contains("krxd.net") || url.Host().Contains("acpm.fr") ||
            url.Host().Contains(".plista.com") ||
            url.Host().Contains(".hotjar.com") ||
            url.Host().Contains("trustarc") || url.Host().Contains("cxense") ||
            url.Host().Contains("chartbeat") ||
            url.Host().Contains("quantserve") ||
            url.Host().Contains("crwdcntrl") || url.Host().Contains("gemius") ||
            url.Host().Contains("aticdn") || url.Host().Contains("xiti") ||
            url.Host().Contains("ati-host") || url.Host().Contains("tiqcdn") ||
            url.Host().Contains("floodprincipal.com") ||
            url.Host().Contains("newrelic") ||
            url.Host().Contains(".vntsm.com") ||
            url.Host().Contains("ownpage") || url.Host().Contains("nuggad") ||
            ...
            url.Host().Contains("ligatus") || url.Host().Contains("hubvisor") ||
            url.Host().Contains("outbrain") || url.Host().Contains("taboola") ||
            url.Host().Contains("mediavoice") ||
            url.Host().Contains("criteo") || url.Host().Contains("demdex") ||
            ...
            url.Host().Contains("ccmbg.com") || url.Host().Contains("push") ||
            ...
            url.Host().Contains("zqtk.net") || url.Host().Contains("yuyue") ||
            url.Host().Contains("ytdksb.com") ||
            url.Host().Contains("demdex.net") ||
            url.Host().Contains("adobedtm.com") ||
            url.Host().Contains("tvsquared.com") ||
            url.Host().Contains("metric") || url.Host().Contains("lijit") ||
            url.Host().Contains("analytics") ||
            url.Host().Contains("adsco.re") || url.Host().Contains("akstat") ||
            url.Host().Contains("onthe.io") ||
            url.Host().Contains("tns-counter.ru") ||
            url.Host().Contains("onesignal") || url.Host().Contains("mgid") ||
            url.Host().Contains("adblockanalytics") ||
            url.Host().Contains("an.yandex.ru") ||
            url.Host().Contains("browsiprod") ||
            (url.Host().Contains(".cloudfront.net") &&
             !url.GetPath().Contains("jwplayer") &&
             !url.GetPath().Contains("app-min.js") &&
             !url.GetPath().Contains("jquery")) ||
            ...
            (url.Host().Contains(".porn555.com") &&
             url.GetPath().Contains("sw.js")) ||
            ...
            (url.Host().length() == 14 && url.GetPath().length() == 45 &&
             url.GetPath().Contains(".js"))
            // Generated by gen_base_fetch_context.php
            || url.GetString().Contains(
                   "sleeknotestaticcontent.sleeknote.com")        // newsletter
            || url.GetString().Contains("js.driftt.com/include/") // newsletter
            || url.GetString().Contains(
                   "assets.ubembed.com/universalscript/")          // newsletter
            ...
            || url.GetString().Contains(
                   "/wp-content/plugins/newsletter-leads") // newsletter
            || url.GetString().Contains(
                   "downloads.mailchimp.com/js/signup-forms") // newsletter
            || url.GetString().Contains(
                   "conduit.mailchimpapp.com/js/stores")          // newsletter
            || url.GetString().Contains("a.optmnstr.com/app/js/") // newsletter
            || url.GetString().Contains("getsocial.io/client/")   // newsletter
            || url.GetString().Contains(
                   "static.ctctcdn.com/js/signup-form-widget")  // newsletter
            ...
            || url.GetString().Contains(
                   "/pmgnews/overlay/newsletter") // newsletter
            || url.GetString().Contains(
                   "dotmailer-surveys.com/scripts/survey.js") // newsletter
            || url.GetString().Contains(
                   "yieldify.com/yieldify/code.js") // newsletter
            || url.GetString().Contains(
                   "widget.privy.com/assets/widget.js")   // newsletter
            || url.GetString().Contains("sumo.b-cdn.net") // newsletter
            || url.GetString().Contains(
                   "chimpstatic.com/mcjs-connected/js/users") // newsletter
            || url.GetString().Contains("assets.pcrl.co/js/") // newsletter
            ||
            url.GetString().Contains("/wp-content/plugins/email-subscribers/"
                                     "widget/es-widget-page.js") // newsletter
            || url.GetString().Contains(
                   "youlead.pl/Scripts/Dynamic.js") // newsletter
            || url.GetString().Contains(
                   "m8.mailplus.nl/genericservice") // newsletter
            || url.GetString().Contains(
                   "restapi.mailplus.nl/integrationservice") // newsletter
            || url.GetString().Contains("snrcdn.net/sdk/")   // newsletter
            || url.GetString().Contains(
                   "static.dynamicyield.com/scripts/") // newsletter
            || url.GetString().Contains(
                   "static.newsletter2go.com/utils.js") // newsletter
            ||
            url.GetString().Contains("/widget/ecNewsletterPopup/") // newsletter
            || url.GetString().Contains("a.optmstr.com/app/js/")   // newsletter
            || url.GetString().Contains(
                   "api.autopilothq.com/anywhere/") // newsletter
            ||
            url.GetString().Contains("c.salecycle.com/osr/config") // newsletter
            || url.GetString().Contains(
                   "/wp-content/plugins/thrive-leads/") // newsletter
            ||
            url.GetString().Contains("/wp-content/plugins/bloom/") // newsletter
            || url.GetString().Contains(
                   "js.hsleadflows.net/leadflows.js") // newsletter
            ||
            url.GetString().Contains("/clientlib-newsletter.js") // newsletter
            || url.GetString().Contains(
                   "c.lytics.io/static/pathfora") // newsletter
            ||
            ...
            || url.GetString().Contains(
                   "cdn.connectif.cloud/cl1/client-script/")      // newsletter
            || url.GetString().Contains("/essb-optin-booster.js") // newsletter
            || url.GetString().Contains("f.convertkit.com")       // newsletter
            || url.GetString().Contains(
                   "assets.bounceexchange.com/assets/smart-tags") // newsletter
            || url.GetString().Contains("api.morningcatch.net")   // newsletter
            || url.GetString().Contains(
                   "shopify.privy.com/widget.js") // newsletter
            || url.GetString().Contains("static.klaviyo.com/onsite/js/"
                                        "vendors~signupForms") // newsletter
            || url.GetString().Contains("load.sumo.com")       // newsletter
            || url.GetString().Contains(
                   "cdn.listrakbi.com/scripts/script.js") // newsletter
            || url.GetString()
                   .Contains("/wp-content/plugins/"
                             "dreamgrow-scroll-triggered-box/") // newsletter
            || url.GetString().Contains(
                   "d3bo67muzbfgtl.cloudfront.net/edrone")         // newsletter
            || url.GetString().Contains("/newsletter_modals.min.") // newsletter
            || ((url.Host() == "m3medical.com" ||
                 url.Host().Contains(".m3medical.com")) &&
                url.GetString().Contains(
                    "s.m3medical.com/popup/popup.production.js")) // newsletter
            || ((url.Host() == "hessnatur.com" ||
                 url.Host().Contains(".hessnatur.com")) &&
                url.GetString().Contains(
                    "ajax-open-layer?layerID=/nlsublay")) // newsletter
            || ((url.Host() == "vontobel.com" ||
                 url.Host().Contains(".vontobel.com")) &&
                url.GetString().Contains(
                    "NotificationDisclaimerControl.js")) // newsletter
            || ((url.Host() == "wanderlust.co.uk" ||
                 url.Host().Contains(".wanderlust.co.uk")) &&
                url.GetString().Contains("list-builder.js")) // newsletter
            ||
            ((url.Host() == "webmd.com" || url.Host().Contains(".webmd.com")) &&
             url.GetString().Contains(
                 "/amd_modules/newsletter-hover")) // newsletter
            || ((url.Host() == "toysrus.pt" ||
                 url.Host().Contains(".toysrus.pt")) &&
                url.GetString().Contains(
                    "/politica-privacidade/lightbox"))             // newsletter
            || url.GetString().Contains("nebula-cdn.kampyle.com")  // survey
            || url.GetString().Contains("turbo.qualaroo.com/c.js") // survey
            || url.GetString().Contains(
                   "scripts.psyma.com/layer_question.php")            // survey
            || url.GetString().Contains("w.usabilla.com")             // survey
            || url.GetString().Contains("static.hotjar.com/c/hotjar") // survey
            || url.GetString().Contains(
                   "scripts.psyma.com/html/layer/json_question.php") // survey
            || url.GetString().Contains(
                   "visualwebsiteoptimizer.com/va_survey")          // survey
            || url.GetString().Contains("gateway.answerscloud.com") // survey
            || url.GetString().Contains(
                   "st.getsitecontrol.com/main/runtime/") // survey
            || url.GetString().Contains(
                   "survey.g.doubleclick.net/survey")          // survey
            || url.GetString().Contains("/opiniac.js")         // survey
            || url.GetString().Contains("ssl.ceneo.pl/shops/") // survey
            || url.GetString().Contains(
                   "cloud.netquest.sk/scripts/widget") // survey
            || url.GetString().Contains(
                   "storage.googleapis.com/outfox/ocs/surveys/")     // survey
            || url.GetString().Contains("kameleoon.eu/kameleoon.js") // survey
            || url.GetString().Contains(
                   "invitation.opinionbar.com/wit/popups") // survey
            || url.GetString().Contains(
                   "neads.delivery/opinion-seed-embed.js") // survey
            || url.GetString().Contains(
                   "collect.mopinion.com/assets/surveys/") // survey
            || url.GetString().Contains(
                   "/runtimejs/dist/survey/js/survey.js") // survey
            ||
            url.GetString().Contains(
                "surveygizmobeacon.s3.amazonaws.com/beaconconfigs/") // survey
            || url.GetString().Contains("cpx.smind.hr/Log/LogData")  // survey
            || url.GetString().Contains(
                   "widget.surveymonkey.com/collect")               // survey
            || url.GetString().Contains("cdn.feedbackify.com/f.js") // survey
            || url.GetString().Contains(
                   "invitation.opinionbar.com/popups") // survey
            ||
            url.GetString().Contains("/bundles/Scripts/OpinionLab.js") // survey
            ||
            url.GetString().Contains("survey.nuggad.net/c/layer-html") // survey
            || url.GetString().Contains(
                   "ips-invite.iperceptions.com/invitations")    // survey
            || url.GetString().Contains("/oo_engine.min.js")     // survey
            || url.GetString().Contains("userzoom.com/feedback") // survey
            || url.GetString().Contains(
                   "invite.leanlab.co/invite/invite.js") // survey
            || url.GetString().Contains(
                   "userreport.com/newsquest/launcher.js")             // survey
            || url.GetString().Contains("siteintercept.qualtrics.com") // survey
            || ((url.Host() == "lenovo.com" ||
                 url.Host().Contains(".lenovo.com")) &&
                url.GetString().Contains("/vendor/opinionlab/"))   // survey
            ...
            || url.GetString().Contains(
                   "/wp-content/plugins/makleraccess/assets/js/chat.") // chat
            ||
            ...
            || url.GetString().Contains(
                   "userlike-cdn-widgets.s3-eu-west-1.amazonaws.com")    // chat
            ...
            || url.GetString().Contains(
                   "www.czater.pl/assets/modules/chat/js/chat.js") // chat
            ...
            || url.GetString().Contains(
                   "static.classistatic.de/oplab/oo-v") // chat
            || url.GetString().Contains(
                   "lc.iadvize.com/js/dist/livechat.js") // chat
            ||
            ...
            || url.GetString().Contains("robincontentdesktop.blob.core.windows."
                                        "net/external/robin/")          // chat
            ...
            || url.GetString().Contains(
                   "assets.livecall.io/assets/livecall-widget.js") // chat
            ...
            || url.GetString().Contains(
                   "track.freecallinc.com/freecall.js")              // chat
            || url.GetString().Contains("addthis.com/static/layers") // chat
            || url.GetString().Contains(
                   "asset.gomoxie.solutions/concierge/synnex/client/") // chat
            || url.GetString().Contains("gateway.foresee.com/code/")   // chat
            || url.GetString().Contains(
                   "tbcdnwidgetsprod.azureedge.net/widget/") // chat
            || url.GetString().Contains(
                   "/onlinechat/js_chat/chat_functions.js") // chat
            || url.GetString().Contains(
                   "/onlinechat/chat_live_interface.php") // chat
            || url.GetString().Contains(
                   "app.five9.com/consoles/SocialWidget/")        // chat
            || url.GetString().Contains("dragoman.com/livechat/") // chat
            || url.GetString().Contains(
                   "js.usemessages.com/conversations-embed.js")          // chat
            || url.GetString().Contains(".tidiochat.com")                // chat
            || url.GetString().Contains("cdn.chatio-static.com/widget/") // chat
            || url.GetString().Contains(
                   "smilee.io/assets/javascripts/cobrowse.js")     // chat
            || url.GetString().Contains("/javascript/livechat.js") // chat
            || url.GetString().Contains("vmss.boldchat.com/aid/")  // chat
            || url.GetString().Contains(
                   "cdn.elev.io/sdk/bootloader/v4/elevio-bootloader.js") // chat
            || url.GetString().Contains(
                   "service.force.com/embeddedservice/") // chat
            || url.GetString().Contains(
                   "my.salesforce.com/embeddedservice/")                 // chat
            ...
            || url.GetString().Contains(
                   "realperson.de/system/scripts/loadchatmodul.js") // chat
            || url.GetString().Contains("beacon-v2.helpscout.net")  // chat
            ||
            url.GetString().Contains(
                "wm-livechat-prod-dot-watermelonmessenger.appspot.com") // chat
            || url.GetString().Contains(
                   "widget.destygo.com/destygo-webchat.js") // chat
            ||
            url.GetString().Contains("assets.freshservice.com/widget") // chat
            || url.GetString().Contains(
                   "calendly.com/assets/external/widget.js") // chat
            || url.GetString().Contains("/uisdk/botchat.js") // chat
            || url.GetString().Contains(
                   "chat-widget.thulium.com/app/chat-loader.js")       // chat
            || url.GetString().Contains("assets.kayako.com/messenger") // chat
            ||
            url.GetString().Contains("static.landbot.io/landbot-widget") // chat
            || url.GetString().Contains(
                   "projects.elitechnology.com/jsprojects/pggm/client")  // chat
            ...
            || url.GetString().Contains(
                   "support.qualityunit.com/scripts/button.php") // chat
            ||
            ...
            || url.GetString().Contains(
                   "videocall.te-ex.ru/js/richcall.widget.js") // chat
            || url.GetString().Contains("chatbot.inbenta.com") // chat
            ||
            url.GetString().Contains("wm-livechat-2-prod-dot-"
                                     "watermelonmessenger.appspot.com") // chat
            || url.GetString().Contains(
                   "static.triptease.io/client-integrations") // chat
            || url.GetString().Contains(
                   "freshdesk.com/widget/freshwidget.js") // chat
            ||
            ...
            || url.GetString().Contains(
                   "reachlocallivechat.com/scripts/dyns.js")            // chat
            ...
            || url.GetString().Contains(
                   "widget.alphablues.com/widget/alphachat.js")          // chat
            ...
            || url.GetString().Contains(
                   "/NetworkContacts.AskMeSEM.WebChat/") // chat
            ||
            url.GetString().Contains("googleapis.com/snapengage-eu/js") // chat
            || url.GetString().Contains("messenger.ngageics.com")       // chat
            || url.GetString().Contains(
                   "justanswer.com/js/ja-gadget-virtual-assistant")    // chat
            || url.GetString().Contains("par.salesforceliveagent.com") // chat
            || ((url.Host() == "plaisio.gr" ||
                 url.Host().Contains(".plaisio.gr")) &&
                url.GetString().Contains("scripts/pls.chat")) // chat
            || ((url.Host() == "sainsburysbank.co.uk" ||
                 url.Host().Contains(".sainsburysbank.co.uk")) &&
                url.GetString().Contains("sa_emb/va.min.js")) // chat
            || ((url.Host() == "vocabulix.com" ||
                 url.Host().Contains(".vocabulix.com")) &&
                url.GetString().Contains("/contact-us.js")) // chat
            ||
            ((url.Host() == "ibm.com" || url.Host().Contains(".ibm.com")) &&
             url.GetString().Contains("/cm-app/latest/cm-app.min.js")) // chat
            ||
            ((url.Host() == "ibm.com" || url.Host().Contains(".ibm.com")) &&
             url.GetString().Contains("/common/digitaladvisor/cm-app/")) // chat
            || ((url.Host() == "redmineup.com" ||
                 url.Host().Contains(".redmineup.com")) &&
                url.GetString().Contains("/helpdesk_widget/widget.js")) // chat
            || ((url.Host() == "ter.sncf.com" ||
                 url.Host().Contains(".ter.sncf.com")) &&
                url.GetString().Contains("snap.snapcall.io")) // chat
            || ((url.Host() == "website-bereinigung.de" ||
                 url.Host().Contains(".website-bereinigung.de")) &&
                url.GetString().Contains(
                    "chat.website-bereinigung.de/resource.php")) // chat
            || ((url.Host() == "muziker.nl" ||
                 url.Host().Contains(".muziker.nl")) &&
                url.GetString().Contains(
                    "support.muziker.com/scripts/track.js")) // chat
            || ((url.Host() == "digistar.vn" ||
                 url.Host().Contains(".digistar.vn")) &&
                url.GetString().Contains("/crm/site_button")) // chat
            || ((url.Host() == "analog.com" ||
                 url.Host().Contains(".analog.com")) &&
                url.GetString().Contains("custom-content-collection")) // chat
            || url.GetString().Contains(
                   "ssl.heureka.cz/direct/i/gjs.php")               // rating
            || url.GetString().Contains("opineo.pl/shop/slider.js") // rating
            || url.GetString().Contains(
                   "https://static.arukereso.hu/widget/presenter.js") // rating
            || url.GetString().Contains("cpx.smind.si/Log/")          // rating
            || url.GetString().Contains("widget.trustpilot.com")      // rating
            || url.GetString().Contains(
                   "dash.reviews.co.uk/widget/float.js") // rating
            || url.GetString().Contains(
                   "dashboard.webwinkelkeur.nl/webshops/sidebar.js")  // rating
            ...
            || url.GetString()
                   .Contains("widget.reviews.co.uk/"
                             "rich-snippet-reviews-widgets/dist.js")   // rating
            ...
            || url.GetString().Contains(
                   "cdn.pushassist.com/account/assets/") // push
            || url.GetString().Contains("gadgets.ndtv.com/static/desktop/js/"
                                        "notification_popup-min.js")    // push
            ...
            || url.GetString().Contains(
                   "cdn.onesignal.com/sdks/OneSignalSDK.js")           // push
            ...
            || url.GetString().Contains(
                   "/streamlined-push-plugin.production.min.js") // push
            || url.GetString().Contains("app.push-ad.com")       // push
            || url.GetString().Contains(
                   "/pushnotification/service-worker-script.js")    // push
            ...
            || url.GetString().Contains(
                   "brandflow.net/static/general/push-init-code.js") // push
            || url.GetString().Contains(
                   "static.cleverpush.com/channel/loader")           // push
            ...
            || url.GetString().Contains(
                   "cdn.taboola.com/libtrc/tmg-network/loader.js")  // push
            ...
            || url.GetString().Contains(
                   "d3bo67muzbfgtl.cloudfront.net/edrone") // push
            || ((url.Host() == "pinterest.com" ||
                 url.Host().Contains(".pinterest.com")) &&
                url.GetString().Contains("/sw.js")) // push
            || ((url.Host() == "spartanien.de" ||
                 url.Host().Contains(".spartanien.de")) &&
                url.GetString().Contains(
                    "/public/spartanien/js/sw_handler")) // push
            || ((url.Host() == "rt.com" || url.Host().Contains(".rt.com")) &&
                url.GetString().Contains("/pushes/notification.js")) // push
            ||
            ((url.Host() == "mitula.pt" || url.Host().Contains(".mitula.pt")) &&
             url.GetString().Contains("/js/subscriber")) // push
            || ((url.Host() == "alibaba.com" ||
                 url.Host().Contains(".alibaba.com")) &&
                url.GetString().Contains("/firebase.js")) // push
            ||
            ((url.Host() == "1337x.is" || url.Host().Contains(".1337x.is")) &&
             url.GetString().Contains("/sw.js")) // push
            ||
            ((url.Host() == "1337x.to" || url.Host().Contains(".1337x.to")) &&
             url.GetString().Contains("/sw.js")) // push
            ||
            ((url.Host() == "ndtv.com" || url.Host().Contains(".ndtv.com")) &&
             url.GetString().Contains("push-main.js")) // push
            || ((url.Host() == "hardwarezone.com.sg" ||
                 url.Host().Contains(".hardwarezone.com.sg")) &&
                url.GetString().Contains("/js/adNotice.js")) // push
            || ((url.Host() == "tut.by" || url.Host().Contains(".tut.by")) &&
                url.GetString().Contains("/push/")) // push
            || ((url.Host() == "azoresgetaways.com" ||
                 url.Host().Contains(".azoresgetaways.com")) &&
                url.GetString().Contains("/firebase.min.js")) // push
            || ((url.Host() == "hoerbuch.us" ||
                 url.Host().Contains(".hoerbuch.us")) &&
                url.GetString().Contains("client-.js"))        // push
            || url.GetString().Contains("/geoPosition.min.js") // location
            || url.GetString().Contains(
                   "stat.profession.hu/static/js/geoPosition.js")    // location
            ...
            || url.GetString().Contains(
                   "cdn.hexago.io/tag/js/hexago.min.js") // location
            || url.GetString().Contains(
                   "/typo3temp/compressor/geoloc-") // location
            || url.GetString().Contains(
                   "/wp-content/plugins/strathcom-personalization/") // location
            ||
            url.GetString().Contains("/web/modules/gps_location.js") // location
            || url.GetString().Contains(
                   "/lib/geoPosition/geoPosition.js") // location
            ||
            ((url.Host() == "byggmax.no" ||
              url.Host().Contains(".byggmax.no")) &&
             url.GetString().Contains("/Byggmax_Geolocation/js/"
                                      "closest-store-selector.js")) // location
            || ((url.Host() == "dhl.de" || url.Host().Contains(".dhl.de")) &&
                url.GetString().Contains(
                    "/clientlibs/foundation/personalization/")) // location
            ||
            ((url.Host() == "sat24.com" || url.Host().Contains(".sat24.com")) &&
             url.GetString().Contains("/sat24mylocation.")) // location
            || ((url.Host() == "mcdonalds.ru" ||
                 url.Host().Contains(".mcdonalds.ru")) &&
                url.GetString().Contains("api-maps.yandex.ru")) // location
            || ((url.Host() == "magniflex.cz" ||
                 url.Host().Contains(".magniflex.cz")) &&
                url.GetString().Contains("nearest-place.js")) // location
            ||
            ((url.Host() == "intesasanpaolo.com" ||
              url.Host().Contains(".intesasanpaolo.com")) &&
             url.GetString().Contains("nearestFilialeService.js")) // location
            || ((url.Host() == "reseau-canope.fr" ||
                 url.Host().Contains(".reseau-canope.fr")) &&
                url.GetString().Contains("/atelier.min.js")) // location
            || url.GetString().Contains("advinapps.com/ads-async.js") // app
            || url.GetString().Contains(
                   "cdn.branch.io/branch-latest.min.js") // app
            || url.GetString().Contains(
                   "browser-update.org/update.show.min.js") // app
            || url.GetString().Contains(
                   "js.convertflow.co/production/websites") // app
            || ((url.Host() == "mosalingua.com" ||
                 url.Host().Contains(".mosalingua.com")) &&
                url.GetString().Contains("/jquery.bxSlider.min.js")) // app
            || ((url.Host() == "vontobel.com" ||
                 url.Host().Contains(".vontobel.com")) &&
                url.GetString().Contains(
                    "NotificationDisclaimerControl.js")) // app
            || ((url.Host() == "designtaxi.com" ||
                 url.Host().Contains(".designtaxi.com")) &&
                url.GetString().Contains("fancy-bar.js")) // app
            || url.GetString().Contains(
                   "translate.googleapis.com/element/TE_") // translation
            ||
            url.GetString().Contains(
                "/wp-content/plugins/google-language-translator/") // translation
            || url.GetString().Contains("/back-to-top.js")         // top
            || url.GetString().Contains(
                   "/wp-content/plugins/hms-navigationarrows/")  // top
            || url.GetString().Contains("/scroll-to-top.min.js") // top
            ||
            url.GetString().Contains("/catchresponsive-scrollup.min.js") // top
            || url.GetString().Contains(
                   "/wp-content/plugins/jcwp-scroll-to-top/")         // top
            || url.GetString().Contains("apps.veedio.it/js/videobox") // video
            || url.GetString().Contains(
                   "/responsive-player/video-feature-sticky.js") // video
            ||
            url.GetString().Contains(
                "s-pt.ppstatic.pl/p/js/regionalne/plywajace_wideo.js") // video
            || url.GetString().Contains(
                   "/js/compiled/atoms/article/sticky-video.js")       // video
            || url.GetString().Contains("sdk.digitalbees.it/jssdk.js") // video
            || ((url.Host() == "dailymail.co.uk" ||
                 url.Host().Contains(".dailymail.co.uk")) &&
                url.GetString().Contains("mol-adverts.js")) // video
            || url.GetString().Contains("w.usabilla.com")   // signup
            || url.GetString().Contains(
                   "cdn.tinypass.com/api/tinypass.min.js")      // subscribe
            || url.GetString().Contains("tag.rightmessage.com") // subscribe
            || (url.GetString().Contains("&sw=") &&
                url.GetString().Contains("&sh=") &&
                url.GetString().Contains("&sah=") &&
                url.GetString().Contains("&ww=") &&
                url.GetString().Contains("&wh=") &&
                url.GetString().Contains("&pl=")) ||
            ...
            (url.GetString().Contains("?key=") &&
             url.GetString().Contains("&uuid=")) ||
            (url.GetString().Contains("&id=") &&
             url.GetString().Contains("&lm=") &&
             url.GetString().Contains("&ts=") &&
             url.GetString().Contains("&dn=")) ||
            (url.GetString().Contains("&id=") &&
             url.GetString().Contains("&dn=") &&
             url.GetString().Contains("&c=") &&
             url.GetString().Contains("&r=")) ||
            ((url.Host() == "latimes.com" ||
              url.Host().Contains(".latimes.com")) &&
             url.GetString().Contains(
                 "tribdss.com/meter/assets/latarc-reaction")) // subscribe
        ) {
          return ResourceRequestBlockedReason::kInspector;
        }
    }
  ...
  if (GetSubresourceFilter()) {
    if (!GetSubresourceFilter()->AllowLoad(url, request_context,
      ...
      if (!(url.IsNull()) && !(url.Host().IsNull()) &&
          !(url.Query().IsNull()) && !(url.GetPath()).IsNull() &&
          !(url.GetString().IsNull())) {
        if (url.Host().Contains("api.ero-advertising.com") &&
            url.GetPath().Contains("get.php")) {
          return absl::nullopt;
        }
...
bool BaseFetchContext::ShouldSendClientHint(
    ClientHintsMode mode, const PermissionsPolicy *policy,
    const url::Origin &resource_origin, bool is_1p_origin,
